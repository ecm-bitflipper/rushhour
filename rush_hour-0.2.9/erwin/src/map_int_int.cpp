...
#ifndef MAP_INT_INT_IMPLEMENTATION
#define MAP_INT_INT_IMPLEMENTATION
#endif
...
#ifdef INT_BEING_USED
#define OBEING_USED INT_BEING_USED
#endif
...
#ifdef INT_OCOPY
#define OCOPY INT_OCOPY
#endif

#ifdef INT_OCOPY_PROVIDED
#define OCOPY_PROVIDED INT_OCOPY_PROVIDED
#endif

#ifdef INT_DETCMP
#define ODETCMP INT_DETCMP
#endif

#ifdef INT_DETCMP_PROVIDED
#define ODETCMP_PROVIDED INT_DETCMP_PROVIDED
#endif

#ifdef INT_EQUAL
#define OEQUAL INT_EQUAL
#endif

#ifdef INT_OFREE
#define OFREE INT_OFREE
#endif

#ifdef INT_OFREE_PROVIDED
#define OFREE_PROVIDED INT_OFREE_PROVIDED
#endif

#ifdef INT_HASH
#define OHASH INT_HASH
#endif

#ifdef INT_PRINT
#define OPRINT INT_PRINT
#endif

#ifdef INT_ZERO
#define OZERO INT_ZERO
#endif
...
#if MAP_INT_INT_INITIAL_SIZE < 0
#  error MAP_INT_INT_INITIAL_SIZE < 0
#endif

#if MAP_INT_INT_MINIMAL_SIZE < 0
#  error MAP_INT_INT_MINIMAL_SIZE < 0
#endif

#if MAP_INT_INT_MINIMAL_SIZE == 0
#  define MAP_INT_INT_ZERO_SIZE         1
#  define MAP_INT_INT_MINIMAL_SIZE_OR_1 1
#elif MAP_INT_INT_INITIAL_SIZE == 0
#  define MAP_INT_INT_ZERO_SIZE         1
#  define MAP_INT_INT_MINIMAL_SIZE_OR_1 MAP_INT_INT_MINIMAL_SIZE
#else
#  define MAP_INT_INT_ZERO_SIZE         0
#  define MAP_INT_INT_MINIMAL_SIZE_OR_1 MAP_INT_INT_MINIMAL_SIZE
#endif

#if MAP_INT_INT_ZERO_SIZE

#  define HAS_HEAP_TABLE_AUX(hashsize)    ((hashsize) > 1)

#  define GET_SLOT(table,hashsize,i)      (HAS_HEAP_TABLE_AUX(hashsize) ? \
                                              (table[(i)])                \
                                            : *((map_int_int_hashlist_t**)(&(table)))     \
                                          )
...
#  else
#      define e_errno (ERWIN_CONST_CAST(map_int_int_t*,self)->m_errno)
#  endif
#else
...
#if MAP_INT_INT_HAVE_SIG
#  define return_if_bad(self) \
      return_if_fail_ppxxt(self->m_sig == MAP_INT_INT_SIG, self, self->m_sig, MAP_INT_INT_SIG, "Structure was probably overwritten");
#  define return_val_if_bad(self, val) \
      return_val_if_fail_ppxxt(self->m_sig == MAP_INT_INT_SIG, val, self, self->m_sig, MAP_INT_INT_SIG, "Structure was probably overwritten");
#  define return_if_bad_pC(self, prog) \
      return_if_fail_ppxxtC(self->m_sig == MAP_INT_INT_SIG, self, self->m_sig, MAP_INT_INT_SIG, "Structure was probably overwritten", prog);
#else
#  define return_if_bad(self)
#  define return_val_if_bad(self, val)
#  define return_if_bad_pC(self, prog)
...
#define USER_ASSERTION_FAILED_HANDLER(FI,L,FU,A)    \
            ASSERTION_FAILED;                       \
            MAP_INT_INT_ASSERTION_FAILED_HANDLER(FI,L,FU,A)
#endif
...
#define TOOSMALL(X,Y) (((X) * MAP_INT_INT_TRIGGER_DENOMINATOR) > ((Y) * MAP_INT_INT_TRIGGER_NUMERATOR))

#define TOOLARGE(X,Y) (((Y) >= MAP_INT_INT_MINIMAL_SIZE) && \
                       (((X) * MAP_INT_INT_TRIGGER_DENOMINATOR) < (((Y) * MAP_INT_INT_TRIGGER_NUMERATOR)/4)))
...
#define ITER               ((map_int_int_internal_Iterator*)iter)
#define SORTITER           ((map_int_int_internal_sorted_Iterator*)iter)
#define SORTITERPTR        ((map_int_int_internal_sorted_ptr_Iterator*)iter)

#ifdef E_PROFILE
typedef union {
   map_int_int_t       *mapt;
   map_int_int_t const *mapconstt;
} hack_no_const;
#endif
 ...
 *    map_int_int_index_t == i_type_t
 *    map_int_int_value_t == o_type_t
 */
...
#define SPLIT_SECTIONS MAP_INT_INT_SPLIT
...
/* Check that MAP_INT_INT_ALLOW_NULL and ERWIN_THREAD_SAFE are not used at
...
#if 0
/* FIXME: NYI */
#if MAP_INT_INT_ALLOW_NULL && defined(ERWIN_THREAD_SAFE)
#  error "Cannot use MAP_INT_INT_ALLOW_NULL and ERWIN_THREAD_SAFE at the same time."
#endif
#endif

#if MAP_INT_INT_NOMEM_IS_FATAL
...
#if MAP_INT_INT_DYN_ZERO
#  define map_int_int_init_internal_q(SE,Z,SI)  map_int_int_init_internal(SE,Z,SI)
#else
#  define map_int_int_init_internal_q(SE,Z,SI) map_int_int_init_internal(SE,SI)
#endif



#  define feature_q(K,V)                feature(K,V)
#  define map_int_int_hashlist_new_q(K,V)               map_int_int_hashlist_new(K,V)
#  define map_int_int_find_any_pair_q(K,V,SE)   map_int_int_find_any_pair(K,V,SE)
#  define map_int_int_insert_q(SE,K,V)          map_int_int_insert(SE,K,V)

#  define map_int_int_insert_no_copy_no_rehash_q(SE,K,V) \
                                        map_int_int_insert_no_copy_no_rehash(SE,K,V)

#  define map_int_int_poke_internal_q(KO,VO,SE,K,KC,DK,V,VK,DV,I,O) \
                                        map_int_int_poke_internal(KO,VO,SE,K,KC,DK,V,VK,DV,I,O)

#  define map_int_int_poke_q(KO,VO,SE,K,V,I,O)  map_int_int_poke(KO,VO,SE,K,V,I,O)

#  define map_int_int_poke_no_icopy_q(KO,VO,SE,K,V,I,O)  \
                                        map_int_int_poke_no_icopy(KO,VO,SE,K,V,I,O)

#  define map_int_int_remove_internal_no_resize_q(SE,K,R) \
                                        map_int_int_remove_internal_no_resize(SE,K,R)

#  define map_int_int_remove_internal_q(SE,K,R) map_int_int_remove_internal(SE,K,R)
...
static
TYPE_STRUCT_T(map_int_int_hashlist_t)=
    TYPE_STRUCT_BEGIN(map_int_int_hashlist_t)
        TYPE_STRUCT_SLOT(map_int_int_hashlist_t, int, key)
        TYPE_STRUCT_SLOT(map_int_int_hashlist_t, int, value)
        TYPE_STRUCT_SLOT(map_int_int_hashlist_t, map_int_int_hashlist_t_p,        next)
    TYPE_STRUCT_END
;


TYPE_INFO_T(map_int_int_hashlist_t)= TYPE_INFO_STRUCT (map_int_int_hashlist_t, map_int_int_hashlist_t);


TYPE_INFO_T(map_int_int_hashlist_t_p)= TYPE_INFO_POINTER (map_int_int_hashlist_t *, map_int_int_hashlist_t);


TYPE_INFO_T(map_int_int_hashlist_t_const_p)= TYPE_INFO_POINTER (map_int_int_hashlist_t const *, map_int_int_hashlist_t);


TYPE_INFO_T(map_int_int_content)= TYPE_INFO_VECTOR (map_int_int_content, map_int_int_hashlist_t_p, map_int_int_t, m_hashsize);


TYPE_INFO_T(map_int_int_content_p)= TYPE_INFO_POINTER (map_int_int_content *, map_int_int_content);

static
TYPE_STRUCT_T(map_int_int_internal_Iterator)=
    TYPE_STRUCT_BEGIN(map_int_int_internal_Iterator)
        TYPE_STRUCT_SLOT(map_int_int_internal_Iterator, map_int_int_hashlist_t_p, cnode)
        TYPE_STRUCT_SLOT(map_int_int_internal_Iterator, int,      cslot)
    TYPE_STRUCT_END
;


TYPE_INFO_T(map_int_int_internal_Iterator)=
    TYPE_INFO_STRUCT (map_int_int_internal_Iterator, map_int_int_internal_Iterator);

static
TYPE_STRUCT_T(map_int_int_t)=
    TYPE_STRUCT_BEGIN(map_int_int_t)
#ifdef  MAP_INT_INT_SUPER_TYPE
        TYPE_SUPER(map_int_int_t, MAP_INT_INT_SUPER_CLASS),
#endif
        MAP_INT_INT_TYPE_INFO_STD_MEMBERS
        map_int_int_record_sig     (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_sig))
        map_int_int_record_table   (TYPE_STRUCT_SLOT(map_int_int_t, map_int_int_content_p,   m_table))
        map_int_int_record_sizes   (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_hashsize))
        map_int_int_record_sizes   (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_nentries))
        map_int_int_record_errno   (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_errno))
        map_int_int_record_zero    (TYPE_STRUCT_SLOT(map_int_int_t, int, m_zero))
        map_int_int_record_compat  (TYPE_STRUCT_SLOT(map_int_int_t, map_int_int_internal_Iterator, compat1_iter))
        map_int_int_record_profile (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_nrehash))
        map_int_int_record_profile (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_nrehashops))
        map_int_int_record_profile (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_nfind))
        map_int_int_record_profile (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_ninsert))
        map_int_int_record_profile (TYPE_STRUCT_SLOT(map_int_int_t, int,             m_ndelete))
    TYPE_STRUCT_END
;


TYPE_INFO_T(map_int_int_t)= TYPE_INFO_STRUCT (map_int_int_t, map_int_int_t);


TYPE_INFO_T(map_int_int_t_p)= TYPE_INFO_POINTER (map_int_int_t *, map_int_int_t);


TYPE_INFO_T(map_int_int_t_const_p)= TYPE_INFO_POINTER (map_int_int_t const *, map_int_int_t);
...
map_int_int_hashlist_t * map_int_int_hashlist_new_q
        (int  key, int  value)
{
    map_int_int_hashlist_t *self;
    E_SET_PTR(self, ERWIN_THING_NEW1 (map_int_int_hashlist_t));
...
void map_int_int_hashlist_delete
        (map_int_int_hashlist_t *list)
{
    map_int_int_hashlist_t *next;

    while (list) {
        IFREE (list->key);
        OFREE (list->value);
        E_SET_PTR(next, list->next);
        ERWIN_THING_DELETE1 (list, map_int_int_hashlist_t);
        E_SET_PTR(list, next);
    }
}
...
void map_int_int_hashlist_delete_flags
        (map_int_int_hashlist_t *list, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    map_int_int_hashlist_t *next;
...
void map_int_int_hashlist_delete_no_child_delete
        (map_int_int_hashlist_t *list)
{
    map_int_int_hashlist_t *next;
...
void map_int_int_hashlist_delete_cell
        (map_int_int_hashlist_t *list)
{
    ERWIN_THING_DELETE1 (list, map_int_int_hashlist_t);
}
...
int map_int_int_hashlist_length
        (map_int_int_hashlist_t *list)
{
    int result= 0;
    while (list) {
...
int map_int_int_errno
        (map_int_int_t const *self)
{
...
ERWIN_BOOL map_int_int_init_internal_q
        (map_int_int_t *self, int  zero, int nelem)
{
    int size;
#ifndef NDEBUG
    int err= 0;
    ...
    if (nelem >= 0) { /* Compute hash size from expected number of elements: */
        size= ((nelem * MAP_INT_INT_TRIGGER_DENOMINATOR + MAP_INT_INT_TRIGGER_NUMERATOR - 1) / /* round up */
                MAP_INT_INT_TRIGGER_NUMERATOR * 2
              ) + 1;
    }

    if (size < 0)
        size= MAP_INT_INT_INITIAL_SIZE;

#if  MAP_INT_INT_HAVE_SIG
    self->m_sig= MAP_INT_INT_SIG;
#endif
    ...
    if (size == 1)    /* this means that the slot table is missing and we have a single list. */
        ...
        E_SET_PTR(self->m_table, ERWIN_THING_CNEW_ARRAY (map_int_int_content, (unsigned)size));
...
int map_int_int_init
        (map_int_int_t *self)
{
    map_int_int_init_internal_q (self, OZERO, MAP_INT_INT_INITIAL_SIZE);
    return e_errno;
}

CODE_SECTION(".text.init_with_initial_size")
int map_int_int_init_with_initial_size
        (map_int_int_t *self, int size)
{
    map_int_int_init_internal_q (self, OZERO, size);
    return e_errno;
}
...
int map_int_int_init_with_zero_and_initial_size
        (map_int_int_t *self, int  zero, int size)
{
    map_int_int_init_internal_q (self, zero, size);
    return e_errno;
}
...
map_int_int_t * map_int_int_new_with_zero_and_initial_size
        (int  zero, int size)
{
    map_int_int_t *self;

    E_SET_PTR(self, ERWIN_THING_NEW1 (map_int_int_t));
    SET_ERRNO(E_OK);
    ...
    if (ERWIN_LIKELY(map_int_int_init_internal_q (self, zero, size)))
       return self;

    ERWIN_THING_DELETE1 (self, map_int_int_t);
    return NULL;
}
...
map_int_int_t * map_int_int_new_with_zero
        (int  zero)
{
    return map_int_int_new_with_zero_and_initial_size (zero, MAP_INT_INT_INITIAL_SIZE);
}

#endif /* MAP_INT_INT_DYN_ZERO */

#if MAP_INT_INT_CONSTANT_ZERO
#define map_int_int_new_with_zero_and_initial_size(Z,S) map_int_int_new_with_initial_size(S)
#define map_int_int_new_with_zero(Z)                    map_int_int_new()
#endif
...
map_int_int_t * map_int_int_new_with_initial_size
        (int size)
{
    map_int_int_t *self;
    ...
    if (ERWIN_LIKELY(map_int_int_init_internal_q (self, OZERO, size)))
       return self;
...
map_int_int_t * map_int_int_new
        (void)
{
    return map_int_int_new_with_zero_and_initial_size (OZERO, MAP_INT_INT_INITIAL_SIZE);
}


CODE_SECTION(".text.zero")
o_type_result_t map_int_int_zero
        (map_int_int_t const *self)
{
    (void)self; /*unused*/
...
void map_int_int_struct_copy
        (map_int_int_t *copy, map_int_int_t const *self)
{
    int i;

    for (i= 0; i < self->m_hashsize; i++) {
       map_int_int_hashlist_t *lauf;
       E_SET_PTR(lauf, SLOT(self,i));
       while (lauf) {
           map_int_int_insert_q (copy, lauf->key, lauf->value);
...
map_int_int_t * map_int_int_copy
        (map_int_int_t const* self)
{
    map_int_int_t *copy;
    if (self == NULL) {
    ...
    E_SET_PTR(copy, map_int_int_new_with_zero_and_initial_size (ZERO(self), self->m_nentries));
    map_int_int_struct_copy (copy, self);
    return copy;
}

CODE_SECTION(".text.copy_err")
map_int_int_t * map_int_int_copy_err
        (map_int_int_t const* self, int *err)
{
    map_int_int_t *result;

    E_SET_PTR(result, map_int_int_copy (self));
...
int map_int_int_hash_size
        (map_int_int_t const* self)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return 0;
#endif
...
void map_int_int_clear_internal
        (map_int_int_t* self)
{
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        map_int_int_hashlist_delete (SLOT(self,i));
        E_SET_PTR(SLOT(self,i), NULL);
    }
}

CODE_SECTION(".text.clear_no_resize")
void map_int_int_clear_no_resize
        (map_int_int_t* self)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    map_int_int_clear_internal (self);
...
void map_int_int_clear_internal_flags
        (map_int_int_t* self, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        map_int_int_hashlist_delete_flags (SLOT(self,i), keys, values);
        E_SET_PTR (SLOT(self,i), NULL);
    }
}
        ...
        (map_int_int_t* self, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    ...
    map_int_int_clear_internal_flags (self, keys, values);
    self->m_nentries= 0;
    SET_ERRNO(E_OK);
}
...
void map_int_int_destroy_internal
        (map_int_int_t* self)
{
    return_if_null2i_or_bad (self);
...
void map_int_int_xchg
        (map_int_int_t *self, map_int_int_t *other)
{
    map_int_int_record_table   (ERWIN_XCHG (map_int_int_content_p,   self->m_table,      other->m_table));
    map_int_int_record_sizes   (ERWIN_XCHG (int,             self->m_hashsize,   other->m_hashsize));
    map_int_int_record_sizes   (ERWIN_XCHG (int,             self->m_nentries,   other->m_nentries));
    map_int_int_record_errno   (ERWIN_XCHG (int,             self->m_errno,      other->m_errno));
    map_int_int_record_zero    (ERWIN_XCHG (int,        self->m_zero,       other->m_zero));
    map_int_int_record_compat  (ERWIN_XCHG (map_iterator_t,  self->compat1_iter, other->compat1_iter));
    map_int_int_record_profile (ERWIN_XCHG (int,             self->m_nrehash,    other->m_nrehash));
    map_int_int_record_profile (ERWIN_XCHG (int,             self->m_nrehashops, other->m_nrehashops));
    map_int_int_record_profile (ERWIN_XCHG (int,             self->m_nfind,      other->m_nfind));
    map_int_int_record_profile (ERWIN_XCHG (int,             self->m_ninsert,    other->m_ninsert));
    map_int_int_record_profile (ERWIN_XCHG (int,             self->m_ndelete,    other->m_ndelete));
}

ERWIN_STATIC_INLINE
CODE_SECTION(".text.destroy_internal_flags")
void map_int_int_destroy_internal_flags
        (
    map_int_int_t* self,
    ...
    ERWIN_BOOL destroy_values)
{
    ...
    map_int_int_clear_internal_flags (self, destroy_keys, destroy_values);
    FREE_TABLE(self);
...
void map_int_int_destroy
        (map_int_int_t* self)
{
    map_int_int_destroy_internal (self);
}

CODE_SECTION(".text.destroy_flags")
void map_int_int_destroy_flags
        (map_int_int_t* self, ERWIN_BOOL k, ERWIN_BOOL v)
{
    map_int_int_destroy_internal_flags (self, k, v);
}
...
void map_int_int_delete_flags
        (
    map_int_int_t* self, ERWIN_BOOL destroy_keys, ERWIN_BOOL destroy_values)
{
    if (self != NULL) {
        map_int_int_destroy_internal_flags (self, destroy_keys, destroy_values);
        ERWIN_THING_DELETE1 (self, map_int_int_t);
    }
}

CODE_SECTION(".text.delete")
void map_int_int_delete
        (map_int_int_t* self)
{
    if (self != NULL) {
        map_int_int_destroy_internal (self);
...
ERWIN_BOOL map_int_int_expect_size
        (map_int_int_t *self, int newsize)
{
    int new_hash_size;
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    if (TOOSMALL(newsize, new_hash_size)) {
        ...
        if (new_hash_size <= 1) {
             ...
             * this too small. :-) */
            if (newsize <= MAP_INT_INT_PLAIN_LIST_LENGTH)
                return ERWIN_FALSE;
        }
#endif
        ...
        map_int_int_rehash (self, new_hash_size);
    ...
    if (TOOLARGE(newsize, new_hash_size)) {
...
#endif
        {
            ...
            if (new_hash_size < MAP_INT_INT_MINIMAL_SIZE_OR_1)
                new_hash_size= MAP_INT_INT_MINIMAL_SIZE_OR_1;
        }
...
ERWIN_BOOL map_int_int_shrink_perhaps
        (map_int_int_t *self)
{
...
#if MAP_INT_INT_ZERO_SIZE
        if (self->m_nentries <= MAP_INT_INT_PLAIN_LIST_LENGTH)
            newsize= 1;
        else
#endif
        {
            ...
            if (newsize < MAP_INT_INT_MINIMAL_SIZE_OR_1)
                newsize= MAP_INT_INT_MINIMAL_SIZE_OR_1;
        }

        map_int_int_rehash (self, newsize);
...
void map_int_int_clear_flags
        (map_int_int_t* self, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    map_int_int_clear_flags_no_resize (self, keys, values);
    map_int_int_shrink_perhaps (self);
}
...
#endif
    map_int_int_clear_no_resize(self);
    map_int_int_shrink_perhaps (self);
    SET_ERRNO(E_OK);
}
...
unsigned map_int_int_slot_no
        (map_int_int_t const *self, int  key)
{
    int slot;
    return0_if_null (self);
...
map_int_int_hashlist_t ** map_int_int_find_internal_ptr
        (map_int_int_t const* self, int  key)
{
    map_int_int_hashlist_t **lauf;
#ifdef E_PROFILE
    ...
    E_SET_PTR(lauf, &SLOT(self, map_int_int_slot_no (self, key)));
    while (*lauf) {
...
map_int_int_hashlist_t * map_int_int_find_internal
        (map_int_int_t const* self, int  key)
{
    map_int_int_hashlist_t **result;

    E_SET_PTR(result, map_int_int_find_internal_ptr (self, key));
#if MAP_INT_INT_ALLOW_NULL
    if (result == NULL)
        return NULL;
#endif
...
map_int_int_element_ptr_t map_int_int_find_ptr
        (map_int_int_t const* self, int  key)
{
    map_int_int_hashlist_t *lauf;

    E_SET_PTR(lauf, map_int_int_find_internal (self, key));
    if (lauf == NULL) {
 ...
 *        map_int_int_ensure which currently evaluates ENSURE_VALUE to often).
 *
...
int map_int_int_poke_internal_q
        (
        int                *key_out,
        int             *value_out,
        map_int_int_t                *self,
        ...
        ERWIN_BOOL     overwrite)
{
    ...
    E_SET_PTR(lauf, map_int_int_find_internal_ptr (self, key));
#if MAP_INT_INT_ALLOW_NULL
...
#endif
    {
        ...
        if (introduce) {
            ...
            if (map_int_int_grow_perhaps (self)) {
                E_SET_PTR(lauf, map_int_int_find_internal_ptr (self, key));
            }
            ...
            if (dovaluecopy) {
                ...
                if (dokeycopy) {
                    ...
                    E_SET_PTR(*lauf, map_int_int_hashlist_new (keycopy, valuecopy));
                    ...
                    E_SET_PTR(*lauf, map_int_int_hashlist_new (givenkeycopy, valuecopy));
            ...
            else
            {
                if (dokeycopy) {
                    ...
                    E_SET_PTR(*lauf, map_int_int_hashlist_new_q (keycopy, givenvaluecopy));
                    ...
                    E_SET_PTR(*lauf, map_int_int_hashlist_new_q (givenkeycopy, givenvaluecopy));
    ...
    ERWIN_BOOL i, ERWIN_BOOL o)
{
    ...
    return map_int_int_poke_internal_q (ko,vo,self, k,IZEROVAR,ERWIN_TRUE, v,ZEROVAR(self),ERWIN_TRUE, i,o);
}

CODE_SECTION(".text.poke_no_icopy_q")
int map_int_int_poke_no_icopy_q
        (
    int *ko, int *vo, map_int_int_t *self, int  k, int  v, ERWIN_BOOL i, ERWIN_BOOL o)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return map_int_int_poke_internal_q (ko,vo,self, k,k,ERWIN_FALSE, v,ZEROVAR(self),ERWIN_TRUE, i,o);
}
...
int map_int_int_poke_no_ocopy
    ...
    int *ko, int *vo, map_int_int_t *self, int  k, int  v, ERWIN_BOOL i, ERWIN_BOOL o)
{
    return map_int_int_poke_internal (ko,vo,self, k,IZEROVAR,ERWIN_TRUE, v,v,ERWIN_FALSE, i,o);
}
...
int map_int_int_poke_no_icopy_no_ocopy
    ...
    int *ko, int *vo, map_int_int_t *self, int  k, int  v, ERWIN_BOOL i, ERWIN_BOOL o)
{
    return map_int_int_poke_internal (ko, vo, self, k,k,ERWIN_FALSE, v,v,ERWIN_FALSE, i,o);
}
...
o_type_result_t map_int_int_find
        (map_int_int_t const* self, int  key)
{
    map_int_int_hashlist_t *lauf;

#if defined(MAP_INT_INT_ALLOW_NULL) && MAP_INT_INT_CONSTANT_ZERO
    if (self == NULL) {
        SET_ERRNO(E_KEYNOTFOUND);
        return OZERO;
    }
...
o_type_result_t map_int_int_find_ensure
        (map_int_int_t * self, int  key)
{
    map_int_int_element_ptr_t result;
    E_SET_PTR(result, map_int_int_find_ptr_ensure (self, key));
#if !MAP_INT_INT_NOMEM_IS_FATAL
...
i_type_result_t map_int_int_find_key
        (map_int_int_t const* self, int  key)
{
    map_int_int_hashlist_t *lauf;
#if defined(MAP_INT_INT_ALLOW_NULL)
    if (self == NULL) {
        SET_ERRNO(E_KEYNOTFOUND);
        return IZERO;
    }
...
o_type_var_t map_int_int_modify
        (map_int_int_t *self, int  key, int  value)
{
...
{
    return0_if_null (self);
    return map_int_int_poke_internal (
            NULL,NULL,self,
...
map_int_int_element_ptr_t map_int_int_find_ptr_ensure
        (map_int_int_t *self, int  key)
/* NOTE: This cannot be implemented with _poke_internal, since it
 *       returns a pointer to an internal structure. */
/* FIXME: extract common parts from map_int_int_ensure and map_int_int_find_ptr_ensure,
 *        which do essentially the same. */
{
    ...
    SET_ERRNO(E_OK);
    if (*lauf == NULL) {
        if (ERWIN_UNLIKELY(map_int_int_grow_perhaps (self))) {
            E_SET_PTR(lauf, map_int_int_find_internal_ptr (self, key));
            return0_if_null_pp (lauf, self);
...

        {
            ...
            {
                int valuecopy;
                E_SET_VALUE (valuecopy, MAP_INT_INT_ENSURE_VALUE(self, key, &copyerror));
                if (ERWIN_UNLIKELY(OCOPY_PROVIDED && copyerror)) {
                    IFREE (keycopy);
                    MAP_NOMEM;
                    return NULL;
...
int map_int_int_insert_no_copy_no_rehash_q
        (
        map_int_int_t* self,
        int keycopy,
        int valuecopy)
 ...
 *       delete keycopy or valuecopy! */
{
    ...
    E_SET_PTR(lauf, map_int_int_find_internal_ptr (self, keycopy));
    return_val_if_null_pp (lauf, ASSERTION_FAILED, self);

    if (*lauf != NULL)
        return PASS_ERRNO(E_EXISTINGKEY);

    E_SET_PTR(*lauf, map_int_int_hashlist_new_q (keycopy, valuecopy));
...
ATTR_FLATTEN
int map_int_int_insert_q
        (map_int_int_t* self, int  key, int  value)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return map_int_int_poke_internal_q (
                NULL, NULL, self,
                key,   IZEROVAR, ERWIN_TRUE,
                value, ZEROVAR(self), ERWIN_TRUE,
...
int map_int_int_insert_map
        (map_int_int_t* self, map_int_int_t const *other)
{
    int i;
    int total_errno;
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       while (lauf) {
           map_int_int_insert_q (self, lauf->key, lauf->value);
           if (e_errno != E_OK)
        ...
        (map_int_int_t* self, map_int_int_t const *other)
{
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           map_int_int_set (self, lauf->key, lauf->value);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
        ...
        (map_int_int_t* self, map_int_int_t const *other)
{
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           map_int_int_modify (self, lauf->key, lauf->value);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
        ...
        (map_int_int_t* self, map_int_int_t const *other)
{
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           map_int_int_erase (self, lauf->key);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
...
int map_int_int_erase_map_no_resize
        (map_int_int_t* self, map_int_int_t const *other)
{
    int i;
    int total_errno;
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           map_int_int_erase_no_resize (self, lauf->key);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
...
int map_int_int_intersect_no_resize
        (map_int_int_t* self, map_int_int_t const *other)
{
    int i;
    int cnt;
    ...
    for (i= 0; i < self->m_hashsize; i++) {
        ...
        E_SET_PTR (lauf, &SLOT(self,i));
        while (*lauf) {
            (void)map_int_int_find (other, (*lauf)->key);
            if (e_errno != E_OK) {
                 map_int_int_hashlist_t *cell;
                 E_SET_PTR (cell,  *lauf);
                 E_SET_PTR (*lauf, (*lauf)->next);

                 IFREE (cell->key);
                 OFREE (cell->value);
                 map_int_int_hashlist_delete_cell (cell);
                 self->m_nentries--;
                 cnt++;
            }
            else
...
int map_int_int_intersect
        (map_int_int_t* self, map_int_int_t const *other)
{
    int cnt= map_int_int_intersect_no_resize (self, other);
    map_int_int_shrink_perhaps (self);
    return cnt;
}
...
i_type_result_t map_int_int_ensure
        (map_int_int_t *self, int  key)
{
    map_int_int_hashlist_t **lauf;
...
i_type_result_t map_int_int_ensure_no_icopy
        (map_int_int_t *self, int  key)
{
    map_int_int_hashlist_t **lauf;
    ...
    if (*lauf == NULL) {
...

        {
            ...
            E_SET_PTR (*lauf, map_int_int_hashlist_new_q (key, valuecopy));
            if (ERWIN_UNLIKELY(*lauf == NULL)) {
                OFREE (valuecopy);
                MAP_NOMEM;
                return IZERO;
...
ERWIN_BOOL map_int_int_remove_internal_no_resize_q
        (
    map_int_int_t *self,
    int  key,
    int *result)
...
ERWIN_BOOL map_int_int_remove_internal_q
        (
    map_int_int_t *self,
    int  key,
    int *result)
{
    ...
    return map_int_int_remove_internal_no_resize_q (self, key, result);
}

CODE_SECTION(".text.remove")
o_type_var_t map_int_int_remove
        (map_int_int_t *self, int  key)
{
    int result;
#if defined(MAP_INT_INT_ALLOW_NULL) && MAP_INT_INT_CONSTANT_ZERO
...
#endif
    if (map_int_int_remove_internal_q (self, key, &result))
        return result;
    return ZEROVAR(self);
}
...
o_type_var_t map_int_int_remove_no_resize
        (map_int_int_t *self, int  key)
{
    int result;
#if defined(MAP_INT_INT_ALLOW_NULL) && MAP_INT_INT_CONSTANT_ZERO
...
#endif
    if (map_int_int_remove_internal_no_resize_q (self, key, &result))
        return result;
    return ZEROVAR(self);
}
        ...
        (map_int_int_t* self, map_int_int_t const *other)
{
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           map_int_int_remove (self, lauf->key);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
...
int map_int_int_remove_if
        (map_int_int_t *self, map_int_int_feature_t feature, ERWIN_BOOL value)
{
    int slot;
    int cnt= 0;
...
int map_int_int_erase_no_resize
        (map_int_int_t *self, int  key)
{
    int result;
#if defined(MAP_INT_INT_ALLOW_NULL) && MAP_INT_INT_CONSTANT_ZERO
        ...
        (map_int_int_t* self, int newsize)
{
    ...
    if (newsize != self->m_hashsize) {
        int i;
        map_int_int_content_p newtable;
        ...
        if (newsize == 1) {
            ...
            E_SET_PTR (newtable, ERWIN_THING_CNEW_ARRAY (map_int_int_content, (unsigned)newsize));
        ...
        for (i=0; i < self->m_hashsize; i++) {
            ...
            while (lauf != NULL) {
                map_int_int_hashlist_t *next;
                map_int_int_hashlist_t **newcell= &GET_SLOT(
                                      newtable,
                                      newsize,
...
map_int_int_hashlist_t * map_int_int_first_internal
        (map_int_int_t const *self, e_iterator_t *iter)
{
...
map_int_int_hashlist_t * map_int_int_next_internal
        (map_int_int_t const*self, e_iterator_t *iter)
{
    if (ERWIN_UNLIKELY(ITER->cslot == -1))
        return map_int_int_first_internal (self, iter);
...
void map_int_int_init_iterator
        (map_int_int_t const *self, e_iterator_t *iter)
{
    (void)self; /*unused*/

#ifndef MAP_INT_INT_ALLOW_NULL
    return_if_null_or_bad (self);
#endif
        ...
        (void const *a, void const *b)
{
    ...
    (void)b; /*unused*/
#ifdef ICMP
#define map_int_int_check_cmp_keys
#define map_int_int_check0_cmp_keys
    return ICMP ((((map_int_int_pair_t*)a)->key),(((map_int_int_pair_t*)b)->key));
#else
#define map_int_int_check_cmp_keys \
...
ATTR_FUNC_UNUSED
int map_int_int_pair_cmp_keys_ptr
        (void const *a, void const *b)
{
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    (void)a; /*unused*/
    (void)b; /*unused*/
#ifdef ICMP
#define map_int_int_check_cmp_keys_ptr
#define map_int_int_check0_cmp_keys_ptr
    return ICMP ((((map_int_int_pair_ptr_t*)a)->key),(((map_int_int_pair_ptr_t*)b)->key));
#else
#define map_int_int_check_cmp_keys_ptr \
    return_if_reached_pt ("ICMP is not implemented.  Sorting keys is not possible.")
#define map_int_int_check0_cmp_keys_ptr \
...
ATTR_FUNC_UNUSED
int map_int_int_pair_cmp_values
        (void const *a, void const *b)
{
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    (void)a; /*unused*/
    (void)b; /*unused*/
#ifdef OCMP
#define map_int_int_check_cmp_values
#define map_int_int_check0_cmp_values
    return OCMP ((((map_int_int_pair_t*)a)->value),(((map_int_int_pair_t*)b)->value));
#else
#define map_int_int_check_cmp_values \
...
ATTR_FUNC_UNUSED
int map_int_int_pair_cmp_values_ptr
        (void const *a, void const *b)
{
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    (void)a; /*unused*/
    (void)b; /*unused*/
#ifdef OCMP
#define map_int_int_check_cmp_values_ptr
#define map_int_int_check0_cmp_values_ptr
    return OCMP ((*(((map_int_int_pair_ptr_t*)a)->value)),(*(((map_int_int_pair_ptr_t*)b)->value)));
#else
#define map_int_int_check_cmp_values_ptr \
    return_if_reached_pt ("OCMP is not implemented.  Sorting values is not possible.")
#define map_int_int_check0_cmp_values_ptr \
...
ATTR_FUNC_UNUSED
int map_int_int_pair_cmp_values_and_keys
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#define map_int_int_check_cmp_values_and_keys
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= OCMP ((((map_int_int_pair_t*)a)->value),(((map_int_int_pair_t*)b)->value));
    if (result == 0)
        result= ICMP ((((map_int_int_pair_t*)a)->key),(((map_int_int_pair_t*)b)->key));
#else
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
#define map_int_int_check_cmp_values_and_keys \
...
ATTR_FUNC_UNUSED
int map_int_int_pair_cmp_values_and_keys_ptr
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#if defined(OCMP) && defined(ICMP)
#define map_int_int_check_cmp_values_and_keys_ptr
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= OCMP ((*(((map_int_int_pair_ptr_t*)a)->value)),(*(((map_int_int_pair_ptr_t*)b)->value)));
    if (result == 0)
        result= ICMP ((((map_int_int_pair_ptr_t*)a)->key),(((map_int_int_pair_ptr_t*)b)->key));
#else
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
#define map_int_int_check_cmp_values_and_keys_ptr \
    return_if_reached_pt ("OCMP or ICMP is not implemented.  Sorting values is not possible.");
...
ATTR_FUNC_UNUSED
int map_int_int_pair_cmp_keys_and_values
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#if defined(OCMP) && defined(ICMP)
#define map_int_int_check_cmp_keys_and_values
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= ICMP ((((map_int_int_pair_t*)a)->key),(((map_int_int_pair_t*)b)->key));
    if (result == 0)
        result= OCMP ((((map_int_int_pair_t*)a)->value),(((map_int_int_pair_t*)b)->value));
...
ATTR_FUNC_UNUSED
int map_int_int_pair_cmp_keys_and_values_ptr
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#if defined(OCMP) && defined(ICMP)
#define map_int_int_check_cmp_keys_and_values_ptr
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= ICMP ((((map_int_int_pair_ptr_t*)a)->key),(((map_int_int_pair_ptr_t*)b)->key));
    if (result == 0)
        result= OCMP ((*(((map_int_int_pair_ptr_t*)a)->value)),(*(((map_int_int_pair_ptr_t*)b)->value)));
...
ATTR_FUNC_UNUSED
int map_int_int_pair_detcmp_keys
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#  define map_int_int_detcmp_pairs     map_int_int_pair_cmp_keys
#  define map_int_int_detcmp_pairs_ptr map_int_int_pair_cmp_keys_ptr
#else
#  define MAP_INT_INT_DET_SORT_PROVIDED 1
#endif

#ifndef MAP_INT_INT_DET_SORT_PROVIDED
#  define MAP_INT_INT_DET_SORT_PROVIDED 0
#endif

#if ODETCMP_PROVIDED || MAP_INT_INT_DET_SORT_PROVIDED
#  define MAP_INT_INT_ODET_REQUIRE 1
#endif

#ifndef MAP_INT_INT_ODET_REQUIRE
#  define MAP_INT_INT_ODET_REQUIRE 0
#endif
...
ATTR_FUNC_UNUSED
int map_int_int_pair_detcmp_values
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#if MAP_INT_INT_DET_SORT_PROVIDED
    ...
    result= OCMP (*((int const*)a), *((int const*)b));
#else
#define map_int_int_check_detcmp_values \
    return0_if_reached_pt ("Need INT_DETCMP or INT_CMP to enforce determinism on map_int_int.  Try to '#define INT_DETCMP(x,y) false_if_reached_pk(INT_DETCMP)' if you think you never invoke it.");
    result=0;
#endif

#ifndef map_int_int_check_detcmp_values
#define map_int_int_check_detcmp_values
#endif /* !defined map_int_int_check_detcmp_values */
        ...
        (void const *a, void const *b)
{
    ...
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= IDETCMP ((((map_int_int_pair_t*)a)->key),(((map_int_int_pair_t*)b)->key));

    if (result == 0)
        result= ODETCMP ((((map_int_int_pair_t*)a)->value),(((map_int_int_pair_t*)b)->value));
...
ATTR_FUNC_UNUSED
int map_int_int_detcmp_pairs_ptr
        (void const *a, void const *b)
{
    /* This uses paranoid paranoid, perhaps duplicate comparisons.  But the user
     * wants determinism and we try to enforce that in any way.  It may be slower,
    ...
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= IDETCMP ((((map_int_int_pair_ptr_t*)a)->key),(((map_int_int_pair_ptr_t*)b)->key));

    if (result == 0)
        result= ODETCMP ((*(((map_int_int_pair_ptr_t*)a)->value)),(*(((map_int_int_pair_ptr_t*)b)->value)));
...
#endif /* MAP_INT_INT_DET_SORT_PROVIDED */
...
void map_int_int_det_sort
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    qsort ((void*)SORTITER->table,
           self->m_nentries,
           sizeof (map_int_int_pair_t),
           map_int_int_detcmp_pairs);
}


ERWIN_STATIC_INLINE_SPEED
CODE_SECTION(".text.det_sort_ptr")
void map_int_int_det_sort_ptr
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    qsort ((void*)SORTITERPTR->table,
           self->m_nentries,
           sizeof (map_int_int_pair_ptr_t),
           map_int_int_detcmp_pairs_ptr);
}
...
void map_int_int_free_sorted_iterator
        (void *iter)
{
    map_int_int_delete_entries (SORTITER->table);
}


static
CODE_SECTION(".text.free_sorted_iterator_ptr")
void map_int_int_free_sorted_iterator_ptr
        (void *iter)
{
    map_int_int_delete_entries_ptr (SORTITERPTR->table);
}


CODE_SECTION(".text.init_iterator_sorted_by_key")
void map_int_int_init_iterator_sorted_by_key
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITER->table, map_int_int_get_entries (self));
        SORTITER->count= 0;
        SORTITER->lauf=  0;
        E_SET_PTR (SORTITER->free_cb, map_int_int_free_sorted_iterator);
        return;
    }
#else
    return_if_null_or_bad (self);
    ...
    return_if_fail_pp ((void*)SORTITER_VOLATILE->free_cb == (void*)map_int_int_free_sorted_iterator, self);
    ...
    if (e_require_determinism ()) {
        map_int_int_det_sort (self, iter);
        e_merge_sort ((void*)SORTITER->table,
               self->m_nentries,
               sizeof (map_int_int_pair_t),
               map_int_int_pair_cmp_keys);
    }
    else
#endif
    {
...
void map_int_int_init_iterator_sorted_by_value
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_pp2 (SORTITER_VOLATILE->p,
        SORTITER, self);
    return_if_fail_pp2 ((void*)SORTITER_VOLATILE->free_cb == (void*)map_int_int_free_sorted_iterator,
        SORTITER, self);
...
void map_int_int_init_iterator_sorted_by_value_and_key
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    map_int_int_check_cmp_values_and_keys;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITER->table,
               self->m_nentries,
               sizeof (map_int_int_pair_t),
               map_int_int_pair_cmp_values_and_keys);
    }
    else
#endif
    {
...
void map_int_int_init_iterator_sorted_by_key_and_value
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    map_int_int_check_cmp_keys_and_values;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITER->table,
               self->m_nentries,
               sizeof (map_int_int_pair_t),
               map_int_int_pair_cmp_keys_and_values);
    }
    else
#endif
    {
...
void map_int_int_init_iterator_sorted_by_user
        (
    map_int_int_t const *self,
    e_iterator_sorted_t *iter,
    map_int_int_pair_cmp_t u)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITER->table, map_int_int_get_entries (self));
...
void map_int_int_init_iterator_ptr_sorted_by_key
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITERPTR->table, map_int_int_get_entries_ptr (self));
        SORTITERPTR->count= 0;
        SORTITERPTR->lauf=  0;
        E_SET_PTR (SORTITERPTR->free_cb, map_int_int_free_sorted_iterator_ptr);
        return;
    }
#endif
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    map_int_int_check_cmp_keys_ptr;
    ...
    return_if_null_pp2 (SORTITER_VOLATILE->p,
        SORTITER, self);
    return_if_fail_pp2 ((void*)SORTITER_VOLATILE->free_cb == (void*)map_int_int_free_sorted_iterator_ptr,
        SORTITER, self);
     ...
     * we still need to pre-sort and use (the stable) merge sort.  */
    if (e_require_determinism ()) {
        map_int_int_det_sort_ptr (self, iter);
        e_merge_sort ((void*)SORTITERPTR->table,
               self->m_nentries,
               sizeof (map_int_int_pair_ptr_t),
               map_int_int_pair_cmp_keys_ptr);
    }
    else
#endif
    {
...
void map_int_int_init_iterator_ptr_sorted_by_value
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    map_int_int_check_cmp_values_ptr;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITERPTR->table,
               self->m_nentries,
               sizeof (map_int_int_pair_ptr_t),
               map_int_int_pair_cmp_values_ptr);
    }
    else
#endif
    {
...
void map_int_int_init_iterator_ptr_sorted_by_value_and_key
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    map_int_int_check_cmp_values_and_keys_ptr;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITERPTR->table,
               self->m_nentries,
               sizeof (map_int_int_pair_ptr_t),
               map_int_int_pair_cmp_values_and_keys_ptr);
    }
    else
#endif
    {
...
void map_int_int_init_iterator_ptr_sorted_by_key_and_value
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    map_int_int_check_cmp_keys_and_values_ptr;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITERPTR->table,
               self->m_nentries,
               sizeof (map_int_int_pair_ptr_t),
               map_int_int_pair_cmp_keys_and_values_ptr);
    }
    else
#endif
    {
...
void map_int_int_init_iterator_ptr_sorted_by_user
        (
    map_int_int_t const *self,
    e_iterator_sorted_t *iter,
    map_int_int_pair_ptr_cmp_t u)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITERPTR->table, map_int_int_get_entries_ptr (self));
...
void map_int_int_init_iterator_sorted_by_key_reverse
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_sorted_by_key (self, iter);
    SORTITER->lauf= SORTITER->count;
}


CODE_SECTION(".text.init_iterator_sorted_by_value_reverse")
void map_int_int_init_iterator_sorted_by_value_reverse
        (map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_sorted_by_value (self, iter);
    SORTITER->lauf= SORTITER->count;
}
...
void map_int_int_init_iterator_sorted_by_value_and_key_reverse
    ...
    map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_sorted_by_value_and_key (self, iter);
    SORTITER->lauf= SORTITER->count;
}
...
void map_int_int_init_iterator_sorted_by_key_and_value_reverse
    ...
    map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_sorted_by_key_and_value (self, iter);
    SORTITER->lauf= SORTITER->count;
}
...
void map_int_int_init_iterator_sorted_by_user_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, map_int_int_pair_cmp_t u)
{
    map_int_int_init_iterator_sorted_by_user (self, iter, u);
    SORTITER->lauf= SORTITER->count;
}


CODE_SECTION(".text.init_iterator_ptr_sorted_by_key_reverse")
void map_int_int_init_iterator_ptr_sorted_by_key_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_ptr_sorted_by_key (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}


CODE_SECTION(".text.init_iterator_ptr_sorted_by_value_reverse")
void map_int_int_init_iterator_ptr_sorted_by_value_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_ptr_sorted_by_value (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}
...
void map_int_int_init_iterator_ptr_sorted_by_value_and_key_reverse
    ...
    map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_ptr_sorted_by_value_and_key (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}
...
void map_int_int_init_iterator_ptr_sorted_by_key_and_value_reverse
    ...
    map_int_int_t const *self, e_iterator_sorted_t *iter)
{
    map_int_int_init_iterator_ptr_sorted_by_key_and_value (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}
...
void map_int_int_init_iterator_ptr_sorted_by_user_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, map_int_int_pair_ptr_cmp_t u)
{
    map_int_int_init_iterator_ptr_sorted_by_user (self, iter, u);
    SORTITERPTR->lauf= SORTITERPTR->count;
}


CODE_SECTION(".text.next_iteration_sorted")
ERWIN_BOOL map_int_int_next_iteration_sorted
        (map_int_int_t const *self,
                                e_iterator_sorted_t *iter,
                                int *keyp,
                                int *valuep)
...
ERWIN_BOOL map_int_int_next_iteration_sorted_ptr
        (map_int_int_t const *self,
                                e_iterator_sorted_t *iter,
                                int *keyp,
                                map_int_int_element_ptr_t *valuepp)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_keys
        (map_int_int_t const *self,
                                e_iterator_sorted_t *iter,
                                int *keyp)
{
...
ERWIN_BOOL map_int_int_next_iteration_sorted_values
        (map_int_int_t const *self,
                                e_iterator_sorted_t *iter,
                                int *valuep)
{
...
ERWIN_BOOL map_int_int_next_iteration_sorted_values_ptr
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, map_int_int_element_ptr_t *valuepp)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_pairs
        (map_int_int_t const *self,
                                e_iterator_sorted_t *iter,
                                map_int_int_pair_t *p)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_pairs_ptr
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, map_int_int_pair_ptr_t *p)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, int *keyp, int *valuep)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_ptr_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, int *keyp, map_int_int_element_ptr_t *valuepp)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_keys_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, int *keyp)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_values_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, int *valuep)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_values_ptr_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, map_int_int_element_ptr_t *valuepp)
{
#ifdef MAP_INT_INT_ALLOW_NULL
...
ERWIN_BOOL map_int_int_next_iteration_sorted_pairs_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, map_int_int_pair_t *p)
{
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL map_int_int_next_iteration_sorted_pairs_ptr_reverse
        (
    map_int_int_t const *self, e_iterator_sorted_t *iter, map_int_int_pair_ptr_t *p)
{
#ifdef MAP_INT_INT_ALLOW_NULL
...
ERWIN_BOOL map_int_int_next_iteration
        (
    map_int_int_t const*self,
    e_iterator_t *iter,
    int *keyp,
    int *valuep)
{
    ...
    E_SET_PTR (it, map_int_int_next_internal (self, iter));
...
ERWIN_BOOL map_int_int_next_iteration_values
        (
    map_int_int_t const *self,
    e_iterator_t *iter,
    int *valuep)
...
ERWIN_BOOL map_int_int_next_iteration_keys
        (map_int_int_t const *self, e_iterator_t *iter, int *keyp)
{
    map_int_int_hashlist_t *it;
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
...
ERWIN_BOOL map_int_int_next_iteration_pairs
        (map_int_int_t const *self, e_iterator_t *iter, map_int_int_pair_t *pairp)
{
    map_int_int_hashlist_t *it;
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
...
ERWIN_BOOL map_int_int_next_iteration_ptr
        (
    map_int_int_t const*self,
    e_iterator_t *iter,
    int *keyp,
...
ERWIN_BOOL map_int_int_next_iteration_values_ptr
        (
    map_int_int_t const *self,
    e_iterator_t *iter,
    map_int_int_element_ptr_t *valuepp)
...
ERWIN_BOOL map_int_int_next_iteration_pairs_ptr
        (map_int_int_t const *self, e_iterator_t *iter, map_int_int_pair_ptr_t *pairp)
{
    map_int_int_hashlist_t *it;
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL)
...
map_int_int_pair_t * map_int_int_get_entries
        (map_int_int_t const *self)
{
    struct _map_int_int_pair_t *result, *rlauf;
    int    i;

#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, PAIR_NEW_ARRAY (map_int_int_pair_dynarray, 1));
    ...
    E_SET_PTR (result, PAIR_NEW_ARRAY (map_int_int_pair_dynarray, self->m_nentries+1));
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (map_int_int_pair_t),
               (int(*)(void const *, void const *))map_int_int_detcmp_pairs);
#endif
...
map_int_int_pair_ptr_t * map_int_int_get_entries_ptr
        (map_int_int_t const*self)
{
    struct _map_int_int_pair_ptr_t *result, *rlauf;
    int    i;

#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, ERWIN_THING_NEW_ARRAY (map_int_int_pair_ptr_dynarray, 1));
        if (result == NULL) {
            MAP_NOMEM;
            return NULL;
        }
    ...
    E_SET_PTR (result, ERWIN_THING_NEW_ARRAY (map_int_int_pair_ptr_dynarray, self->m_nentries+1));
    E_SET_PTR (rlauf,  result);
    if (ERWIN_UNLIKELY(result == NULL)) {
        MAP_NOMEM;
        return NULL;
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (map_int_int_pair_ptr_t),
               (int(*)(void const *, void const *))map_int_int_detcmp_pairs_ptr);
#endif
...
o_type_t * map_int_int_get_values
        (map_int_int_t const *self)
{
    int *result;
    int *rlauf;
...
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, VALUE_NEW_ARRAY (map_int_int_value_dynarray, 1));
        if (ERWIN_UNLIKELY(result == NULL)) {
            MAP_NOMEM;
            return NULL;
        }
    ...
    E_SET_PTR (result, VALUE_NEW_ARRAY (map_int_int_value_dynarray, self->m_nentries+1));
    E_SET_PTR (rlauf,  result);
    if (ERWIN_UNLIKELY(result == NULL)) {
        MAP_NOMEM;
        return NULL;
...
#if defined(E_REQUIRE_DETERMINISM) && !defined(E_WEAK_DETERMINISM)
    if (e_require_determinism ()) {
        map_int_int_check_detcmp_values;
        qsort (result,
               self->m_nentries,
               sizeof (int),
               (int(*)(void const *, void const *))map_int_int_pair_detcmp_values);
    }
#endif
        ...
        (map_int_int_t const *self)
{
...
#ifdef MAP_INT_INT_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, KEY_NEW_ARRAY (map_int_int_key_dynarray, 1));
        if (ERWIN_UNLIKELY(result == NULL)) {
            MAP_NOMEM;
            return NULL;
        }
    ...
    E_SET_PTR (result, KEY_NEW_ARRAY (map_int_int_key_dynarray, self->m_nentries+1));
    E_SET_PTR (rlauf,  result);
    if (ERWIN_UNLIKELY(result == NULL)) {
        MAP_NOMEM;
        return NULL;
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (int),
               (int(*)(void const *, void const *))map_int_int_pair_detcmp_keys);
#endif
...
void map_int_int_delete_entries
        (map_int_int_pair_t *entries)
{
    PAIR_DELETE_ARRAY (entries);
}

CODE_SECTION(".text.delete_entries_ptr")
void map_int_int_delete_entries_ptr
        (map_int_int_pair_ptr_t *entries_ptr)
{
...
void map_int_int_delete_values
        (int *values)
{
...
ERWIN_BOOL map_int_int_equal
        (map_int_int_t const *self, map_int_int_t const *other)
{
    ...
    map_int_int_forall_nondet (self, iter, key, value)
    {
        int *value_ptr;
        E_SET_PTR (value_ptr, map_int_int_find_ptr (other, key));
        if (value_ptr == NULL)
            return ERWIN_FALSE;
    ...
    error_return0_if_reached_ppt (
        self,
        "Because neither INT_EQUAL nor INT_CMP was #defined, map_int_int_equal could not be implemented.");
#endif
}


CODE_SECTION(".text.find_any_pair_q")
int map_int_int_find_any_pair_q
        (map_int_int_key_result_t *key_p, map_int_int_element_ptr_t *value_p, map_int_int_t const* self)
{
    int i;
    ...
    { /* Find smallest entry. */
        ...
        for (i= 0; i < self->m_hashsize; i++) {
           ...
           E_SET_PTR (lauf, SLOT(self,i));
           while (lauf) {
               if (first || map_int_int_pair_detcmp_keys (&lauf->key, &last) < 0) {
                   first= 0;
...
i_type_result_t map_int_int_find_any_key
        (map_int_int_t const *self)
{
    int result;
    int n= map_int_int_find_any_pair_q (&result, NULL, self);
...
map_int_int_element_ptr_t map_int_int_find_any_ptr
        (map_int_int_t const *self)
{
    map_int_int_element_ptr_t result;
    int n= map_int_int_find_any_pair (NULL, &result, self);
    if (ERWIN_UNLIKELY(n == 0))
        return NULL;
    return result;
}
...
o_type_result_t map_int_int_find_any
        (map_int_int_t const *self)
{
    map_int_int_element_ptr_t result_p;
    int n= map_int_int_find_any_pair (NULL, &result_p, self);
    if (ERWIN_UNLIKELY(n == 0))
        return ZERO(self);
    return *result_p;
}
...
ERWIN_BOOL map_int_int_equal_keys
        (map_int_int_t const *self, map_int_int_t const *other)
{
    e_iterator_t iter;
    int key;
    ...
    map_int_int_forall_keys_nondet (self, iter, key) {
        (void)map_int_int_find_ptr (other, key);
        if (e_errno != E_OK)
            return ERWIN_FALSE;
    }
...
/* must be NULL safe per definition! */
{
    ...
    map_int_int_pair_t *them1;
    map_int_int_pair_t *them2;
    int i;
    int result= 0;
    int cnt;
    map_int_int_check0_cmp_keys;
    map_int_int_check0_cmp_values;
#endif
    ...
    E_SET_PTR (them1, map_int_int_get_entries (self));
    if (ERWIN_UNLIKELY(e_errno != E_OK))
        return 0;

    E_SET_PTR (them2, map_int_int_get_entries (other));
    if (ERWIN_UNLIKELY(e_errno != E_OK)) {
        map_int_int_delete_entries (them1);
        return 0;
    }

#if defined(E_REQUIRE_DETERMINISM) && !defined(E_WEAK_DETERMINISM)
    if (e_require_determinism()) {
        qsort ((void*)them1, cnt, sizeof (map_int_int_pair_t), map_int_int_pair_detcmp_keys);
        qsort ((void*)them2, cnt, sizeof (map_int_int_pair_t), map_int_int_pair_detcmp_keys);

        e_merge_sort ((void*)them1, cnt, sizeof (map_int_int_pair_t), map_int_int_pair_cmp_keys);
        e_merge_sort ((void*)them2, cnt, sizeof (map_int_int_pair_t), map_int_int_pair_cmp_keys);
    }
    else
#else
    {
        qsort ((void*)them1, cnt, sizeof (map_int_int_pair_t), map_int_int_pair_cmp_keys);
        qsort ((void*)them2, cnt, sizeof (map_int_int_pair_t), map_int_int_pair_cmp_keys);
    }
#endif

    for (i=0; i < cnt; i++) {
        result= map_int_int_pair_cmp_keys (them1 + i, them2 + i);
        if (result != 0)
            break;

        result= map_int_int_pair_cmp_values (them1 + i, them2 + i);
        if (result != 0)
            break;
    }

    map_int_int_delete_entries (them1);
    map_int_int_delete_entries (them2);
    ...
    error_return0_if_reached_ppt (
        self,
        "Because either INT_CMP or INT_CMP was not #defined, map_int_int_cmp could not be implemented.");
#endif
}
...
/* must be NULL safe per definition! */
{
    ...
    E_SET_PTR (them1, map_int_int_get_keys (self));
    if (ERWIN_UNLIKELY(e_errno != E_OK))
        return 0;

    E_SET_PTR (them2, map_int_int_get_keys (other));
    if (ERWIN_UNLIKELY(e_errno != E_OK)) {
        map_int_int_delete_keys (them1);
        return 0;
    }

    qsort ((void*)them1, cnt, sizeof (int), map_int_int_wrap_icmp);
    qsort ((void*)them2, cnt, sizeof (int), map_int_int_wrap_icmp);
    ...
    error_return0_if_reached_ppt (
        self,
        "Because either INT_CMP was not #defined, map_int_int_cmp_keys could not be implemented.");
#endif
}
...
e_hashval_t map_int_int_hash_raw
        (map_int_int_t const *self)
/* must be NULL-safe */
{
#ifdef OHASH
    ...
    error_return0_if_reached_ppt (
        self,
        "Because INT_HASH was not #defined, map_int_int_hash_raw could not be implemented.");
#endif
}
...
CODE_SECTION(".text.t")
 map_int_int_t::map_int_int_t
        (void)
{
    ERWIN_BOOL ok= map_int_int_init_internal_q (it(), OZERO, MAP_INT_INT_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}

CODE_SECTION(".text._constructor")
void map_int_int_t::_constructor
        (void)
{
    ERWIN_BOOL ok= map_int_int_init_internal_q (it(), OZERO, MAP_INT_INT_INITIAL_SIZE);
    (void)ok; /*unused*/
...
map_int_int_t const & map_int_int_t::static_zero
        ()
{
    static map_int_int_t const *z= NULL;
    if (ERWIN_UNLIKELY(!z))
        E_SET_PTR (z, new map_int_int_t);
    return *z;
}
...
CODE_SECTION(".text.t")
 map_int_int_t::map_int_int_t
        (int size)
{
    ERWIN_BOOL ok= map_int_int_init_internal_q (it(), OZERO, size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* MAP_INT_INT_HAVE_INT_CONSTRUCTOR */
...
CODE_SECTION(".text.t")
 map_int_int_t::map_int_int_t
        (int  zeroval)
{
    ERWIN_BOOL ok= map_int_int_init_internal_q (it(), zeroval, MAP_INT_INT_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
...
{
    ERWIN_BOOL ok= map_int_int_init_internal_q (it(), zeroval, size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* MAP_INT_INT_DYN_ZERO */
...
CODE_SECTION(".text.t")
 map_int_int_t::map_int_int_t
        (map_int_int_t const &other)
{
#ifdef ERWIN_DOS
    if (other.it()->m_table == NULL && other.it()->m_nentries == 0) {
        /* See copy constructor for vectors for explanation. */
        map_int_int_init_internal_q (it(), OZERO, MAP_INT_INT_INITIAL_SIZE);
    }
    else
#endif
    {
        map_int_int_init_internal_q (it(), ZERO(other.it()), other.it()->m_nentries);
        map_int_int_struct_copy (it(), other.it());
    }
}
...
CODE_SECTION(".text.t")
 map_int_int_t::map_int_int_t
        (map_int_int_t const *other)
{
    map_int_int_init_internal_q (it(), ZERO(other), other->m_nentries);
    map_int_int_struct_copy (it(), other);
}
...
CODE_SECTION(".text.map_int_int_t")
 map_int_int_t::~map_int_int_t
        ()
{
    map_int_int_destroy_internal (it());
}


CODE_SECTION(".text._destructor")
void map_int_int_t::_destructor
        ()
{
    map_int_int_destroy_internal (it());
}
...
void * map_int_int_t::operator new
        (size_t s)
{
...
void map_int_int_t::operator delete
        (void *p, size_t s)
{
    (void)s; /*unused*/
...
map_int_int_t & map_int_int_t::operator=
        (map_int_int_t const *other)
{
    map_int_int_clear_no_resize (it());
    map_int_int_expect_size (it(), other->it()->m_nentries); /* realloc to good size */
    map_int_int_struct_copy (it(), other->it());
    return *this;
}
...
double map_int_int_average_line_length
        (map_int_int_t const *self)
{
    double sum = 0.0;
    int i;
    for (i=0; i < self->m_hashsize; i++)
        sum+= map_int_int_hashlist_length (SLOT(self,i));
    return sum / (double)(self->m_hashsize);
}

CODE_SECTION(".text.variance_line_length")
double map_int_int_variance_line_length
        (map_int_int_t const *self)
{
    double average= map_int_int_average_line_length (self);
    double sum2= 0.0;
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        double diff= map_int_int_hashlist_length (SLOT(self,i)) - average;
        sum2+= diff*diff;
    }
...
int map_int_int_max_line_length
        (map_int_int_t const *self)
{
    int i;
    int max = 0;
    for (i=0; i < self->m_hashsize; i++) {
        int len= map_int_int_hashlist_length (SLOT(self,i));
        if (len > max) max= len;
    }
    return max;
}
...
int map_int_int_min_line_length
        (map_int_int_t const *self)
{
    int i;
...
double map_int_int_deviation_line_length
        (map_int_int_t const *self)
{
    return sqrt (map_int_int_variance_line_length (self));
}
#endif /* defined(HAVE_SQRT) */


CODE_SECTION(".text.dump")
void map_int_int_dump
        (FILE *thefile, map_int_int_t const *self)
{
    int i;
    fprintf (thefile, "(map_int_int_dump):\n");
...
#undef MAP_INT_INT_IMPLEMENTATION
