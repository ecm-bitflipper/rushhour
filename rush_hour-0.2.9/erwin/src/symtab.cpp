...
#ifndef SYMTAB_IMPLEMENTATION
#define SYMTAB_IMPLEMENTATION
#endif
...
#ifndef CHAR_CONST_P_BEING_USED
#define CHAR_CONST_P_BEING_USED 1
#endif
...
#ifdef CHAR_CONST_P_BEING_USED
#define IBEING_USED CHAR_CONST_P_BEING_USED
#endif

#ifdef CHAR_CONST_P_CMP
#define ICMP CHAR_CONST_P_CMP
#endif

#ifdef CHAR_CONST_P_ICOPY
#define ICOPY CHAR_CONST_P_ICOPY
#endif

#ifdef CHAR_CONST_P_ICOPY_PROVIDED
#define ICOPY_PROVIDED CHAR_CONST_P_ICOPY_PROVIDED
#endif

#ifdef CHAR_CONST_P_DETCMP
#define IDETCMP CHAR_CONST_P_DETCMP
#endif

#ifdef CHAR_CONST_P_DETCMP_PROVIDED
#define IDETCMP_PROVIDED CHAR_CONST_P_DETCMP_PROVIDED
#endif

#ifdef CHAR_CONST_P_EQUAL
#define IEQUAL CHAR_CONST_P_EQUAL
#endif

#ifdef CHAR_CONST_P_EQUAL_PROVIDED
#define IEQUAL_PROVIDED CHAR_CONST_P_EQUAL_PROVIDED
#endif

#ifdef CHAR_CONST_P_IFREE
#define IFREE CHAR_CONST_P_IFREE
#endif

#ifdef CHAR_CONST_P_IFREE_PROVIDED
#define IFREE_PROVIDED CHAR_CONST_P_IFREE_PROVIDED
#endif

#ifdef CHAR_CONST_P_HASH
#define IHASH CHAR_CONST_P_HASH
#endif

#ifdef CHAR_CONST_P_PRINT
#define IPRINT CHAR_CONST_P_PRINT
#endif

#ifdef CHAR_CONST_P_ZERO
#define IZERO CHAR_CONST_P_ZERO
#endif
...
#if SYMTAB_INITIAL_SIZE < 0
#  error SYMTAB_INITIAL_SIZE < 0
#endif

#if SYMTAB_MINIMAL_SIZE < 0
#  error SYMTAB_MINIMAL_SIZE < 0
#endif

#if SYMTAB_MINIMAL_SIZE == 0
#  define SYMTAB_ZERO_SIZE         1
#  define SYMTAB_MINIMAL_SIZE_OR_1 1
#elif SYMTAB_INITIAL_SIZE == 0
#  define SYMTAB_ZERO_SIZE         1
#  define SYMTAB_MINIMAL_SIZE_OR_1 SYMTAB_MINIMAL_SIZE
#else
#  define SYMTAB_ZERO_SIZE         0
#  define SYMTAB_MINIMAL_SIZE_OR_1 SYMTAB_MINIMAL_SIZE
#endif
...
#if defined(CHAR_CONST_P_IS_PTR) && defined(INT_IS_PTR) && !CHAR_CONST_P_IS_PTR && !INT_IS_PTR
...
#  else
#      define e_errno (ERWIN_CONST_CAST(symtab_t*,self)->m_errno)
#  endif
#else
...
#  define return_if_bad(self) \
      return_if_fail_ppxxt(self->m_sig == SYMTAB_SIG, self, self->m_sig, SYMTAB_SIG, "Structure was probably overwritten");
#  define return_val_if_bad(self, val) \
      return_val_if_fail_ppxxt(self->m_sig == SYMTAB_SIG, val, self, self->m_sig, SYMTAB_SIG, "Structure was probably overwritten");
#  define return_if_bad_pC(self, prog) \
      return_if_fail_ppxxtC(self->m_sig == SYMTAB_SIG, self, self->m_sig, SYMTAB_SIG, "Structure was probably overwritten", prog);
#else
#  define return_if_bad(self)
#  define return_val_if_bad(self, val)
#  define return_if_bad_pC(self, prog)
...
#define USER_ASSERTION_FAILED_HANDLER(FI,L,FU,A)    \
            ASSERTION_FAILED;                       \
            SYMTAB_ASSERTION_FAILED_HANDLER(FI,L,FU,A)
#endif
...
#define TOOSMALL(X,Y) (((X) * SYMTAB_TRIGGER_DENOMINATOR) > ((Y) * SYMTAB_TRIGGER_NUMERATOR))

#define TOOLARGE(X,Y) (((Y) >= SYMTAB_MINIMAL_SIZE) && \
                       (((X) * SYMTAB_TRIGGER_DENOMINATOR) < (((Y) * SYMTAB_TRIGGER_NUMERATOR)/4)))
...
#define ITER               ((symtab_internal_Iterator*)iter)
#define SORTITER           ((symtab_internal_sorted_Iterator*)iter)
#define SORTITERPTR        ((symtab_internal_sorted_ptr_Iterator*)iter)

#ifdef E_PROFILE
typedef union {
   symtab_t       *mapt;
   symtab_t const *mapconstt;
} hack_no_const;
#endif
 ...
 *    symtab_index_t == i_type_t
 *    symtab_value_t == o_type_t
 */
typedef int       o_type_t;
typedef int  o_type_result_t;
typedef int    o_type_var_t;
typedef char const *       i_type_t;
typedef char const *  i_type_result_t;

#define SPLIT_SECTIONS SYMTAB_SPLIT
...
/* Check that SYMTAB_ALLOW_NULL and ERWIN_THREAD_SAFE are not used at
...
#if 0
/* FIXME: NYI */
#if SYMTAB_ALLOW_NULL && defined(ERWIN_THREAD_SAFE)
#  error "Cannot use SYMTAB_ALLOW_NULL and ERWIN_THREAD_SAFE at the same time."
#endif
#endif

#if SYMTAB_NOMEM_IS_FATAL
...
#if SYMTAB_DYN_ZERO
#  define symtab_init_internal_q(SE,Z,SI)  symtab_init_internal(SE,Z,SI)
#else
#  define symtab_init_internal_q(SE,Z,SI) symtab_init_internal(SE,SI)
#endif



#  define feature_q(K,V)                feature(K,V)
#  define symtab_hashlist_new_q(K,V)               symtab_hashlist_new(K,V)
#  define symtab_find_any_pair_q(K,V,SE)   symtab_find_any_pair(K,V,SE)
#  define symtab_insert_q(SE,K,V)          symtab_insert(SE,K,V)

#  define symtab_insert_no_copy_no_rehash_q(SE,K,V) \
                                        symtab_insert_no_copy_no_rehash(SE,K,V)

#  define symtab_poke_internal_q(KO,VO,SE,K,KC,DK,V,VK,DV,I,O) \
                                        symtab_poke_internal(KO,VO,SE,K,KC,DK,V,VK,DV,I,O)

#  define symtab_poke_q(KO,VO,SE,K,V,I,O)  symtab_poke(KO,VO,SE,K,V,I,O)

#  define symtab_poke_no_icopy_q(KO,VO,SE,K,V,I,O)  \
                                        symtab_poke_no_icopy(KO,VO,SE,K,V,I,O)

#  define symtab_remove_internal_no_resize_q(SE,K,R) \
                                        symtab_remove_internal_no_resize(SE,K,R)

#  define symtab_remove_internal_q(SE,K,R) symtab_remove_internal(SE,K,R)
...
static
TYPE_STRUCT_T(symtab_hashlist_t)=
    TYPE_STRUCT_BEGIN(symtab_hashlist_t)
        TYPE_STRUCT_SLOT(symtab_hashlist_t, char_p, key)
        TYPE_STRUCT_SLOT(symtab_hashlist_t, int, value)
        TYPE_STRUCT_SLOT(symtab_hashlist_t, symtab_hashlist_t_p,        next)
    TYPE_STRUCT_END
;


TYPE_INFO_T(symtab_hashlist_t)= TYPE_INFO_STRUCT (symtab_hashlist_t, symtab_hashlist_t);


TYPE_INFO_T(symtab_hashlist_t_p)= TYPE_INFO_POINTER (symtab_hashlist_t *, symtab_hashlist_t);


TYPE_INFO_T(symtab_hashlist_t_const_p)= TYPE_INFO_POINTER (symtab_hashlist_t const *, symtab_hashlist_t);


TYPE_INFO_T(symtab_content)= TYPE_INFO_VECTOR (symtab_content, symtab_hashlist_t_p, symtab_t, m_hashsize);


TYPE_INFO_T(symtab_content_p)= TYPE_INFO_POINTER (symtab_content *, symtab_content);

static
TYPE_STRUCT_T(symtab_internal_Iterator)=
    TYPE_STRUCT_BEGIN(symtab_internal_Iterator)
        TYPE_STRUCT_SLOT(symtab_internal_Iterator, symtab_hashlist_t_p, cnode)
        TYPE_STRUCT_SLOT(symtab_internal_Iterator, int,      cslot)
    TYPE_STRUCT_END
;


TYPE_INFO_T(symtab_internal_Iterator)=
    TYPE_INFO_STRUCT (symtab_internal_Iterator, symtab_internal_Iterator);

static
TYPE_STRUCT_T(symtab_t)=
    TYPE_STRUCT_BEGIN(symtab_t)
#ifdef  SYMTAB_SUPER_TYPE
        TYPE_SUPER(symtab_t, SYMTAB_SUPER_CLASS),
#endif
        SYMTAB_TYPE_INFO_STD_MEMBERS
        symtab_record_sig     (TYPE_STRUCT_SLOT(symtab_t, int,             m_sig))
        symtab_record_table   (TYPE_STRUCT_SLOT(symtab_t, symtab_content_p,   m_table))
        symtab_record_sizes   (TYPE_STRUCT_SLOT(symtab_t, int,             m_hashsize))
        symtab_record_sizes   (TYPE_STRUCT_SLOT(symtab_t, int,             m_nentries))
        symtab_record_errno   (TYPE_STRUCT_SLOT(symtab_t, int,             m_errno))
        symtab_record_zero    (TYPE_STRUCT_SLOT(symtab_t, int, m_zero))
        symtab_record_compat  (TYPE_STRUCT_SLOT(symtab_t, symtab_internal_Iterator, compat1_iter))
        symtab_record_profile (TYPE_STRUCT_SLOT(symtab_t, int,             m_nrehash))
        symtab_record_profile (TYPE_STRUCT_SLOT(symtab_t, int,             m_nrehashops))
        symtab_record_profile (TYPE_STRUCT_SLOT(symtab_t, int,             m_nfind))
        symtab_record_profile (TYPE_STRUCT_SLOT(symtab_t, int,             m_ninsert))
        symtab_record_profile (TYPE_STRUCT_SLOT(symtab_t, int,             m_ndelete))
    TYPE_STRUCT_END
;


TYPE_INFO_T(symtab_t)= TYPE_INFO_STRUCT (symtab_t, symtab_t);


TYPE_INFO_T(symtab_t_p)= TYPE_INFO_POINTER (symtab_t *, symtab_t);


TYPE_INFO_T(symtab_t_const_p)= TYPE_INFO_POINTER (symtab_t const *, symtab_t);
...
symtab_hashlist_t * symtab_hashlist_new_q
        (char *  key, int  value)
{
    symtab_hashlist_t *self;
    E_SET_PTR(self, ERWIN_THING_NEW1 (symtab_hashlist_t));
...
void symtab_hashlist_delete
        (symtab_hashlist_t *list)
{
    symtab_hashlist_t *next;

    while (list) {
        IFREE (list->key);
        OFREE (list->value);
        E_SET_PTR(next, list->next);
        ERWIN_THING_DELETE1 (list, symtab_hashlist_t);
        E_SET_PTR(list, next);
    }
}
...
void symtab_hashlist_delete_flags
        (symtab_hashlist_t *list, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    symtab_hashlist_t *next;
...
void symtab_hashlist_delete_no_child_delete
        (symtab_hashlist_t *list)
{
    symtab_hashlist_t *next;
...
void symtab_hashlist_delete_cell
        (symtab_hashlist_t *list)
{
    ERWIN_THING_DELETE1 (list, symtab_hashlist_t);
}
...
ERWIN_BOOL symtab_init_internal_q
        (symtab_t *self, int  zero, int nelem)
{
    int size;
#ifndef NDEBUG
    int err= 0;
    ...
    if (nelem >= 0) { /* Compute hash size from expected number of elements: */
        size= ((nelem * SYMTAB_TRIGGER_DENOMINATOR + SYMTAB_TRIGGER_NUMERATOR - 1) / /* round up */
                SYMTAB_TRIGGER_NUMERATOR * 2
              ) + 1;
    }

    if (size < 0)
        size= SYMTAB_INITIAL_SIZE;

#if  SYMTAB_HAVE_SIG
    self->m_sig= SYMTAB_SIG;
#endif
    ...
    if (size == 1)    /* this means that the slot table is missing and we have a single list. */
        ...
        E_SET_PTR(self->m_table, ERWIN_THING_CNEW_ARRAY (symtab_content, (unsigned)size));
...
int symtab_init
        (symtab_t *self)
{
    symtab_init_internal_q (self, OZERO, SYMTAB_INITIAL_SIZE);
    return e_errno;
}

CODE_SECTION(".text.init_with_initial_size")
int symtab_init_with_initial_size
        (symtab_t *self, int size)
{
    symtab_init_internal_q (self, OZERO, size);
    return e_errno;
}
...
int symtab_init_with_zero_and_initial_size
        (symtab_t *self, int  zero, int size)
{
    symtab_init_internal_q (self, zero, size);
    return e_errno;
}
...
symtab_t * symtab_new_with_zero_and_initial_size
        (int  zero, int size)
{
    symtab_t *self;

    E_SET_PTR(self, ERWIN_THING_NEW1 (symtab_t));
    SET_ERRNO(E_OK);
    ...
    if (ERWIN_LIKELY(symtab_init_internal_q (self, zero, size)))
       return self;

    ERWIN_THING_DELETE1 (self, symtab_t);
    return NULL;
}
...
symtab_t * symtab_new_with_zero
        (int  zero)
{
    return symtab_new_with_zero_and_initial_size (zero, SYMTAB_INITIAL_SIZE);
}

#endif /* SYMTAB_DYN_ZERO */

#if SYMTAB_CONSTANT_ZERO
#define symtab_new_with_zero_and_initial_size(Z,S) symtab_new_with_initial_size(S)
#define symtab_new_with_zero(Z)                    symtab_new()
#endif
...
symtab_t * symtab_new_with_initial_size
        (int size)
{
    symtab_t *self;
    ...
    if (ERWIN_LIKELY(symtab_init_internal_q (self, OZERO, size)))
       return self;
...
symtab_t * symtab_new
        (void)
{
    return symtab_new_with_zero_and_initial_size (OZERO, SYMTAB_INITIAL_SIZE);
}


CODE_SECTION(".text.zero")
o_type_result_t symtab_zero
        (symtab_t const *self)
{
    (void)self; /*unused*/
...
void symtab_struct_copy
        (symtab_t *copy, symtab_t const *self)
{
    int i;

    for (i= 0; i < self->m_hashsize; i++) {
       symtab_hashlist_t *lauf;
       E_SET_PTR(lauf, SLOT(self,i));
       while (lauf) {
           symtab_insert_q (copy, lauf->key, lauf->value);
        ...
        (symtab_t const* self)
{
    ...
    E_SET_PTR(copy, symtab_new_with_zero_and_initial_size (ZERO(self), self->m_nentries));
    symtab_struct_copy (copy, self);
    return copy;
}

CODE_SECTION(".text.copy_err")
symtab_t * symtab_copy_err
        (symtab_t const* self, int *err)
{
    symtab_t *result;

    E_SET_PTR(result, symtab_copy (self));
...
int symtab_hash_size
        (symtab_t const* self)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return 0;
#endif
...
void symtab_clear_internal
        (symtab_t* self)
{
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        symtab_hashlist_delete (SLOT(self,i));
        E_SET_PTR(SLOT(self,i), NULL);
    }
}

CODE_SECTION(".text.clear_no_resize")
void symtab_clear_no_resize
        (symtab_t* self)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
...
void symtab_clear_internal_flags
        (symtab_t* self, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        symtab_hashlist_delete_flags (SLOT(self,i), keys, values);
        E_SET_PTR (SLOT(self,i), NULL);
    }
}
        ...
        (symtab_t* self, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    ...
    symtab_clear_internal_flags (self, keys, values);
    self->m_nentries= 0;
    SET_ERRNO(E_OK);
}
...
void symtab_xchg
        (symtab_t *self, symtab_t *other)
{
    symtab_record_table   (ERWIN_XCHG (symtab_content_p,   self->m_table,      other->m_table));
    symtab_record_sizes   (ERWIN_XCHG (int,             self->m_hashsize,   other->m_hashsize));
    symtab_record_sizes   (ERWIN_XCHG (int,             self->m_nentries,   other->m_nentries));
    symtab_record_errno   (ERWIN_XCHG (int,             self->m_errno,      other->m_errno));
    symtab_record_zero    (ERWIN_XCHG (int,        self->m_zero,       other->m_zero));
    symtab_record_compat  (ERWIN_XCHG (map_iterator_t,  self->compat1_iter, other->compat1_iter));
    symtab_record_profile (ERWIN_XCHG (int,             self->m_nrehash,    other->m_nrehash));
    symtab_record_profile (ERWIN_XCHG (int,             self->m_nrehashops, other->m_nrehashops));
    symtab_record_profile (ERWIN_XCHG (int,             self->m_nfind,      other->m_nfind));
    symtab_record_profile (ERWIN_XCHG (int,             self->m_ninsert,    other->m_ninsert));
    symtab_record_profile (ERWIN_XCHG (int,             self->m_ndelete,    other->m_ndelete));
}

ERWIN_STATIC_INLINE
CODE_SECTION(".text.destroy_internal_flags")
void symtab_destroy_internal_flags
        (
    symtab_t* self,
    ...
    ERWIN_BOOL destroy_values)
{
    ...
    symtab_clear_internal_flags (self, destroy_keys, destroy_values);
    FREE_TABLE(self);
...
void symtab_destroy
        (symtab_t* self)
{
    symtab_destroy_internal (self);
}

CODE_SECTION(".text.destroy_flags")
void symtab_destroy_flags
        (symtab_t* self, ERWIN_BOOL k, ERWIN_BOOL v)
{
    symtab_destroy_internal_flags (self, k, v);
}
...
void symtab_delete_flags
        (
    symtab_t* self, ERWIN_BOOL destroy_keys, ERWIN_BOOL destroy_values)
{
    if (self != NULL) {
        symtab_destroy_internal_flags (self, destroy_keys, destroy_values);
        ERWIN_THING_DELETE1 (self, symtab_t);
    }
}
...
ERWIN_BOOL symtab_expect_size
        (symtab_t *self, int newsize)
{
    int new_hash_size;
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    if (TOOSMALL(newsize, new_hash_size)) {
        ...
        if (new_hash_size <= 1) {
             ...
             * this too small. :-) */
            if (newsize <= SYMTAB_PLAIN_LIST_LENGTH)
                return ERWIN_FALSE;
        }
#endif
        ...
        symtab_rehash (self, new_hash_size);
    ...
    if (TOOLARGE(newsize, new_hash_size)) {
...
#endif
        {
            ...
            if (new_hash_size < SYMTAB_MINIMAL_SIZE_OR_1)
                new_hash_size= SYMTAB_MINIMAL_SIZE_OR_1;
        }
...
ERWIN_BOOL symtab_shrink_perhaps
        (symtab_t *self)
{
...
#if SYMTAB_ZERO_SIZE
        if (self->m_nentries <= SYMTAB_PLAIN_LIST_LENGTH)
            newsize= 1;
        else
#endif
        {
            ...
            if (newsize < SYMTAB_MINIMAL_SIZE_OR_1)
                newsize= SYMTAB_MINIMAL_SIZE_OR_1;
        }

        symtab_rehash (self, newsize);
...
void symtab_clear_flags
        (symtab_t* self, ERWIN_BOOL keys, ERWIN_BOOL values)
{
    symtab_clear_flags_no_resize (self, keys, values);
    symtab_shrink_perhaps (self);
}
...
unsigned symtab_slot_no
        (symtab_t const *self, char const *  key)
{
    int slot;
    return0_if_null (self);
...
symtab_hashlist_t ** symtab_find_internal_ptr
        (symtab_t const* self, char const *  key)
{
    symtab_hashlist_t **lauf;
#ifdef E_PROFILE
    ...
    E_SET_PTR(lauf, &SLOT(self, symtab_slot_no (self, key)));
    while (*lauf) {
...
symtab_hashlist_t * symtab_find_internal
        (symtab_t const* self, char const *  key)
{
    symtab_hashlist_t **result;

    E_SET_PTR(result, symtab_find_internal_ptr (self, key));
#if SYMTAB_ALLOW_NULL
    if (result == NULL)
        return NULL;
#endif
...
symtab_element_ptr_t symtab_find_ptr
        (symtab_t const* self, char const *  key)
{
    symtab_hashlist_t *lauf;

    E_SET_PTR(lauf, symtab_find_internal (self, key));
    if (lauf == NULL) {
 ...
 *        symtab_ensure which currently evaluates ENSURE_VALUE to often).
 *
        ...
        ERWIN_BOOL     overwrite)
{
    ...
    E_SET_PTR(lauf, symtab_find_internal_ptr (self, key));
#if SYMTAB_ALLOW_NULL
        ...
        if (introduce) {
            ...
            if (dovaluecopy) {
                ...
                if (dokeycopy) {
                    ...
                    E_SET_PTR(*lauf, symtab_hashlist_new (keycopy, valuecopy));
                    ...
                    E_SET_PTR(*lauf, symtab_hashlist_new (givenkeycopy, valuecopy));
            ...
            else
            {
                if (dokeycopy) {
                    ...
                    E_SET_PTR(*lauf, symtab_hashlist_new_q (keycopy, givenvaluecopy));
                    ...
                    E_SET_PTR(*lauf, symtab_hashlist_new_q (givenkeycopy, givenvaluecopy));
...
int symtab_poke_q
        (   
    char const * *ko, int *vo,
    symtab_t *self,
    char const *  k, int  v,
    ERWIN_BOOL i, ERWIN_BOOL o)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return symtab_poke_internal_q (ko,vo,self, k,IZEROVAR,ERWIN_TRUE, v,ZEROVAR(self),ERWIN_TRUE, i,o);
}

CODE_SECTION(".text.poke_no_icopy_q")
int symtab_poke_no_icopy_q
        (
    char const * *ko, int *vo, symtab_t *self, char *  k, int  v, ERWIN_BOOL i, ERWIN_BOOL o)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return symtab_poke_internal_q (ko,vo,self, k,k,ERWIN_FALSE, v,ZEROVAR(self),ERWIN_TRUE, i,o);
}

CODE_SECTION(".text.poke_no_ocopy")
int symtab_poke_no_ocopy
        (
    char const * *ko, int *vo, symtab_t *self, char const *  k, int  v, ERWIN_BOOL i, ERWIN_BOOL o)
{
    return symtab_poke_internal (ko,vo,self, k,IZEROVAR,ERWIN_TRUE, v,v,ERWIN_FALSE, i,o);
}
...
int symtab_poke_no_icopy_no_ocopy
    ...
    char const * *ko, int *vo, symtab_t *self, char *  k, int  v, ERWIN_BOOL i, ERWIN_BOOL o)
{
    return symtab_poke_internal (ko, vo, self, k,k,ERWIN_FALSE, v,v,ERWIN_FALSE, i,o);
}
...
o_type_result_t symtab_find
        (symtab_t const* self, char const *  key)
{
    symtab_hashlist_t *lauf;

#if defined(SYMTAB_ALLOW_NULL) && SYMTAB_CONSTANT_ZERO
    if (self == NULL) {
        SET_ERRNO(E_KEYNOTFOUND);
        return OZERO;
    }
...
o_type_result_t symtab_find_ensure
        (symtab_t * self, char const *  key)
{
    symtab_element_ptr_t result;
    E_SET_PTR(result, symtab_find_ptr_ensure (self, key));
#if !SYMTAB_NOMEM_IS_FATAL
...
i_type_result_t symtab_find_key
        (symtab_t const* self, char const *  key)
{
    symtab_hashlist_t *lauf;
...
o_type_var_t symtab_modify
        (symtab_t *self, char const *  key, int  value)
{
    /* Does not work without a cast because poke_internal takes char const * ,
     * not char const * .
...
symtab_element_ptr_t symtab_find_ptr_ensure
        (symtab_t *self, char const *  key)
/* NOTE: This cannot be implemented with _poke_internal, since it
 *       returns a pointer to an internal structure. */
/* FIXME: extract common parts from symtab_ensure and symtab_find_ptr_ensure,
 *        which do essentially the same. */
{
    ...
    SET_ERRNO(E_OK);
    if (*lauf == NULL) {
        if (ERWIN_UNLIKELY(symtab_grow_perhaps (self))) {
            E_SET_PTR(lauf, symtab_find_internal_ptr (self, key));
            return0_if_null_pp (lauf, self);
...

        {
            ...
            {
                int valuecopy;
                E_SET_VALUE (valuecopy, SYMTAB_ENSURE_VALUE(self, key, &copyerror));
                if (ERWIN_UNLIKELY(OCOPY_PROVIDED && copyerror)) {
                    IFREE (keycopy);
                    MAP_NOMEM;
                    return NULL;
...
int symtab_insert_no_copy_no_rehash_q
        (
        symtab_t* self,
        char * keycopy,
        int valuecopy)
 ...
 *       delete keycopy or valuecopy! */
{
    ...
    E_SET_PTR(lauf, symtab_find_internal_ptr (self, keycopy));
    return_val_if_null_pp (lauf, ASSERTION_FAILED, self);

    if (*lauf != NULL)
        return PASS_ERRNO(E_EXISTINGKEY);

    E_SET_PTR(*lauf, symtab_hashlist_new_q (keycopy, valuecopy));
...
int symtab_insert_map
        (symtab_t* self, symtab_t const *other)
{
    int i;
    int total_errno;
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       while (lauf) {
           symtab_insert_q (self, lauf->key, lauf->value);
           if (e_errno != E_OK)
        ...
        (symtab_t* self, symtab_t const *other)
{
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           symtab_set (self, lauf->key, lauf->value);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
        ...
        (symtab_t* self, symtab_t const *other)
{
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           symtab_modify (self, lauf->key, lauf->value);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
...
int symtab_erase_map_no_resize
        (symtab_t* self, symtab_t const *other)
{
    int i;
    int total_errno;
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           symtab_erase_no_resize (self, lauf->key);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
...
int symtab_intersect_no_resize
        (symtab_t* self, symtab_t const *other)
{
    int i;
    int cnt;
    ...
    for (i= 0; i < self->m_hashsize; i++) {
        ...
        E_SET_PTR (lauf, &SLOT(self,i));
        while (*lauf) {
            (void)symtab_find (other, (*lauf)->key);
            if (e_errno != E_OK) {
                 symtab_hashlist_t *cell;
                 E_SET_PTR (cell,  *lauf);
                 E_SET_PTR (*lauf, (*lauf)->next);

                 IFREE (cell->key);
                 OFREE (cell->value);
                 symtab_hashlist_delete_cell (cell);
                 self->m_nentries--;
                 cnt++;
            }
            else
...
int symtab_intersect
        (symtab_t* self, symtab_t const *other)
{
    int cnt= symtab_intersect_no_resize (self, other);
    symtab_shrink_perhaps (self);
    return cnt;
}
 ...
 * able to return a pointer to that it char const *  has a `const &' appended.
 * Of course we could search again, but that's not fast.
 */
CODE_SECTION(".text.ensure")
i_type_result_t symtab_ensure
        (symtab_t *self, char const *  key)
{
    symtab_hashlist_t **lauf;
...
i_type_result_t symtab_ensure_no_icopy
        (symtab_t *self, char *  key)
{
    symtab_hashlist_t **lauf;
    ...
    if (*lauf == NULL) {
...

        {
            ...
            E_SET_PTR (*lauf, symtab_hashlist_new_q (key, valuecopy));
            if (ERWIN_UNLIKELY(*lauf == NULL)) {
                OFREE (valuecopy);
                MAP_NOMEM;
                return IZERO;
...
ERWIN_BOOL symtab_remove_internal_no_resize_q
        (
    symtab_t *self,
    char const *  key,
    int *result)
...
ERWIN_BOOL symtab_remove_internal_q
        (
    symtab_t *self,
    char const *  key,
    int *result)
{
    ...
    return symtab_remove_internal_no_resize_q (self, key, result);
}

CODE_SECTION(".text.remove")
o_type_var_t symtab_remove
        (symtab_t *self, char const *  key)
{
    int result;
#if defined(SYMTAB_ALLOW_NULL) && SYMTAB_CONSTANT_ZERO
...
#endif
    if (symtab_remove_internal_q (self, key, &result))
        return result;
    return ZEROVAR(self);
}
...
o_type_var_t symtab_remove_no_resize
        (symtab_t *self, char const *  key)
{
    int result;
#if defined(SYMTAB_ALLOW_NULL) && SYMTAB_CONSTANT_ZERO
...
#endif
    if (symtab_remove_internal_no_resize_q (self, key, &result))
        return result;
    return ZEROVAR(self);
}
...
int symtab_remove_if
        (symtab_t *self, symtab_feature_t feature, ERWIN_BOOL value)
{
    int slot;
    int cnt= 0;
        ...
        (symtab_t* self, int newsize)
{
    ...
    if (newsize != self->m_hashsize) {
        ...
        if (newsize == 1) {
            ...
            E_SET_PTR (newtable, ERWIN_THING_CNEW_ARRAY (symtab_content, (unsigned)newsize));
        ...
        for (i=0; i < self->m_hashsize; i++) {
            ...
            while (lauf != NULL) {
                symtab_hashlist_t *next;
                symtab_hashlist_t **newcell= &GET_SLOT(
                                      newtable,
                                      newsize,
...
symtab_hashlist_t * symtab_first_internal
        (symtab_t const *self, e_iterator_t *iter)
{
...
symtab_hashlist_t * symtab_next_internal
        (symtab_t const*self, e_iterator_t *iter)
{
    if (ERWIN_UNLIKELY(ITER->cslot == -1))
        return symtab_first_internal (self, iter);
...
void symtab_init_iterator
        (symtab_t const *self, e_iterator_t *iter)
{
    (void)self; /*unused*/

#ifndef SYMTAB_ALLOW_NULL
    return_if_null_or_bad (self);
#endif
        ...
        (void const *a, void const *b)
{
    ...
    (void)b; /*unused*/
#ifdef ICMP
#define symtab_check_cmp_keys
#define symtab_check0_cmp_keys
    return ICMP ((((symtab_pair_t*)a)->key),(((symtab_pair_t*)b)->key));
#else
#define symtab_check_cmp_keys \
...
#define symtab_check_cmp_keys
#define symtab_check0_cmp_keys
    return ICMP ((*((char const * *)a)),(*((char const * *)b)));
#else
#define symtab_check_cmp_keys \
    return_if_reached_pt ("ICMP is not implemented.  Sorting keys is not possible.")
#define symtab_check0_cmp_keys \
        ...
        (void const *a, void const *b)
{
    ...
    (void)b; /*unused*/
#ifdef ICMP
#define symtab_check_cmp_keys_ptr
#define symtab_check0_cmp_keys_ptr
    return ICMP ((((symtab_pair_ptr_t*)a)->key),(((symtab_pair_ptr_t*)b)->key));
#else
#define symtab_check_cmp_keys_ptr \
    return_if_reached_pt ("ICMP is not implemented.  Sorting keys is not possible.")
#define symtab_check0_cmp_keys_ptr \
        ...
        (void const *a, void const *b)
{
    ...
    (void)b; /*unused*/
#ifdef OCMP
#define symtab_check_cmp_values
#define symtab_check0_cmp_values
    return OCMP ((((symtab_pair_t*)a)->value),(((symtab_pair_t*)b)->value));
#else
#define symtab_check_cmp_values \
...
ATTR_FUNC_UNUSED
int symtab_pair_cmp_values_ptr
        (void const *a, void const *b)
{
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    (void)a; /*unused*/
    (void)b; /*unused*/
#ifdef OCMP
#define symtab_check_cmp_values_ptr
#define symtab_check0_cmp_values_ptr
    return OCMP ((*(((symtab_pair_ptr_t*)a)->value)),(*(((symtab_pair_ptr_t*)b)->value)));
#else
#define symtab_check_cmp_values_ptr \
    return_if_reached_pt ("OCMP is not implemented.  Sorting values is not possible.")
#define symtab_check0_cmp_values_ptr \
...
ATTR_FUNC_UNUSED
int symtab_pair_cmp_values_and_keys
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#define symtab_check_cmp_values_and_keys
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= OCMP ((((symtab_pair_t*)a)->value),(((symtab_pair_t*)b)->value));
    if (result == 0)
        result= ICMP ((((symtab_pair_t*)a)->key),(((symtab_pair_t*)b)->key));
#else
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
#define symtab_check_cmp_values_and_keys \
...
ATTR_FUNC_UNUSED
int symtab_pair_cmp_values_and_keys_ptr
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#if defined(OCMP) && defined(ICMP)
#define symtab_check_cmp_values_and_keys_ptr
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= OCMP ((*(((symtab_pair_ptr_t*)a)->value)),(*(((symtab_pair_ptr_t*)b)->value)));
    if (result == 0)
        result= ICMP ((((symtab_pair_ptr_t*)a)->key),(((symtab_pair_ptr_t*)b)->key));
#else
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
#define symtab_check_cmp_values_and_keys_ptr \
    return_if_reached_pt ("OCMP or ICMP is not implemented.  Sorting values is not possible.");
...
ATTR_FUNC_UNUSED
int symtab_pair_cmp_keys_and_values
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#if defined(OCMP) && defined(ICMP)
#define symtab_check_cmp_keys_and_values
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= ICMP ((((symtab_pair_t*)a)->key),(((symtab_pair_t*)b)->key));
    if (result == 0)
        result= OCMP ((((symtab_pair_t*)a)->value),(((symtab_pair_t*)b)->value));
...
ATTR_FUNC_UNUSED
int symtab_pair_cmp_keys_and_values_ptr
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#if defined(OCMP) && defined(ICMP)
#define symtab_check_cmp_keys_and_values_ptr
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= ICMP ((((symtab_pair_ptr_t*)a)->key),(((symtab_pair_ptr_t*)b)->key));
    if (result == 0)
        result= OCMP ((*(((symtab_pair_ptr_t*)a)->value)),(*(((symtab_pair_ptr_t*)b)->value)));
        ...
        (void const *a, void const *b)
{
    ...
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= IDETCMP ((*((char const * const*)a)), (*((char const * const*)b)));
#if defined(ICMP) && IDETCMP_PROVIDED
    if (result == 0)
        result= ICMP ((*((char const * const*)a)), (*((char const * const*)b)));
#endif
    return result;
}

#if !IDETCMP_PROVIDED && !IEQUAL_PROVIDED
#  define symtab_detcmp_pairs     symtab_pair_cmp_keys
#  define symtab_detcmp_pairs_ptr symtab_pair_cmp_keys_ptr
#else
#  define SYMTAB_DET_SORT_PROVIDED 1
#endif

#ifndef SYMTAB_DET_SORT_PROVIDED
#  define SYMTAB_DET_SORT_PROVIDED 0
#endif

#if ODETCMP_PROVIDED || SYMTAB_DET_SORT_PROVIDED
#  define SYMTAB_ODET_REQUIRE 1
#endif

#ifndef SYMTAB_ODET_REQUIRE
#  define SYMTAB_ODET_REQUIRE 0
#endif
        ...
        (void const *a, void const *b)
{
    ...
    result= OCMP (*((int const*)a), *((int const*)b));
#else
#define symtab_check_detcmp_values \
    return0_if_reached_pt ("Need INT_DETCMP or INT_CMP to enforce determinism on symtab.  Try to '#define INT_DETCMP(x,y) false_if_reached_pk(CHAR_CONST_P_DETCMP)' if you think you never invoke it.");
    result=0;
#endif

#ifndef symtab_check_detcmp_values
#define symtab_check_detcmp_values
#endif /* !defined symtab_check_detcmp_values */
        ...
        (void const *a, void const *b)
{
    ...
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= IDETCMP ((((symtab_pair_t*)a)->key),(((symtab_pair_t*)b)->key));

    if (result == 0)
        result= ODETCMP ((((symtab_pair_t*)a)->value),(((symtab_pair_t*)b)->value));
        ...
        (void const *a, void const *b)
{
    ...
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= IDETCMP ((((symtab_pair_ptr_t*)a)->key),(((symtab_pair_ptr_t*)b)->key));

    if (result == 0)
        result= ODETCMP ((*(((symtab_pair_ptr_t*)a)->value)),(*(((symtab_pair_ptr_t*)b)->value)));
...
#endif /* SYMTAB_DET_SORT_PROVIDED */
...
void symtab_det_sort
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
    qsort ((void*)SORTITER->table,
           self->m_nentries,
           sizeof (symtab_pair_t),
           symtab_detcmp_pairs);
}
...
void symtab_det_sort_ptr
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
    qsort ((void*)SORTITERPTR->table,
           self->m_nentries,
           sizeof (symtab_pair_ptr_t),
           symtab_detcmp_pairs_ptr);
}
...
void symtab_free_sorted_iterator
        (void *iter)
{
    symtab_delete_entries (SORTITER->table);
}


static
CODE_SECTION(".text.free_sorted_iterator_ptr")
void symtab_free_sorted_iterator_ptr
        (void *iter)
{
    symtab_delete_entries_ptr (SORTITERPTR->table);
}


CODE_SECTION(".text.init_iterator_sorted_by_key")
void symtab_init_iterator_sorted_by_key
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITER->table, symtab_get_entries (self));
        SORTITER->count= 0;
        SORTITER->lauf=  0;
        E_SET_PTR (SORTITER->free_cb, symtab_free_sorted_iterator);
        return;
    }
#else
    return_if_null_or_bad (self);
    ...
    return_if_fail_pp ((void*)SORTITER_VOLATILE->free_cb == (void*)symtab_free_sorted_iterator, self);
    ...
    if (e_require_determinism ()) {
        symtab_det_sort (self, iter);
        e_merge_sort ((void*)SORTITER->table,
               self->m_nentries,
               sizeof (symtab_pair_t),
               symtab_pair_cmp_keys);
    }
    else
#endif
    {
...
void symtab_init_iterator_sorted_by_value
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_pp2 (SORTITER_VOLATILE->p,
        SORTITER, self);
    return_if_fail_pp2 ((void*)SORTITER_VOLATILE->free_cb == (void*)symtab_free_sorted_iterator,
        SORTITER, self);
...
void symtab_init_iterator_sorted_by_value_and_key
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    symtab_check_cmp_values_and_keys;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITER->table,
               self->m_nentries,
               sizeof (symtab_pair_t),
               symtab_pair_cmp_values_and_keys);
    }
    else
#endif
    {
...
void symtab_init_iterator_sorted_by_key_and_value
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    symtab_check_cmp_keys_and_values;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITER->table,
               self->m_nentries,
               sizeof (symtab_pair_t),
               symtab_pair_cmp_keys_and_values);
    }
    else
#endif
    {
...
void symtab_init_iterator_sorted_by_user
        (
    symtab_t const *self,
...
void symtab_init_iterator_ptr_sorted_by_key
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITERPTR->table, symtab_get_entries_ptr (self));
        SORTITERPTR->count= 0;
        SORTITERPTR->lauf=  0;
        E_SET_PTR (SORTITERPTR->free_cb, symtab_free_sorted_iterator_ptr);
        return;
    }
#endif
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    symtab_check_cmp_keys_ptr;
    ...
    return_if_null_pp2 (SORTITER_VOLATILE->p,
        SORTITER, self);
    return_if_fail_pp2 ((void*)SORTITER_VOLATILE->free_cb == (void*)symtab_free_sorted_iterator_ptr,
        SORTITER, self);
     ...
     * we still need to pre-sort and use (the stable) merge sort.  */
    if (e_require_determinism ()) {
        symtab_det_sort_ptr (self, iter);
        e_merge_sort ((void*)SORTITERPTR->table,
               self->m_nentries,
               sizeof (symtab_pair_ptr_t),
               symtab_pair_cmp_keys_ptr);
    }
    else
#endif
    {
...
void symtab_init_iterator_ptr_sorted_by_value
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
...
void symtab_init_iterator_ptr_sorted_by_value_and_key
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    symtab_check_cmp_values_and_keys_ptr;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITERPTR->table,
               self->m_nentries,
               sizeof (symtab_pair_ptr_t),
               symtab_pair_cmp_values_and_keys_ptr);
    }
    else
#endif
    {
...
void symtab_init_iterator_ptr_sorted_by_key_and_value
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    return_if_null_or_bad (self);
    return_if_null_pp (iter, self);
    symtab_check_cmp_keys_and_values_ptr;
    ...
    if (e_require_determinism ()) {
        ...
        e_merge_sort ((void*)SORTITERPTR->table,
               self->m_nentries,
               sizeof (symtab_pair_ptr_t),
               symtab_pair_cmp_keys_and_values_ptr);
    }
    else
#endif
    {
...
void symtab_init_iterator_ptr_sorted_by_user
        (
    symtab_t const *self,
    e_iterator_sorted_t *iter,
    symtab_pair_ptr_cmp_t u)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITERPTR->table, symtab_get_entries_ptr (self));
...
void symtab_init_iterator_sorted_by_key_reverse
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_sorted_by_key (self, iter);
    SORTITER->lauf= SORTITER->count;
}


CODE_SECTION(".text.init_iterator_sorted_by_value_reverse")
void symtab_init_iterator_sorted_by_value_reverse
        (symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_sorted_by_value (self, iter);
    SORTITER->lauf= SORTITER->count;
}
...
void symtab_init_iterator_sorted_by_value_and_key_reverse
    ...
    symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_sorted_by_value_and_key (self, iter);
    SORTITER->lauf= SORTITER->count;
}
...
void symtab_init_iterator_sorted_by_key_and_value_reverse
    ...
    symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_sorted_by_key_and_value (self, iter);
    SORTITER->lauf= SORTITER->count;
}
...
void symtab_init_iterator_sorted_by_user_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, symtab_pair_cmp_t u)
{
    symtab_init_iterator_sorted_by_user (self, iter, u);
    SORTITER->lauf= SORTITER->count;
}


CODE_SECTION(".text.init_iterator_ptr_sorted_by_key_reverse")
void symtab_init_iterator_ptr_sorted_by_key_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_ptr_sorted_by_key (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}


CODE_SECTION(".text.init_iterator_ptr_sorted_by_value_reverse")
void symtab_init_iterator_ptr_sorted_by_value_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_ptr_sorted_by_value (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}
...
void symtab_init_iterator_ptr_sorted_by_value_and_key_reverse
    ...
    symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_ptr_sorted_by_value_and_key (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}
...
void symtab_init_iterator_ptr_sorted_by_key_and_value_reverse
    ...
    symtab_t const *self, e_iterator_sorted_t *iter)
{
    symtab_init_iterator_ptr_sorted_by_key_and_value (self, iter);
    SORTITERPTR->lauf= SORTITERPTR->count;
}
...
void symtab_init_iterator_ptr_sorted_by_user_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, symtab_pair_ptr_cmp_t u)
{
    symtab_init_iterator_ptr_sorted_by_user (self, iter, u);
    SORTITERPTR->lauf= SORTITERPTR->count;
}


CODE_SECTION(".text.next_iteration_sorted")
ERWIN_BOOL symtab_next_iteration_sorted
        (symtab_t const *self,
                                e_iterator_sorted_t *iter,
                                char const * *keyp,
                                int *valuep)
...
ERWIN_BOOL symtab_next_iteration_sorted_ptr
        (symtab_t const *self,
                                e_iterator_sorted_t *iter,
                                char const * *keyp,
                                symtab_element_ptr_t *valuepp)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_keys
        (symtab_t const *self,
                                e_iterator_sorted_t *iter,
                                char const * *keyp)
{
...
ERWIN_BOOL symtab_next_iteration_sorted_values
        (symtab_t const *self,
                                e_iterator_sorted_t *iter,
                                int *valuep)
{
...
ERWIN_BOOL symtab_next_iteration_sorted_values_ptr
        (
    symtab_t const *self, e_iterator_sorted_t *iter, symtab_element_ptr_t *valuepp)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_pairs
        (symtab_t const *self,
                                e_iterator_sorted_t *iter,
                                symtab_pair_t *p)
{
...
ERWIN_BOOL symtab_next_iteration_sorted_pairs_ptr
        (
    symtab_t const *self, e_iterator_sorted_t *iter, symtab_pair_ptr_t *p)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, char const * *keyp, int *valuep)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_ptr_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, char const * *keyp, symtab_element_ptr_t *valuepp)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_keys_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, char const * *keyp)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_values_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, int *valuep)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_values_ptr_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, symtab_element_ptr_t *valuepp)
{
#ifdef SYMTAB_ALLOW_NULL
...
ERWIN_BOOL symtab_next_iteration_sorted_pairs_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, symtab_pair_t *p)
{
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERWIN_FALSE;
...
ERWIN_BOOL symtab_next_iteration_sorted_pairs_ptr_reverse
        (
    symtab_t const *self, e_iterator_sorted_t *iter, symtab_pair_ptr_t *p)
{
#ifdef SYMTAB_ALLOW_NULL
...
ERWIN_BOOL symtab_next_iteration
        (
    symtab_t const*self,
    e_iterator_t *iter,
    char const * *keyp,
    int *valuep)
{
    ...
    E_SET_PTR (it, symtab_next_internal (self, iter));
...
ERWIN_BOOL symtab_next_iteration_values
        (
    symtab_t const *self,
    e_iterator_t *iter,
    int *valuep)
...
ERWIN_BOOL symtab_next_iteration_keys
        (symtab_t const *self, e_iterator_t *iter, char const * *keyp)
{
    symtab_hashlist_t *it;
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
...
ERWIN_BOOL symtab_next_iteration_pairs
        (symtab_t const *self, e_iterator_t *iter, symtab_pair_t *pairp)
{
    symtab_hashlist_t *it;
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
...
ERWIN_BOOL symtab_next_iteration_ptr
        (
    symtab_t const*self,
    e_iterator_t *iter,
    char const * *keyp,
...
ERWIN_BOOL symtab_next_iteration_values_ptr
        (
    symtab_t const *self,
    e_iterator_t *iter,
    symtab_element_ptr_t *valuepp)
...
ERWIN_BOOL symtab_next_iteration_pairs_ptr
        (symtab_t const *self, e_iterator_t *iter, symtab_pair_ptr_t *pairp)
{
    symtab_hashlist_t *it;
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL)
...
symtab_pair_t * symtab_get_entries
        (symtab_t const *self)
{
    struct _symtab_pair_t *result, *rlauf;
    int    i;

#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, PAIR_NEW_ARRAY (symtab_pair_dynarray, 1));
    ...
    E_SET_PTR (result, PAIR_NEW_ARRAY (symtab_pair_dynarray, self->m_nentries+1));
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (symtab_pair_t),
               (int(*)(void const *, void const *))symtab_detcmp_pairs);
#endif
...
symtab_pair_ptr_t * symtab_get_entries_ptr
        (symtab_t const*self)
{
    struct _symtab_pair_ptr_t *result, *rlauf;
    int    i;

#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, ERWIN_THING_NEW_ARRAY (symtab_pair_ptr_dynarray, 1));
        if (result == NULL) {
            MAP_NOMEM;
            return NULL;
        }
    ...
    E_SET_PTR (result, ERWIN_THING_NEW_ARRAY (symtab_pair_ptr_dynarray, self->m_nentries+1));
    E_SET_PTR (rlauf,  result);
    if (ERWIN_UNLIKELY(result == NULL)) {
        MAP_NOMEM;
        return NULL;
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (symtab_pair_ptr_t),
               (int(*)(void const *, void const *))symtab_detcmp_pairs_ptr);
#endif
        ...
        (symtab_t const *self)
{
...
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, VALUE_NEW_ARRAY (symtab_value_dynarray, 1));
        if (ERWIN_UNLIKELY(result == NULL)) {
            MAP_NOMEM;
            return NULL;
        }
    ...
    E_SET_PTR (result, VALUE_NEW_ARRAY (symtab_value_dynarray, self->m_nentries+1));
    E_SET_PTR (rlauf,  result);
    if (ERWIN_UNLIKELY(result == NULL)) {
        MAP_NOMEM;
        return NULL;
...
#if defined(E_REQUIRE_DETERMINISM) && !defined(E_WEAK_DETERMINISM)
    if (e_require_determinism ()) {
        symtab_check_detcmp_values;
        qsort (result,
               self->m_nentries,
               sizeof (int),
               (int(*)(void const *, void const *))symtab_pair_detcmp_values);
    }
#endif
        ...
        (symtab_t const *self)
{
...
#ifdef SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, KEY_NEW_ARRAY (symtab_key_dynarray, 1));
        if (ERWIN_UNLIKELY(result == NULL)) {
            MAP_NOMEM;
            return NULL;
        }
    ...
    E_SET_PTR (result, KEY_NEW_ARRAY (symtab_key_dynarray, self->m_nentries+1));
    E_SET_PTR (rlauf,  result);
    if (ERWIN_UNLIKELY(result == NULL)) {
        MAP_NOMEM;
        return NULL;
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (char const *),
               (int(*)(void const *, void const *))symtab_pair_detcmp_keys);
#endif
...
void symtab_delete_entries
        (symtab_pair_t *entries)
{
    PAIR_DELETE_ARRAY (entries);
}

CODE_SECTION(".text.delete_entries_ptr")
void symtab_delete_entries_ptr
        (symtab_pair_ptr_t *entries_ptr)
{
...
ERWIN_BOOL symtab_equal
        (symtab_t const *self, symtab_t const *other)
{
    ...
    symtab_forall_nondet (self, iter, key, value)
    {
        int *value_ptr;
        E_SET_PTR (value_ptr, symtab_find_ptr (other, key));
        if (value_ptr == NULL)
            return ERWIN_FALSE;
    ...
    error_return0_if_reached_ppt (
        self,
        "Because neither INT_EQUAL nor INT_CMP was #defined, symtab_equal could not be implemented.");
#endif
}


CODE_SECTION(".text.find_any_pair_q")
int symtab_find_any_pair_q
        (symtab_key_result_t *key_p, symtab_element_ptr_t *value_p, symtab_t const* self)
{
    int i;
    ...
    { /* Find smallest entry. */
        ...
        for (i= 0; i < self->m_hashsize; i++) {
           ...
           E_SET_PTR (lauf, SLOT(self,i));
           while (lauf) {
               if (first || symtab_pair_detcmp_keys (&lauf->key, &last) < 0) {
                   first= 0;
...
i_type_result_t symtab_find_any_key
        (symtab_t const *self)
{
    char const * result;
    int n= symtab_find_any_pair_q (&result, NULL, self);
...
symtab_element_ptr_t symtab_find_any_ptr
        (symtab_t const *self)
{
    symtab_element_ptr_t result;
    int n= symtab_find_any_pair (NULL, &result, self);
    if (ERWIN_UNLIKELY(n == 0))
        return NULL;
    return result;
}
...
o_type_result_t symtab_find_any
        (symtab_t const *self)
{
    symtab_element_ptr_t result_p;
    int n= symtab_find_any_pair (NULL, &result_p, self);
    if (ERWIN_UNLIKELY(n == 0))
        return ZERO(self);
    return *result_p;
}
        ...
        (symtab_t const *self, symtab_t const *other)
{
    ...
    symtab_forall_keys_nondet (self, iter, key) {
        (void)symtab_find_ptr (other, key);
        if (e_errno != E_OK)
            return ERWIN_FALSE;
    }
...
/* must be NULL safe per definition! */
{
    ...
    int result= 0;
    int cnt;
    symtab_check0_cmp_keys;
    symtab_check0_cmp_values;
#endif
    ...
    E_SET_PTR (them1, symtab_get_entries (self));
    if (ERWIN_UNLIKELY(e_errno != E_OK))
        return 0;

    E_SET_PTR (them2, symtab_get_entries (other));
    if (ERWIN_UNLIKELY(e_errno != E_OK)) {
        symtab_delete_entries (them1);
        return 0;
    }

#if defined(E_REQUIRE_DETERMINISM) && !defined(E_WEAK_DETERMINISM)
    if (e_require_determinism()) {
        qsort ((void*)them1, cnt, sizeof (symtab_pair_t), symtab_pair_detcmp_keys);
        qsort ((void*)them2, cnt, sizeof (symtab_pair_t), symtab_pair_detcmp_keys);

        e_merge_sort ((void*)them1, cnt, sizeof (symtab_pair_t), symtab_pair_cmp_keys);
        e_merge_sort ((void*)them2, cnt, sizeof (symtab_pair_t), symtab_pair_cmp_keys);
    }
    else
#else
    {
        qsort ((void*)them1, cnt, sizeof (symtab_pair_t), symtab_pair_cmp_keys);
        qsort ((void*)them2, cnt, sizeof (symtab_pair_t), symtab_pair_cmp_keys);
    }
#endif

    for (i=0; i < cnt; i++) {
        result= symtab_pair_cmp_keys (them1 + i, them2 + i);
        if (result != 0)
            break;

        result= symtab_pair_cmp_values (them1 + i, them2 + i);
        if (result != 0)
            break;
    }

    symtab_delete_entries (them1);
    symtab_delete_entries (them2);
    ...
    error_return0_if_reached_ppt (
        self,
        "Because either CHAR_CONST_P_CMP or INT_CMP was not #defined, symtab_cmp could not be implemented.");
#endif
}
...
/* must be NULL safe per definition! */
{
    ...
    E_SET_PTR (them1, symtab_get_keys (self));
    if (ERWIN_UNLIKELY(e_errno != E_OK))
        return 0;

    E_SET_PTR (them2, symtab_get_keys (other));
    if (ERWIN_UNLIKELY(e_errno != E_OK)) {
        symtab_delete_keys (them1);
        return 0;
    }

    qsort ((void*)them1, cnt, sizeof (char const *), symtab_wrap_icmp);
    qsort ((void*)them2, cnt, sizeof (char const *), symtab_wrap_icmp);
    ...
    error_return0_if_reached_ppt (
        self,
        "Because either CHAR_CONST_P_CMP was not #defined, symtab_cmp_keys could not be implemented.");
#endif
}
...
/* must be NULL-safe */
{
    ...
    error_return0_if_reached_ppt (
        self,
        "Because INT_HASH was not #defined, symtab_hash_raw could not be implemented.");
#endif
}
...
CODE_SECTION(".text.t")
 symtab_t::symtab_t
        (void)
{
    ERWIN_BOOL ok= symtab_init_internal_q (it(), OZERO, SYMTAB_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}

CODE_SECTION(".text._constructor")
void symtab_t::_constructor
        (void)
{
    ERWIN_BOOL ok= symtab_init_internal_q (it(), OZERO, SYMTAB_INITIAL_SIZE);
    (void)ok; /*unused*/
...
symtab_t const & symtab_t::static_zero
        ()
{
    static symtab_t const *z= NULL;
    if (ERWIN_UNLIKELY(!z))
        E_SET_PTR (z, new symtab_t);
    return *z;
}
...
CODE_SECTION(".text.t")
 symtab_t::symtab_t
        (int size)
{
    ERWIN_BOOL ok= symtab_init_internal_q (it(), OZERO, size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* SYMTAB_HAVE_INT_CONSTRUCTOR */
...
CODE_SECTION(".text.t")
 symtab_t::symtab_t
        (int  zeroval)
{
    ERWIN_BOOL ok= symtab_init_internal_q (it(), zeroval, SYMTAB_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
...
{
    ERWIN_BOOL ok= symtab_init_internal_q (it(), zeroval, size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* SYMTAB_DYN_ZERO */
...
CODE_SECTION(".text.t")
 symtab_t::symtab_t
        (symtab_t const &other)
{
#ifdef ERWIN_DOS
    if (other.it()->m_table == NULL && other.it()->m_nentries == 0) {
        /* See copy constructor for vectors for explanation. */
        symtab_init_internal_q (it(), OZERO, SYMTAB_INITIAL_SIZE);
    }
    else
#endif
    {
        symtab_init_internal_q (it(), ZERO(other.it()), other.it()->m_nentries);
        symtab_struct_copy (it(), other.it());
    }
}
...
CODE_SECTION(".text.t")
 symtab_t::symtab_t
        (symtab_t const *other)
{
    symtab_init_internal_q (it(), ZERO(other), other->m_nentries);
    symtab_struct_copy (it(), other);
}
...
CODE_SECTION(".text.symtab_t")
 symtab_t::~symtab_t
        ()
{
    symtab_destroy_internal (it());
}


CODE_SECTION(".text._destructor")
void symtab_t::_destructor
        ()
{
    symtab_destroy_internal (it());
}
...
symtab_t & symtab_t::operator=
        (symtab_t const *other)
{
    symtab_clear_no_resize (it());
    symtab_expect_size (it(), other->it()->m_nentries); /* realloc to good size */
    symtab_struct_copy (it(), other->it());
    return *this;
}
...
double symtab_average_line_length
        (symtab_t const *self)
{
    double sum = 0.0;
    int i;
    for (i=0; i < self->m_hashsize; i++)
        sum+= symtab_hashlist_length (SLOT(self,i));
    return sum / (double)(self->m_hashsize);
}

CODE_SECTION(".text.variance_line_length")
double symtab_variance_line_length
        (symtab_t const *self)
{
    double average= symtab_average_line_length (self);
    double sum2= 0.0;
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        double diff= symtab_hashlist_length (SLOT(self,i)) - average;
        sum2+= diff*diff;
    }
...
int symtab_max_line_length
        (symtab_t const *self)
{
    int i;
    int max = 0;
    for (i=0; i < self->m_hashsize; i++) {
        int len= symtab_hashlist_length (SLOT(self,i));
        if (len > max) max= len;
    }
    return max;
}
...
double symtab_deviation_line_length
        (symtab_t const *self)
{
    return sqrt (symtab_variance_line_length (self));
}
...
void symtab_dump
        (FILE *thefile, symtab_t const *self)
{
    int i;
    fprintf (thefile, "(symtab_dump):\n");
