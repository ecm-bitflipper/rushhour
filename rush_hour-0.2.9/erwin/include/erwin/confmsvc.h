...
#define HAVE_BROKEN_OPERATOR_OVERLOADING 1
   /* FIXME: Make this more finegrained.  Possibly there exist compilers
    *        that do better than the one I tested. */

#if defined(_M_X64) || defined(_M_IA64) || defined(_WIN64)
/* 64bit environment */

#  error "64 bit builds are currently not supported for MSVC."
...
#  define SIZEOF_LONG_LONG       0
     /* no 'long long', but we have a very long type: __int64 */

#  define ERWIN_SIZEOF_LONG_LONG 8
#  define ERWIN_LONG_LONG        __int64
...
#  define SIZEOF_WCHAR_T	     2
     /* MSVC only has 16 bit wchar_t, so it is not capable of
      * supporting UCS32 */
 ...
 *
 *   Seemingly, you need some unknown library for this.  This only
 *   confuses the users.  Windows is obsolete anyway, so we assume
 *   it has no wcrtomb()...
 */
...
#define ERWIN_THREAD_KEYWORD __declspec(thread)
