...
#  define HV(X,Y) ERR_HASHVAL_C(ERWIN_CONCAT(0x,ERWIN_CONCAT(X,Y)))
#else
#  define HV(X,Y) ERR_HASHVAL_C(ERWIN_CONCAT(0x,Y))
#endif
...
err_erwin_random_table_t err_erwin_random_table= {
...
err_erwin_random_table_t err_erwin_random_table2= {
...
/* good for x + table[(x >> 0) & 0xff] */
err_erwin_random_table_t err_erwin_random_table3= {
...
err_erwin_random_table_t err_erwin_random_table4= {
...
#endif /* ERR_ERWIN_HASH_STRENGTH >= 3 */
#endif /* ERR_ERWIN_HASH_STRENGTH >= 1 */

ERR_ERWIN_BOOL err_erwin_mapinitialised= ERR_ERWIN_FALSE;

#if !defined(ERR_ERWIN_THREAD_SAFE) || ERR_ERWIN_USE_THREAD_KEYWORD

ERR_ERWIN_THREAD_LOCAL
int err_erwininternalmaperrno= ERR_ERWININTERNALMAPOK;
...
int *err_erwininternalmaperrnoptr(void)
{
    int *errno_ptr;
    ...
    if (ERWIN_UNLIKELY(!errno_ptr)) {
        ...
        *errno_ptr= ERR_ERWININTERNALMAPOK;
...
char const *err_erwininternalmapstrerror (int i)
{
    switch (i) {
        case ERR_ERWININTERNALMAPOK:
            return "No error";

        case ERR_ERWININTERNALMAPERRNOMEM:
            return "Error: Memory exhausted";

        case ERR_ERWININTERNALMAPERRASSERTIONFAILED:
            return "Error: Assertion failed";

        case ERR_ERWININTERNALMAPWARNEXISTINGKEY:
            return "Warning: Key already exists";

        case ERR_ERWININTERNALMAPWARNKEYNOTFOUND:
            return "Warning: Key not found";

        case ERR_ERWININTERNALMAPWARNEMPTY:
            return "Warning: Map is empty";

        case ERR_ERWININTERNALMAPWARNNOMOREELEMS:
            return "Warning: No more elements";

        case ERR_ERWININTERNALMAPREHASHNOMEM:
            return "Warning: Rehash ran out of memory";

        case ERR_ERWININTERNALMAPREHASHDUPLICATEKEY:
            return "Warning: Rehash found duplicate key";

        case ERR_ERWININTERNALMAPREHASHRECURSION:
            return "Warning: Rehash went into recursion";
    }
    return "Error: Unknown error code";
}

void err_erwin_init_map (void)
{

    if (ERWIN_UNLIKELY(!err_erwin_mapinitialised))
    {
        ...
        int i;
#if ERR_ERWIN_HASH_STRENGTH >= 1
        err_hashval_t a1= ERR_ERWIN_RANDOM_FUNC() & ~((err_hashval_t)0x00ff);
        err_hashval_t a2= ERR_ERWIN_RANDOM_FUNC() & ~((err_hashval_t)0xff00);
#if ERR_ERWIN_HASH_STRENGTH >= 3
        err_hashval_t a3= ERR_ERWIN_RANDOM_FUNC() & ~((err_hashval_t)0x00ff);
        err_hashval_t a4= ERR_ERWIN_RANDOM_FUNC() & ~((err_hashval_t)0xff00);
#endif /* ERR_ERWIN_HASH_STRENGTH >= 3 */
        ...
        for (i= 0; i < 256; i++) {
            err_erwin_random_table [i]^= a1;
            err_erwin_random_table2[i]^= a2;
#if ERR_ERWIN_HASH_STRENGTH >= 3
            err_erwin_random_table3[i]^= a3;
            err_erwin_random_table4[i]^= a4;
#endif
        }
#endif /* ERR_ERWIN_HASH_STRENGTH >= 1 */
#endif /* ERR_ERWIN_REQUIRE_DETERMINISM */
        err_erwin_mapinitialised= ERR_ERWIN_TRUE;
    }
}
...
#define HASH1U8(state, hash,c,e)                                      \
        do{                                                           \
            if (c >= e) return ERR_ERWIN_STATE_GET_HASHVAL(state); \
            ERR_ERWIN_STATE_MIX_ORDERED(state,hash(*(c)));         \
            (c)++;                                                    \
        }while(0);
...
#define HASH1U16(state, c,e)                                                   \
        do{                                                                    \
            if (c >= e) return ERR_ERWIN_STATE_GET_HASHVAL(state);          \
            ERR_ERWIN_STATE_MIX_ORDERED(state,err_erwin_u16_hash(*(c))); \
            (c)++;                                                             \
        }while(0);

err_hashval_t err_erwin_u8_array_hash (ERWIN_U8 const *c, size_t l)
/* distinguishes NULL and "" */
{
    ...
    ERR_ERWIN_STATE_INIT_WITH (state, err_size_t_hash(l));
    e= c+l;
    ...
    if ((((unsigned long)c) & 3) == 0) {
        for (; c + 4 <= e; c+= 4)
            ERR_ERWIN_STATE_MIX_ORDERED(
                state,
                err_erwin_u32_hash(
                    err_erwin_int2lbf32(*((ERWIN_U32*)c))   /* make it low byte first */
                )
            );
    }
    else
    ...
    HASH1U8(state, err_char_hash2, c, e);
    HASH1U8(state, err_char_hash3, c, e);
    HASH1U8(state, err_char_hash4, c, e);

    return ERR_ERWIN_STATE_GET_HASHVAL(state);
}


err_hashval_t err_erwin_u8_array_case_hash (ERWIN_U8 const *c, size_t l)
{
    ERWIN_U8 const *e;
    err_erwin_hash_state_t state;
    ...
    for (; c + 4 < e; c+= 4)
        ERR_ERWIN_STATE_MIX_ORDERED(
            ...
            err_erwin_u32_hash(
                make_uint32(
                    err_char_to_lower(c[0]),
                    err_char_to_lower(c[1]),
                    err_char_to_lower(c[2]),
                    err_char_to_lower(c[3]))));

    HASH1U8(state, err_char_case_hash2, c, e);
    HASH1U8(state, err_char_case_hash3, c, e);
    HASH1U8(state, err_char_case_hash4, c, e);

    return ERR_ERWIN_STATE_GET_HASHVAL(state);
}

err_hashval_t err_erwin_u16_array_hash (ERWIN_U16 const *c, size_t l)
{
    ERWIN_U16 const *e;
    err_erwin_hash_state_t state;
...
err_hashval_t err_erwin_u32_array_hash (ERWIN_U32 const *c, size_t l)
{
    ERWIN_U32 const *e;
    err_erwin_hash_state_t state;
    ...
    for (; c < e; c++)
        ERR_ERWIN_STATE_MIX_ORDERED(state, err_erwin_u32_hash(*c));
...
err_hashval_t err_erwin_u64_array_hash (ERWIN_U64 const *c, size_t l)
{
    ERWIN_U64 const *e;
    err_erwin_hash_state_t state;
    ...
    for (; c < e; c++)
        ERR_ERWIN_STATE_MIX_ORDERED(state, err_erwin_u64_hash(*c));
