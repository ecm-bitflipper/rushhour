...
#warning "Including err_symtab.h"
#endif

#ifndef ERWIN_ERR_SYMTAB_h
#define ERWIN_ERR_SYMTAB_h

#ifdef ERWIN_DEBUG_INCLUDE
#warning "First inclusion of err_symtab.h"
#endif
...
typedef char const * err_symtab_index_t;
typedef void err_symtab_value_t;

typedef struct _err_symtab_pair_t {
    char const * key;
} err_symtab_pair_t;

typedef err_symtab_pair_t err_symtab_pair_dynarray;


typedef struct _err_symtab_pair_ptr_t {
    char const * key;
} err_symtab_pair_ptr_t;


typedef err_symtab_pair_ptr_t err_symtab_pair_ptr_dynarray;

typedef int (*err_symtab_pair_cmp_t)(err_symtab_pair_t const *, err_symtab_pair_t const *);

typedef int (*err_symtab_pair_ptr_cmp_t)(err_symtab_pair_ptr_t const *, err_symtab_pair_ptr_t const *);

typedef int (*err_symtab_void_pair_cmp_t)(void const *, void const *);



typedef ERR_ERWIN_BOOL (*err_symtab_feature_t)(char const * );



typedef char const * err_symtab_key_result_t;
...
typedef struct _err_map_iterator_t {
     ...
     * manually.  err_symtab iteration looks like this:
     *     : err_map_iterator_t iter;
     *     : ...
     *     : err_symtab_forall (map, iter, key, value) {
     *     :     ...
     *     : }
     *
...
/*--BEGIN-C--*/
ERR_ERWIN_EXPORT
err_symtab_t *err_symtab_new (void) ATTR_MALLOC;
  /* Return a new map with default settings.  I.e., with the
   * default zero element ERR_SYMTAB_ZERO and the default initial hash table
   * size of ERR_SYMTAB_INITIAL_SIZE.
   *
   * \errno(OK, ERR_NOMEM)
   */

ERR_ERWIN_EXPORT
err_symtab_t *err_symtab_new_with_initial_size (int /*initial_size*/) ATTR_MALLOC;
   ...
   *
   * If initial_size < 0 the default size ERR_SYMTAB_INITIAL_SIZE will be used.
   * If initial_size == 0 but ERR_SYMTAB_MINIMAL_SIZE > 0, a size of 1 will be
   * used.
   *
   ...
   * ERR_SYMTAB_TRIGGER_NUMERATOR * 80 / ERR_SYMTAB_TRIGGER_DENOMINATOR.
   *
   * \errno(OK, ERR_NOMEM)
   */
...
ERR_ERWIN_EXPORT
err_symtab_t *err_symtab_new_with_zero (void  /*zero*/) ATTR_MALLOC;
  /* zero will eventually be tried to be copied!  So if you provide
   * VOID_OCOPY, make sure it handles the zero element appropriately.
   *
   * \errno(OK, ERR_NOMEM)
   */

ERR_ERWIN_EXPORT
err_symtab_t *err_symtab_new_with_zero_and_initial_size (void  /*zero*/, int /*initial_size*/) ATTR_MALLOC;
  /* Create a new, empty map.  The first form uses
   * err_void_zero as the zero element.
   *
   * If initial_size < 0 the default size ERR_SYMTAB_INITIAL_SIZE will be used.
   * If initial_size == 0 but ERR_SYMTAB_MINIMAL_SIZE > 0, a size of 1 will be
   * used.
...
ERR_ERWIN_EXPORT
int err_symtab_init (
    err_symtab_t * /*self*/) ATTR_NONNULL((1));
  /*
   * This is for initialising non-heap err_symtab_t objects.
   *
...
int err_symtab_init_with_zero_and_initial_size (
  ...
  /*
   * This is for initialising non-heap err_symtab_t objects that are allocated
   * manually (thus, not via err_symtab_new).
   *
   * Invoking this function manually is usually discouraged.  It is thought
   * to be for nesting data structures.  However, if you need to optimise
   * heap usage, it is ok to use this function.
...
ERR_ERWIN_EXPORT
void err_symtab_destroy (err_symtab_t * /*self*/);
  /*
   * This is for deleting non-heap err_symtab_t objects.
   *
   * Invoking this function manually is usually discouraged.  It is thought
   * to be for nesting data structures.  However, if you need to optimise
   * heap usage, it is ok to use this function.
...
ERR_ERWIN_EXPORT
void err_symtab_xchg (err_symtab_t * /*self*/, err_symtab_t * /*other*/);
  /*
...
void err_symtab_destroy_flags (
  ...
  /*
   * Same as err_symtab_destroy with the possiblity to determine what to destroy.
   *
   * Invoking this function manually is usually discouraged.  It is thought
   * to be for nesting data structures.  However, if you need to optimise
   * heap usage, it is ok to use this function.
...
#if !ERR_ERWIN_GLOBAL_ERRNO
ERR_ERWIN_EXPORT
int err_symtab_errno(err_symtab_t const *) ATTR_PURE ATTR_NONNULL((1));
...
ERR_ERWIN_EXPORT
void err_symtab_clear_errno(err_symtab_t const *) ATTR_NONNULL((1));
...
#define err_symtab_errno(X)          err_map_errno
  /* If the library was not compiled with ERR_ERWIN_THREAD_SAFE, err_symtab_errno(X)
...
#define err_symtab_clear_errno(X)    ((void)(err_map_errno= ERR_MAP_OK))
  /* If the library was not compiled with ERR_ERWIN_THREAD_SAFE, err_symtab_clear_errno(X)
...
ERR_ERWIN_EXPORT
err_symtab_t *err_symtab_copy (err_symtab_t const* /*self*/);
...
ERR_ERWIN_EXPORT
err_symtab_t *err_symtab_copy_err (err_symtab_t const* /*self*/, int *err);
...
ERR_ERWIN_EXPORT
void err_symtab_delete (err_symtab_t* /*self*/);
...
ERR_ERWIN_EXPORT
void   err_symtab_delete_flags (err_symtab_t* /* self*/, ERR_ERWIN_BOOL /* destroy_keys */, ERR_ERWIN_BOOL /* destroy_values */);
  /* Deletes only the map structure, not the entries.  This is
   * useful if you got the entries via err_symtab_get_{values|keys}.
   * err_symtab_delete_flat (a, true, true) is the same as
   * err_symtab_delete (a);
   *
   * \errno(OK)
   */
 ...
 * easily possible. */
ERR_ERWIN_EXPORT
int err_symtab_insert (err_symtab_t* /*self*/, char const *  /*key*/) ATTR_NONNULL((1));
...
ERR_ERWIN_EXPORT
int err_symtab_insert_map (err_symtab_t* /*self*/, err_symtab_t const * /* other */);
...
ERR_ERWIN_EXPORT
char const *  err_symtab_ensure (err_symtab_t* /*self*/, char const *  /*key*/) ATTR_NONNULL((1));
  /* This ensures that key is inserted in the table, i.e., it allocates a cell
   * for it.  The inserted value will be ERR_VOID_ZERO.  If you
   * want to change this, re-define ERR_SYMTAB_ENSURE_VALUE.
   * The result is the key as inserted in the hash table, so if you defined
   * ERR_CHAR_CONST_P_ICOPY, you will get a copy of your key.
   *
...
ERR_ERWIN_EXPORT
char const *  err_symtab_ensure_no_icopy (err_symtab_t * /*self*/, char *  /*key*/) ATTR_NONNULL((1));
  /* This is like err_symtab_ensure(), but the input key will be stolen or freed, i.e.,
   * it will be used as a copy inside the hash table, instead of being copied
   * as in err_symtab_ensure.  If the key already exists, the key passed will be freed.
   *
   * Note: This function implements symbol tables.
   *
   * This could be called err_symtab_ensure_steal(), but we already have poke_no_icopy().
   *
   * \errno(OK, WARN_EXISTINGKEY, REHASH_*, ERR_NOMEM)
   */
...
ERR_ERWIN_EXPORT
int err_symtab_find_any_pair (err_symtab_key_result_t *, err_symtab_t const* /*self*/)
  ATTR_ERRNO_PURE;
   ...
   * the return value.
   *
   * See err_symtab_find_any() for more documentation.
   *
...
ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL  err_symtab_find (err_symtab_t const* /*self*/, char const *  /*index*/)
  ATTR_ERRNO_PURE;
...
ERR_ERWIN_EXPORT
char const *   err_symtab_find_key (err_symtab_t const* /*self*/, char const *  /*index*/) ATTR_ERRNO_PURE;
...
ERR_ERWIN_EXPORT
char const *   err_symtab_find_any_key (err_symtab_t const* /*self*/) ATTR_ERRNO_PURE;
  /* Returns some key that is stored in the table.
   *
   * See err_symtab_find_any() for docu.
   *
   * \errno(OK, WARN_KEYNOTFOUND)
   */
...
ERR_ERWIN_EXPORT
int err_symtab_intersect (err_symtab_t* /*self*/, err_symtab_t const * /* other */);
  /* Implements set intersection: this invokes err_symtab_erase on all entries
   * in self that are not in other.
   * Returns the number of erased elements (quite like err_symtab_erase_if).
   *
   * \errno(OK, REHASH_*, ERR_NOMEM)
   */

ERR_ERWIN_EXPORT
int err_symtab_intersect_no_resize (err_symtab_t* /*self*/, err_symtab_t const * /* other */);
  /* Like err_symtab_intersect, but does not shrink the internal hash table.
   *
   * \errno(OK, REHASH_*, ERR_NOMEM)
   */
...
ERR_ERWIN_EXPORT
int err_symtab_erase (err_symtab_t* /*self*/, char const *  /*index*/) ATTR_NONNULL((1));
  /* Same as err_symtab_remove, but value is freed.  In error-free operation, returns
...
ERR_ERWIN_EXPORT
int err_symtab_erase_no_resize (err_symtab_t* /*self*/, char const *  /*index*/) ATTR_NONNULL((1));
  /* Same as err_symtab_erase, but does not shrink the internal hash table.
   *
   * \errno(OK, WARN_KEYNOTFOUND)
   */

ERR_ERWIN_EXPORT
int err_symtab_erase_map (err_symtab_t* /*self*/, err_symtab_t const * /* other */);
  /* like err_symtab_erase on all entries in other.  If any key was not found,
...
ERR_ERWIN_EXPORT
int err_symtab_erase_map_no_resize (err_symtab_t* /*self*/, err_symtab_t const * /* other */);
  /* Like err_symtab_erase_map, but does not shrink the internal hash table.
   *
   * \errno(OK, WARN_KEYNOTFOUND)
   */

ERR_ERWIN_EXPORT
int err_symtab_erase_if (err_symtab_t * /*self */, err_symtab_feature_t /* feature */, ERR_ERWIN_BOOL /* value */);
...
int err_symtab_poke (
        ...
        err_symtab_t *      /* self */,
        char const *  /* key */,
        ERR_ERWIN_BOOL       /* introduce */) ATTR_NONNULL((2));
  /* This is the generalised interface to err_symtab_insert, err_symtab_modify, err_symtab_set, err_symtab_ensure, err_symtab_find
   * and err_symtab_find_key.  This function cannot erase, however (see err_symtab_erase).
   *
   * Input parameters:
   *   +--------------+----------------------------------------------------------------------+
   ...
   *
   * err_symtab_insert:
   *       : return err_symtab_poke (NULL, NULL, self, key, value,
   ...
   * err_symtab_modify:
   *       : void *result;
   *       : err_symtab_poke (NULL, &result, self, key, value,
   *       :           ERR_ERWIN_FALSE, ERR_ERWIN_TRUE);
   *       : return result;
   *
   * err_symtab_find:
   *       : void *result;
   *       : err_symtab_poke (NULL, &result, self, key, ZERO (void),
   *       :           ERR_ERWIN_FALSE, ERR_ERWIN_FALSE);
   *       : return (void*)result;
   *
   * err_symtab_find_key:
   *       : char * *result;
   *       : err_symtab_poke (&result, NULL, self, key, ZERO (void),
   *       :           ERR_ERWIN_FALSE, ERR_ERWIN_FALSE);
   ...
   *       :           ERR_ERWIN_TRUE, ERR_ERWIN_FALSE);
   *       : result;
   * (Only err_symtab_ensure uses ERR_SYMTAB_ENSURE_VALUE to introduce a new element, not
   * ZERO and without copying)
   *
   * Note that err_symtab_find_ptr, err_symtab_remove and err_symtab_erase cannot be implemented with
   * this function.
   *
   * Return Values:
   *    - ERR_MAP_OK:
...
ERR_ERWIN_EXPORT
int err_symtab_poke_no_icopy (
   char const * * /*key_out*/,
   err_symtab_t * /*self*/,
   char *  /*key*/,
   ERR_ERWIN_BOOL /*introduce*/) ATTR_NONNULL((2));
  /* Like err_symtab_poke, but the key is not copied.  However, \p key will be freed if it is not
...
ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_zero (err_symtab_t const* /*self*/) ATTR_PURE ATTR_NONNULL((1));
  /* Returns the zero element of void
   *
   * \noerrno
   */

ERR_ERWIN_EXPORT
int err_symtab_cmp (err_symtab_t const* /*self*/, err_symtab_t const * /*other */) ATTR_ERRNO_PURE;
   ...
   *
   * For equality tests, use err_symtab_equal instead, which is faster.
   *
   ...
   *    ERR_CHAR_CONST_P_CMP or ERR_VOID_CMP is not defined.
   *
   * \errno(OK, ERR_NOMEM)
   */

ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_equal (err_symtab_t const* /*self*/, err_symtab_t const * /*other */) ATTR_ERRNO_PURE;
   ...
   * ERR_VOID_EQUAL is not defined.
   *
   * \errno(OK)
   */

ERR_ERWIN_EXPORT
int err_symtab_cmp_keys (err_symtab_t const* /*self*/, err_symtab_t const * /*other */) ATTR_ERRNO_PURE;
   ...
   *
   * Further Note: Calling this results in a runtime error if either
   *    ERR_CHAR_CONST_P_CMP is not defined.
   *
   * \errno(OK, ERR_NOMEM)
   */

ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_equal_keys (err_symtab_t const* /*self*/, err_symtab_t const * /*other */) ATTR_ERRNO_PURE;
  /* Returns whether the set of keys in the map is equal.
   *
...
ERR_ERWIN_EXPORT
err_hashval_t err_symtab_hash_raw (err_symtab_t const * /*self*/) ATTR_ERRNO_PURE;
  /* Calculates a hash value for this hash table.  In earlier versions of Erwin,
   * the result was inferior to that of err_symtab_hash, but they are now the same
   * and no additional hashing of the result needs to be done.
   *
   * Note: Calling this results in a runtime error if ERR_VOID_HASH is not defined.
   *
   * \errno(OK)
   */

ERWIN_WRAPPER err_hashval_t err_symtab_hash (err_symtab_t const *) ATTR_ERRNO_PURE;
    /* Same as err_symtab_hash_raw() now that hash_raw() is good enough. */

ERWIN_WRAPPER err_hashval_t err_symtab_hash (err_symtab_t const *x)
{
    return err_symtab_hash_raw(x);
}


/* ***** access to all entries: ***** */
ERR_ERWIN_EXPORT
void err_symtab_clear (err_symtab_t* /*self*/);
   /* Like err_symtab_erase for all elements
    *
    * \errno(OK)
    */

ERR_ERWIN_EXPORT
void err_symtab_clear_no_resize (err_symtab_t* /*self*/);
   /* Like err_symtab_erase for all elements, but does not shrink the internal hash table.
    *
    * \errno(OK)
    */
...
void err_symtab_clear_flags_no_resize (
    ...
    ERR_ERWIN_BOOL     /*destroy_keys*/,
    ERR_ERWIN_BOOL     /*destroy_values*/);
   /* Like err_symtab_clear_flags, but does not shrink the internal hash table.
    *
    * \errno(OK)
    */

ERR_ERWIN_EXPORT
int err_symtab_nentries (err_symtab_t const* /*self*/) ATTR_PURE;
...
ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_empty (err_symtab_t const* /*self*/) ATTR_PURE;
...
ERR_ERWIN_EXPORT
err_symtab_pair_t *err_symtab_get_entries (err_symtab_t const* /*self*/) ATTR_MALLOC;
   /* Use err_symtab_delete_entries to deallocate the returned array.
    *
    * \errno(OK, ERR_NOMEM)
    */

ERR_ERWIN_EXPORT
void err_symtab_delete_entries (err_symtab_pair_t * /*entries*/);
   /* Deallocate keys obtained by err_symtab_get_entries(). */



ERR_ERWIN_EXPORT
char const * *err_symtab_get_keys   (err_symtab_t const* /*self*/) ATTR_MALLOC;
    ...
    *       freed by the user using err_symtab_delete_keys().  In earlier versions
    *       of Erwin, free() was recommended for freeing, but due to more
    *       sophisticated memory management, err_symtab_delete_keys() is the
...
ERR_ERWIN_EXPORT
void err_symtab_delete_keys (char const * * /*keys*/);
   /* Deallocate keys obtained by err_symtab_get_keys(). */


/* ***** debugging and special purposes (use rarely!): ***** */
ERR_ERWIN_EXPORT
int err_symtab_hash_size (err_symtab_t const* /*self*/) ATTR_PURE;
...
ERR_ERWIN_EXPORT
void err_symtab_rehash (err_symtab_t* /*self*/, int /*newsize*/);
...
ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_expect_size (err_symtab_t* /*self*/, int /*newsize*/);
...
ERR_ERWIN_EXPORT
double err_symtab_average_line_length (err_symtab_t const* /*self*/) ATTR_PURE;

ERR_ERWIN_EXPORT
double err_symtab_variance_line_length (err_symtab_t const* /*self*/) ATTR_PURE;

#ifdef HAVE_SQRT
ERR_ERWIN_EXPORT
double err_symtab_deviation_line_length (err_symtab_t const* /*self*/) ATTR_PURE;
#endif

ERR_ERWIN_EXPORT
int err_symtab_max_line_length (err_symtab_t const* /*self*/) ATTR_PURE;

ERR_ERWIN_EXPORT
int err_symtab_min_line_length (err_symtab_t const* /*self*/) ATTR_PURE;

ERR_ERWIN_EXPORT
void err_symtab_dump (FILE *, err_symtab_t const * /*self*/);
  /* Dumps the internal structure of the map to the given file. */

/* ***** iteration: ***** */
/* New approach: */
#define err_symtab_forall_nondet(h,i,k,v) \
        for(err_symtab_init_iterator ((h),&i); \
            err_symtab_next_iteration ((h),&i,&k,&v);)
 ...
 * using this.  See err_symtab_forall for details.
 */

#define err_symtab_forall_ptr_nondet(h,i,k,v) \
        for(err_symtab_init_iterator ((h),&i); \
            err_symtab_next_iteration_ptr ((h),&i,&k,&v);)
/* Operator for non-deterministic hash iteration.  This is non-deterministic even if
 * ERR_ERWIN_REQUIRE_DETERMINISM is #defined.
 *
 * This iterates over keys and pointers to values.  You need a map_iterator_t as a
 * temporary for using this.  See err_symtab_forall for details.  Iterating over pointers
 * allows you to change entries in the table on the fly.
 */

#define err_symtab_forall_values_nondet(h,i,v) \
        for(err_symtab_init_iterator ((h),&i); \
            err_symtab_next_iteration_values ((h),&i,&v);)
/* Operator for non-deterministic hash iteration.  This is non-deterministic even if
 * ERR_ERWIN_REQUIRE_DETERMINISM is #defined.
 *
...
#define err_symtab_forall_values_ptr_nondet(h,i,v) \
        for(err_symtab_init_iterator ((h),&i); \
            err_symtab_next_iteration_values_ptr ((h),&i,&v);)
/* Operator for non-deterministic hash iteration.  This is non-deterministic even if
 * ERR_ERWIN_REQUIRE_DETERMINISM is #defined.
 *
 * This iterates over values only.  You need a map_iterator_t as a temporary for
 * using this.  See err_symtab_forall for details.  Iterating over pointers
 * allows you to change entries in the table on the fly.
 */

#define err_symtab_forall_keys_nondet(h,i,k) \
        for(err_symtab_init_iterator ((h),&i); \
            err_symtab_next_iteration_keys ((h),&i,&k);)
/* Operator for non-deterministic hash iteration.  This is non-deterministic even if
 * ERR_ERWIN_REQUIRE_DETERMINISM is #defined.
 *
...
#define err_symtab_forall_pairs_nondet(h,i,p) \
        for(err_symtab_init_iterator ((h),&i); \
            err_symtab_next_iteration_pairs ((h),&i,&p);)
/* Operator for non-deterministic hash iteration.  This is non-deterministic even if
 * ERR_ERWIN_REQUIRE_DETERMINISM is #defined.
 *
...
#define err_symtab_forall_pairs_ptr_nondet(h,i,p) \
        for(err_symtab_init_iterator ((h),&i); \
            err_symtab_next_iteration_pairs_ptr ((h),&i,&p);)
/* Operator for non-deterministic hash iteration.  This is non-deterministic even if
 * ERR_ERWIN_REQUIRE_DETERMINISM is #defined.
 *
 * This iterates over key/value pairs.  You need a map_iterator_t as a temporary for
...
#  define err_symtab_forall(h,i,k,v) err_symtab_forall_nondet(h,i,k,v)
...
#  define err_symtab_forall_ptr(h,i,k,v) err_symtab_forall_ptr_nondet(h,i,k,v)
...
#  define err_symtab_forall_values(h,i,v) err_symtab_forall_values_nondet(h,i,v)
/* Works similar to map_forall.
 *
 * This iterates over values only.  You need a map_iterator_t as a temporary for
 * using this.  See err_symtab_forall for details.
...
#  define err_symtab_forall_values_ptr(h,i,v) err_symtab_forall_values_ptr_nondet(h,i,v)
/* Works similar to map_forall.
 *
 * This iterates over values only.  You need a map_iterator_t as a temporary for
 * using this.  See err_symtab_forall for details.  Iterating over pointers
...
#  define err_symtab_forall_keys(h,i,k)   err_symtab_forall_keys_nondet(h,i,k)
/* Works similar to map_forall.
 *
 * This iterates over keys only.  You need a map_iterator_t as a temporary for
 * using this.  See err_symtab_forall for details.
...
#  define err_symtab_forall_pairs(h,i,p)  err_symtab_forall_pairs_nondet(h,i,p)
/* Works similar to map_forall.
 *
 * This iterates over key/value pairs.  You need a map_iterator_t as a temporary for
 * using this.  See err_symtab_forall for details.
...
#  define err_symtab_forall_pairs_ptr(h,i,p)  err_symtab_forall_pairs_ptr_nondet(h,i,p)
/* Works similar to map_forall.
 *
 * This iterates over key/value pairs.  You need a map_iterator_t as a temporary for
 * using this.  See err_symtab_forall for details.  Iterating over pointers
...
ERR_ERWIN_EXPORT
void err_symtab_init_iterator (err_symtab_t const *, err_map_iterator_t *);
/*private*/

#ifdef __cplusplus
ERR_ERWIN_EXPORT
void err_symtab_init_iterator_sorted_by_key (err_symtab_t const *, err_map_iterator_sorted_t *);
/*private*/


ERR_ERWIN_EXPORT
void err_symtab_init_iterator_sorted_by_user (err_symtab_t const *, err_map_iterator_sorted_t *,
    err_symtab_pair_cmp_t);
/*private*/


ERR_ERWIN_EXPORT
void err_symtab_init_iterator_sorted_by_key_reverse (err_symtab_t const *, err_map_iterator_sorted_t *);
/*private*/


ERR_ERWIN_EXPORT
void err_symtab_init_iterator_sorted_by_user_reverse (
    err_symtab_t const *, err_map_iterator_sorted_t *, err_symtab_pair_cmp_t);
/*private*/


ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_keys (err_symtab_t const *,
    err_map_iterator_sorted_t *, char const * *);
/*private*/


ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_pairs (err_symtab_t const *,
    err_map_iterator_sorted_t *, err_symtab_pair_t *);
/*private*/



ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_keys_reverse (err_symtab_t const *,
    err_map_iterator_sorted_t *, char const * *);
/*private*/


ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_pairs_reverse (err_symtab_t const *,
    err_map_iterator_sorted_t *, err_symtab_pair_t *);
/*private*/
...
ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_next_iteration_keys  (err_symtab_t const* /*self*/,
    err_map_iterator_t *, char const * * /* key */);
/*private*/

ERR_ERWIN_EXPORT
ERR_ERWIN_BOOL err_symtab_next_iteration_pairs (err_symtab_t const* /*self*/,
    err_map_iterator_t *, err_symtab_pair_t * /*pair*/);
/*private*/
...
ERR_ERWIN_EXPORT
int err_symtab_nrehash_ops (err_symtab_t const* /*self*/) ATTR_PURE;

ERR_ERWIN_EXPORT
int err_symtab_nrehash (err_symtab_t const* /*self*/) ATTR_PURE;
  /* rehash_ops is the number of pure rehash operations and rehash
   * is the number of elements moved around during rehashes */

ERR_ERWIN_EXPORT
int err_symtab_ninsert (err_symtab_t const* /*self*/) ATTR_PURE;

ERR_ERWIN_EXPORT
int err_symtab_nremove (err_symtab_t const* /*self*/) ATTR_PURE;

ERR_ERWIN_EXPORT
int err_symtab_nfind (err_symtab_t const* /*self*/) ATTR_PURE;

ERR_ERWIN_EXPORT
int err_symtab_nops (err_symtab_t const* /*self*/) ATTR_PURE;
  /* Sum of inserts, deletions, and searches */

  /* Most interesting are err_symtab_nops / err_symtab_nrehash. */
#endif
...
struct err_symtab_t
#ifdef ERR_SYMTAB_SUPER_CLASS
   : ERR_SYMTAB_SUPER_CLASS_ACCESS ERR_SYMTAB_SUPER_CLASS
#endif
{
    /* The err_symtab Class
     * =============
     ...
     *
     *   - err_symtab_:
     *       This is equivalent to err_symtab, or the name
     ...
     *       +-------------------+------------------------------+
     *       |! Template         |! Instantiation               |
     *       | err_symtab_t             | xyz_map_int_p_int_t          |
     *       | ERR_SYMTAB_ALLOW_NULL    | XYZ_MAP_INT_P_INT_ALLOW_NULL |
     *       +-------------------+------------------------------+
     *   - ErrSymtab:
     *       This is the name of the data structure type in C++: capitalised
     *       and with underbars removed:
     *       +-------------------+-----------------+
     *       |! err_symtab_t            |! ErrSymtab      |
     ...
     * I.e. NULL for pointers, 0 for integers, the user specified zero element for char const * or void,
    ...
    ERR_SYMTAB_STD_MEMBERS(err_symtab_t)
...
public:
    ...
    /*! doc-ignore */
    err_symtab_t *it()             { return this; }

    /*! doc-ignore */
    err_symtab_t const *it() const { return this; }
...
public:
    ...
    static err_symtab_t const &static_zero();

#if ERR_SYMTAB_HAVE_INT_CONSTRUCTOR
    ERR_ERWIN_EXPLICIT err_symtab_t (int initial_size);
#endif
#if ERR_SYMTAB_DYN_ZERO
    ERR_ERWIN_EXPLICIT err_symtab_t (void  zero_element);
    err_symtab_t (void  zero_element, int initial_size);
#endif

    /* status code */
#if !ERR_ERWIN_GLOBAL_ERRNO
    int get_errno(void) const        { cn(); return it()->m_errno; }
    void clear_errno(void) const     { cn(); err_symtab_clear_errno (it()); }
#else
    ...
    ERR_ERWIN_EXPLICIT err_symtab_t (err_symtab_t const *a);

    err_symtab_t (err_symtab_t const &a);

    err_symtab_t *copy (void) const
    {
        cn();
        return new err_symtab_t (it());
    }

    err_symtab_t *copy_err (int *err) const
    {
        cn();
        err_symtab_t *result= new err_symtab_t (it());
        if (err != NULL && ERR_MAP_IS_ERROR (get_errno ()))
            *err= 1;
        return result;
    }

    err_symtab_t &xchg(err_symtab_t *other)
    {
        err_symtab_xchg (it(), other);
        return *this;
    }

    err_symtab_t &xchg(err_symtab_t &other)
    {
        err_symtab_xchg (this, &other);
        return *this;
    }

    /* Assignment */

    err_symtab_t &operator= (err_symtab_t const &);
     ...
     * DO NOT CALL err_symtab_delete or err_symtab_delete_flags ON THE RETURNED VALUE!
     * All other things you do are ok. */

    operator err_symtab_t *(void)             { cn(); return it(); }
    operator err_symtab_t const *(void) const { cn(); return it(); }

    /* More operators for easy use: */

    /* ERR_ERWIN_BOOL == ERR_ERWIN_BOOL */

    ERR_ERWIN_BOOL operator[] (char const *  i) const
    {
        nocn();
        return err_symtab_find (it(), i);
    }
    ...
    ERR_ERWIN_BOOL find (char const *  i) const
        { nocn(); return err_symtab_find (it(), i); }
      

    char const *  find_any_key() const
        { nocn(); return err_symtab_find_any_key (it()); }



    /* insert */
    err_symtab_t  &insert (char const *  k)
        { cn(); err_symtab_insert (it(), k); return *this; }

    err_symtab_t  &insert_map (err_symtab_t const &other)
        { cn(&other); err_symtab_insert_map (it(), other.it()); return *this; }

    err_symtab_t  &insert_map (err_symtab_t const *other)
        { cn(other); err_symtab_insert_map (it(), other->it()); return *this; }

#if ERR_SYMTAB_DIRECT_RECURSION == 0
    err_symtab_t  &insert (err_symtab_t const &other)
        { cn(&other); err_symtab_insert_map (it(), other.it()); return *this; }
    ...
    /* erase */
    err_symtab_t  &erase (char const *  i)
        { nocn(); err_symtab_erase (it(), i); return *this; }

    err_symtab_t  &erase_map (err_symtab_t const &other)
        { nocn(&other); err_symtab_erase_map (it(), other.it()); return *this; }

    err_symtab_t  &erase_map (err_symtab_t const *other)
        { nocn(other); err_symtab_erase_map (it(), other->it()); return *this; }

#if ERR_SYMTAB_DIRECT_RECURSION == 0
    err_symtab_t  &erase (err_symtab_t const &other)
        { nocn(other); err_symtab_erase_map (it(), other.it()); return *this; }
    ...
    int erase_if (err_symtab_feature_t f, bool value = true)
    {
        nocn();
        return err_symtab_erase_if (it(), f, value);
    }

    err_symtab_t &intersect (err_symtab_t const &other)
        { nocn(&other); err_symtab_intersect (it(), other.it()); return *this; }

    err_symtab_t &intersect (err_symtab_t const *other)
        { nocn(other); err_symtab_intersect (it(), other->it()); return *this; }

    err_symtab_t &intersect_no_resize (err_symtab_t const &other)
        { nocn(&other); err_symtab_intersect_no_resize (it(), other.it()); return *this; }
    ...
    /* others */
    char const *  find_key (char const *  i) const { nocn(); return err_symtab_find_key (it(), i); }

    ERR_ERWIN_BOOL zero(void) const { nocn(); return err_symtab_zero (it()); }

    char const *  ensure (char const *  k)
        { cn(); return err_symtab_ensure (it(), k); }

    char const *  ensure_no_icopy (char *  k)
        { cn(); return err_symtab_ensure_no_icopy (it(), k); }
    ...
    err_symtab_t &poke (
        char const * *kp,
        char const *  k,
        bool aintroduce ERR_SYMTAB_DEFAULT_ARG(true))
    {
        cn();
        err_symtab_poke (kp, it(), k, aintroduce);
        return *this;
    }

    err_symtab_t &poke_no_icopy (
        char const * *kp,
        char *  k,
        bool aintroduce ERR_SYMTAB_DEFAULT_ARG(true))
    {
        cn();
        err_symtab_poke_no_icopy (kp, it(), k, aintroduce);
        return *this;
    }


    /* clear */
    err_symtab_t &clear (void)
    {
        nocn();
        err_symtab_clear (it());
        return *this;
    ...
    err_symtab_t &clear_no_resize (void)
    {
        nocn();
    ...
    err_symtab_t &clear (bool k, bool v)
    {
        nocn();
        err_symtab_clear_flags (it(), k, v);
        return *this;
    }

    int   nentries (void) const                  { nocn(); return err_symtab_nentries (it()); }
    bool  empty (void) const                     { nocn(); return ERR_ERWIN_TO_BOOL(err_symtab_empty (it())); }
    bool  non_empty (void) const                 { nocn(); return !err_symtab_empty (it()); }

    char const *          *get_keys   (void) const      { nocn(); return err_symtab_get_keys (it()); }
    static void delete_keys        (char const *          *k) { err_symtab_delete_keys (k); }


    int     hash_size (void) const            { nocn(); return err_symtab_hash_size (it()); }
    err_symtab_t &rehash (int n)                     { nocn(); err_symtab_rehash (it(), n); return *this;}
    double  average_line_length (void) const  { nocn(); return err_symtab_average_line_length (it()); }
    double  variance_line_length (void) const { nocn(); return err_symtab_variance_line_length (it()); }
#ifdef HAVE_SQRT
    double  deviation_line_length (void) const { nocn(); return err_symtab_deviation_line_length (it()); }
#endif
    int     max_line_length (void) const      { nocn(); return err_symtab_max_line_length (it()); }
    int     min_line_length (void) const      { nocn(); return err_symtab_min_line_length (it()); }

    void    dump (FILE *f) const              { nocn(); err_symtab_dump (f, it()); }

#ifdef ERR_ERWIN_PROFILE
    int     nrehash_ops (void) const          { nocn(); return err_symtab_nrehash_ops (it()); }
    int     nrehash (void) const              { nocn(); return err_symtab_nrehash (it()); }
    int     ninsert (void) const              { nocn(); return err_symtab_ninsert (it()); }
    int     nremove (void) const              { nocn(); return err_symtab_nremove (it()); }
    int     nfind (void) const                { nocn(); return err_symtab_nfind (it()); }
    int     nops(void) const                  { nocn(); return err_symtab_nops (it()); }
#endif

    err_hashval_t hash_raw(void) const { nocn(); return err_symtab_hash_raw (it()); }
    err_hashval_t hash(void) const     { nocn(); return err_symtab_hash     (it()); }

    bool equal(err_symtab_t const *other) const
    {
        if (this == NULL || other == NULL) /* must be NULL-safe */
            return this == other;
        return ERR_ERWIN_TO_BOOL(err_symtab_equal (it(), other->it()));
    }
    ...
    int cmp (err_symtab_t const *other) const
    {
        if (this == NULL)
            return other == NULL ? 0 : -1;
        if (other == NULL)
            return +1;
        return err_symtab_cmp (it(), other->it());
    }

    int cmp (err_symtab_t const &other) const { nocn(); return cmp (&other); }

    bool operator== (err_symtab_t const &b) const { nocn(); return equal(b); }    /* see equal() */
    bool operator== (err_symtab_t const *b) const { nocn(); return equal(b); }    /* see equal() */

    bool operator!= (err_symtab_t const &b) const { nocn(); return !equal(b); }   /* see equal() */
    bool operator!= (err_symtab_t const *b) const { nocn(); return !equal(b); }   /* see equal() */

    bool operator<= (err_symtab_t const &b) const { nocn(); return cmp(b) <= 0; } /* see cmp() */
    bool operator<= (err_symtab_t const *b) const { nocn(); return cmp(b) <= 0; } /* see cmp() */
...
/* Note: The err_symtab_pair_cmp_t argument to the next_user_... functions is useful only
 ...
 */
    ...
    {
        err_symtab_init_iterator (((ErrSymtab const *)map)->it(), iter);
    }


    static void init_iterator_sorted_by_key (void const *map, err_map_iterator_sorted_t *iter)
    {
        err_symtab_init_iterator_sorted_by_key (((ErrSymtab const *)map)->it(), iter);
    }
    ...
    static void init_iterator_sorted_by_user (void const *map, err_map_iterator_sorted_t *iter, err_symtab_void_pair_cmp_t u)
    {
        err_symtab_init_iterator_sorted_by_user (((ErrSymtab const *)map)->it(), iter, (err_symtab_pair_cmp_t)u);
    }
    ...
    bool next_key (err_map_iterator_t &iter, char const * &k) const
    {
        return err_symtab_next_iteration_keys (it(), &iter, &k) != ERR_ERWIN_FALSE;
    }

    bool next_sorted_key (err_map_iterator_sorted_t &iter, char const * &k) const
    {
        return err_symtab_next_iteration_sorted_keys (it(), &iter, &k) != ERR_ERWIN_FALSE;
    }

    bool next_user_sorted_key (err_map_iterator_sorted_t &iter, err_symtab_pair_cmp_t, char const * &k) const
    {
        return err_symtab_next_iteration_sorted_keys (it(), &iter, &k) != ERR_ERWIN_FALSE;
    }
    ...
    bool next_pair (err_map_iterator_t &iter, err_symtab_pair_t &p) const
    {
        return err_symtab_next_iteration_pairs (it(), &iter, &p) != ERR_ERWIN_FALSE;
    }

    bool next_sorted_pair (err_map_iterator_sorted_t &iter, err_symtab_pair_t &p) const
    {
        return err_symtab_next_iteration_sorted_pairs (it(), &iter, &p) != ERR_ERWIN_FALSE;
    }

    bool next_user_sorted_pair (err_map_iterator_sorted_t &iter, err_symtab_pair_cmp_t, err_symtab_pair_t &p) const
    {
        return err_symtab_next_iteration_sorted_pairs (it(), &iter, &p) != ERR_ERWIN_FALSE;
    }
    ...
    {
        err_symtab_init_iterator_sorted_by_key_reverse (((ErrSymtab const *)map)->it(), iter);
    }
    ...
    static void init_iterator_sorted_by_user_reverse (void const *map, err_map_iterator_sorted_t *iter, err_symtab_void_pair_cmp_t u)
    {
        err_symtab_init_iterator_sorted_by_user_reverse (((ErrSymtab const *)map)->it(), iter, (err_symtab_pair_cmp_t)u);
    }
    ...
    bool next_sorted_key_reverse (err_map_iterator_sorted_t &iter, char const * &k) const
    {
        return err_symtab_next_iteration_sorted_keys_reverse (it(), &iter, &k) != ERR_ERWIN_FALSE;
    }

    bool next_user_sorted_key_reverse (err_map_iterator_sorted_t &iter, err_symtab_pair_cmp_t, char const * &k) const
    {
        return err_symtab_next_iteration_sorted_keys_reverse (it(), &iter, &k) != ERR_ERWIN_FALSE;
    }
    ...
    bool next_sorted_pair_reverse (err_map_iterator_sorted_t &iter, err_symtab_pair_t &p) const
    {
        return err_symtab_next_iteration_sorted_pairs_reverse (it(), &iter, &p) != ERR_ERWIN_FALSE;
    }

    bool next_user_sorted_pair_reverse (err_map_iterator_sorted_t &iter, err_symtab_pair_cmp_t, err_symtab_pair_t &p) const
    {
        return err_symtab_next_iteration_sorted_pairs_reverse (it(), &iter, &p) != ERR_ERWIN_FALSE;
    }
...
extern "C++" {
ERWIN_WRAPPER
err_symtab_t *err_erwin_ptr_of(err_symtab_t *x) { return x;  }

ERWIN_WRAPPER
err_symtab_t *err_erwin_ptr_of(err_symtab_t &x) { return &x; }

ERWIN_WRAPPER
err_symtab_t const *err_erwin_ptr_const_of (err_symtab_t const *x) { return x;  }
...
/*
 * For the sorted iteration you must provide ERR_CHAR_CONST_P_CMP and ERR_VOID_CMP resp.  Otherwise, you
 ...
 *    : ...
 *
 * It might be interesting to compare this to the C version err_symtab_forall.
 *
 ...
 * to arbitrarily modify (err_symtab_erase, err_symtab_insert, err_symtab_modify) the map while you
 ...
 * Use err_symtab_erase and err_symtab_insert for this.
 *
...
#   define err_map_forall_sorted_by_user(h,u,k,v) \
        for(err_map_iterator_sorted_t \
                ERWIN_GENSYM(erwin_local_i)(err_erwin_ptr_const_of(h)->init_iterator_sorted_by_user, \
                    (void const *)err_erwin_ptr_const_of(h), (err_symtab_void_pair_cmp_t)u); \
...
#   define err_map_forall_ptr_sorted_by_user(h,U,k,V) \
        for(err_map_iterator_sorted_t \
                ERWIN_GENSYM(erwin_local_i)(err_erwin_ptr_of(h)->init_iterator_ptr_sorted_by_user, \
                    (void const *)err_erwin_ptr_of(h), (err_symtab_void_pair_cmp_t)U); \
...
#endif /* !defined(ERWIN_ERR_SYMTAB_h) */
