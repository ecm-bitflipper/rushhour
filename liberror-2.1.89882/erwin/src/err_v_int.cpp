...
#ifndef ERR_V_INT_IMPLEMENTATION
#define ERR_V_INT_IMPLEMENTATION
#endif
...
#ifdef ERR_INT_CONSTRUCTOR
#define OCONSTRUCTOR ERR_INT_CONSTRUCTOR
#endif
...
#ifdef ERR_INT_DESTRUCTOR
#define ODESTRUCTOR ERR_INT_DESTRUCTOR
#endif
...
#ifdef ERR_INT_PRIORITY_CMP
#define OPRIORITY_CMP ERR_INT_PRIORITY_CMP
#endif

#ifdef ERR_INT_UPDATE_POS2
#define OUPDATE_POS2 ERR_INT_UPDATE_POS2
#endif

#ifdef ERR_INT_UPDATE_POS
#define OUPDATE_POS ERR_INT_UPDATE_POS
#endif
...
#  else
#      define e_errno          (ERWIN_CONST_CAST(err_v_int_t*,self)->m_errno)
#  endif
#else
...
#if ERR_V_INT_INITIAL_SIZE < 0
#error ERR_V_INT_INITIAL_SIZE < 0
#endif

#if ERR_V_INT_MINIMAL_SIZE < 0
#error ERR_V_INT_MINIMAL_SIZE < 0
#endif
 ...
 */
#if ERR_V_INT_INLINE_STORE
#  define ERR_V_INT_ZERO_SIZE 0
#elif ERR_V_INT_MINIMAL_SIZE == 0 || ERR_V_INT_INITIAL_SIZE == 0
#  define ERR_V_INT_ZERO_SIZE 1
#else
#  define ERR_V_INT_ZERO_SIZE 0
#endif
...
#define ERR_V_INT_DUMMY_TABLE ((int*)(void*)&err_v_int_dummy_table)
...
#if ERR_V_INT_LOW_MEM
#  define err_v_int_ensure_table_size_force   err_v_int_ensure_size
#else
#  define err_v_int_ensure_table_size_force   err_v_int_ensure_table_size
#endif
...
#  define ERR_V_INT_INLINE_CNT     ((err_v_int_cnt_t)(sizeof(err_v_int_content_p) / sizeof(err_v_int_content)))

#  define HAVE_INLINE_STORE     (ERR_V_INT_INLINE_CNT > 0)

#  if ERR_V_INT_LOW_MEM
#    define HAS_HEAP_TABLE(X)   ((X) && ((X)->m_nentries > ERR_V_INT_INLINE_CNT))
#  else
#    define HAS_HEAP_TABLE(X)   ((X) && ((X)->m_tablesize> ERR_V_INT_INLINE_CNT))
#  endif

#  define INLINE_TABLE(X)       ((err_v_int_content_p)(&((X)->m_table)))

#  define TABLE(X)              (HAS_HEAP_TABLE(X) ? (X)->m_table : INLINE_TABLE(X))

#  define GET_MINIMAL_SIZE      (HAVE_INLINE_STORE ? ERR_V_INT_INLINE_CNT : ERR_V_INT_MINIMAL_SIZE)
#  define GET_INITIAL_SIZE      (HAVE_INLINE_STORE ? ERR_V_INT_INLINE_CNT : ERR_V_INT_INITIAL_SIZE)

#  define CLAMP_MINIMAL(N)      ((N) <= ERR_V_INT_INLINE_CNT ?      \
                                    ERR_V_INT_INLINE_CNT            \
                                  : (N) <= ERR_V_INT_MINIMAL_SIZE ? \
                                    ERR_V_INT_MINIMAL_SIZE          \
                                  : (N)                          \
                                )
...
#  define ERR_V_INT_INLINE_CNT     0

#  if ERR_V_INT_ZERO_SIZE
#    define HAS_HEAP_TABLE(X)   ((X) && (X)->m_table != ERR_V_INT_DUMMY_TABLE)
#  else
...
#  define GET_MINIMAL_SIZE      ERR_V_INT_MINIMAL_SIZE
#  define GET_INITIAL_SIZE      ERR_V_INT_INITIAL_SIZE

#  define CLAMP_MINIMAL(N)      ((N) <= ERR_V_INT_MINIMAL_SIZE ? \
                                    ERR_V_INT_MINIMAL_SIZE       \
                                  : (N)                       \
                                )
 ...
 * Why?
 *
 * The problem is the err_v_int_shrink() function that might resize to a non-power of 2
...
#define SPLIT_SECTIONS ERR_V_INT_SPLIT
...
#if ERR_V_INT_HAVE_SIG

#  define return_if_bad(self) \
      return_if_fail_ppxxt(self->m_sig == ERR_V_INT_SIG, self, self->m_sig, ERR_V_INT_SIG, "Structure was probably overwritten"); \
      return_if_invalid(self);
#  define return_val_if_bad(self, val) \
      return_val_if_fail_ppxxt(self->m_sig == ERR_V_INT_SIG, val, self, self->m_sig, ERR_V_INT_SIG, "Structure was probably overwritten"); \
      return_val_if_invalid(self, val);
#  define return_if_bad_pC(self, prog) \
      return_if_fail_ppxxtC(self->m_sig == ERR_V_INT_SIG, self, self->m_sig, ERR_V_INT_SIG, "Structure was probably overwritten", prog); \
...
#define USER_ASSERTION_FAILED_HANDLER(FI,L,FU,A)       \
            ASSERTION_FAILED;                          \
            ERR_V_INT_ASSERTION_FAILED_HANDLER(FI,L,FU,A)
#endif

#if ERR_V_INT_SIZE_INDEX_DIFF
#  define VALID_SIZE(A)          (((err_v_int_cnt_t)(err_v_int_size_t)(A)) == ((err_v_int_cnt_t)(A)))
#  define VALID_NENTRIES(A)      1
#else
#  define VALID_SIZE(A)          1
#  define VALID_NENTRIES(A)      (((err_v_int_cnt_t)(A)) >= 0)
#endif

#if ERR_V_INT_MAY_BE_INVALID
...
/* Check that ERR_V_INT_ALLOW_NULL and ERR_ERWIN_THREAD_SAFE are not used at
...
#if ERR_V_INT_ALLOW_NULL && defined(ERR_ERWIN_THREAD_SAFE)
#error "Cannot use ERR_V_INT_ALLOW_NULL and ERR_ERWIN_THREAD_SAFE at the same time."
#endif
 ...
 * allocator/deallocator functions err_v_int_new/err_v_int_delete.  If
 ...
 *     err_v_int_init/err_v_int_destroy manually.
...
#  define ERR_V_INT_NEW                    ERR_ERWIN_TMALLOC1
#  define ERR_V_INT_DELETE                 ERR_ERWIN_TFREE1

#  if defined(INT_IS_PTR) && !INT_IS_PTR
#    define ERR_V_INT_NEW_ARRAY            ERR_ERWIN_TMALLOC_ATOMS
#    define ERR_V_INT_DELETE_ARRAY         ERR_ERWIN_TFREE_ATOMS
#ifdef ERR_ERWIN_TREALLOC_ATOMS
#    define ERR_V_INT_RENEW_ARRAY          ERR_ERWIN_TREALLOC_ATOMS
#endif /* defined ERR_ERWIN_TREALLOC_ATOMS */
#  else
#    define ERR_V_INT_NEW_ARRAY            ERR_ERWIN_TMALLOC_ARRAY
#    define ERR_V_INT_DELETE_ARRAY         ERR_ERWIN_TFREE_ARRAY
#ifdef ERR_ERWIN_TREALLOC_ARRAY
#    define ERR_V_INT_RENEW_ARRAY          ERR_ERWIN_TREALLOC_ARRAY
#endif /* defined ERR_ERWIN_TREALLOC_ARRAY */
#  endif

#else

#  define ERR_V_INT_NEW                    ERR_ERWIN_THING_NEW1
#  define ERR_V_INT_DELETE                 ERR_ERWIN_THING_DELETE1

#  if defined(INT_IS_PTR) && !INT_IS_PTR
#    define ERR_V_INT_NEW_ARRAY            ERR_ERWIN_THING_NEW_ATOMS
#    define ERR_V_INT_DELETE_ARRAY         ERR_ERWIN_THING_DELETE_ATOMS
#ifdef ERR_ERWIN_THING_RENEW_ATOMS
#    define ERR_V_INT_RENEW_ARRAY          ERR_ERWIN_THING_RENEW_ATOMS
#endif /* defined ERR_ERWIN_THING_RENEW_ATOMS */
#  else
#    define ERR_V_INT_NEW_ARRAY            ERR_ERWIN_THING_NEW_ARRAY
#    define ERR_V_INT_DELETE_ARRAY         ERR_ERWIN_THING_DELETE_ARRAY
#ifdef ERR_ERWIN_THING_RENEW_ARRAY
#    define ERR_V_INT_RENEW_ARRAY          ERR_ERWIN_THING_RENEW_ARRAY
...
#  define ERR_V_INT_SOME_BIGGER_NUMBER(X)  err_erwin_next_power2_minus1(X)

#  define ERR_V_INT_ALIGN_TABLESIZE(X) \
             do { (X)= ERR_V_INT_SOME_BIGGER_NUMBER(X); } while(0)

#  define ERR_V_INT_GET_TABLESIZE(X) \
             ((err_v_int_cnt_t)ERR_V_INT_SOME_BIGGER_NUMBER (CLAMP_MINIMAL((X)->m_nentries+1)))
...
#  define ERR_V_INT_SET_TABLESIZE(X,Y) do {} while(0)

#else

#  define ERR_V_INT_ALIGN_TABLESIZE(X) do {} while(0)
#  define ERR_V_INT_GET_TABLESIZE(X)   ((X)->m_tablesize)
#  define ERR_V_INT_SET_TABLESIZE(X,Y) do { (X)->m_tablesize= (Y); } while(0)
...
#if ERR_V_INT_NOMEM_IS_FATAL
#define VECTOR_NOMEM        error_exit_if_reached_pt("Fatal: Memory exhausted.")
#else
#define VECTOR_NOMEM        SET_ERRNO(E_NOMEM)
#endif

#if ERR_V_INT_OUTOFRANGE_IS_FATAL

#  define err_v_int_if_outofrange(X)                                \
          error_exit_unless_fail_pt (X, "Fatal: Out Of Range."); \
          if (0)

#  define err_v_int_if_outofrange_minmax(X,I,N)                             \
          error_exit_unless_fail_ptii (X, "Fatal: Out Of Range.", I, N); \
          if (0)

#  define err_v_int_if_outofrange_min(X,I)                              \
          error_exit_unless_fail_pti (X, "Fatal: Out Of Range.", I); \
          if (0)

#elif ERR_V_INT_RANGE_CHECK
#  define err_v_int_if_outofrange(X)             if (ERWIN_UNLIKELY(X))
#  define err_v_int_if_outofrange_minmax(X,I,N)  if (ERWIN_UNLIKELY(X))
#  define err_v_int_if_outofrange_min(X,I)       if (ERWIN_UNLIKELY(X))
#else
#  define err_v_int_if_outofrange(X)             if (0)
#  define err_v_int_if_outofrange_minmax(X,I,N)  if (0)
#  define err_v_int_if_outofrange_min(X,I)       if (0)
#endif


/* ********************************************************************** */

#if !defined(ERR_INT_UPDATE_POS)

#  define ERR_V_INT_UPDATE_POS_SET(SELF, I)
#  define ERR_V_INT_UPDATE_POS_SET_MANY(SELF, A, N)

#  define ERR_V_INT_UPDATE_POS_RESET(SELF, I)
#  define ERR_V_INT_UPDATE_POS_RESET_MANY(SELF, A, N)

#  define ERR_V_INT_UPDATE_POS_SET2(SELF, I1, V1, I2, V2)

#else

#  define ERR_V_INT_UPDATE_POS_SET(SELF, I)                           \
          do {                                                     \
              ERR_INT_UPDATE_POS(SELF, &(TABLE(SELF)[I]), I); \
          } while(0)

#  define ERR_V_INT_UPDATE_POS_RESET(SELF, I)                          \
          do {                                                      \
              ERR_INT_UPDATE_POS(SELF, &(TABLE(SELF)[I]), -1); \
          } while(0)

#  define ERR_V_INT_UPDATE_POS_SET_MANY(SELF, I0, IN)        \
          do {                                            \
              err_v_int_index_t i;                           \
              for (i=(I0); i < (err_v_int_index_t)(IN); i++) \
                  ERR_V_INT_UPDATE_POS_SET(SELF, i);         \
          } while(0)

#  define ERR_V_INT_UPDATE_POS_RESET_MANY(SELF, I0, IN)      \
          do {                                            \
              err_v_int_index_t i;                           \
              for (i=(I0); i < (err_v_int_index_t)(IN); i++) \
                  ERR_V_INT_UPDATE_POS_RESET(SELF, i);       \
          } while(0)

#  if !defined(ERR_INT_UPDATE_POS2)

#    define ERR_V_INT_UPDATE_POS_SET2(SELF, I1, V1, I2, V2) \
            do {                                         \
                ERR_V_INT_UPDATE_POS_SET(SELF, I1);         \
                ERR_V_INT_UPDATE_POS_SET(SELF, I2);         \
            } while(0)
...
#if ERR_V_INT_POSITION_POINTER && !ERR_V_INT_MANUAL_POSITION
...
#define ERR_V_INT_UPDATE_POS_INSERT(SELF, I)   \
        do {                                \
            ADD_POS_AT(SELF, I, 1);         \
            ERR_V_INT_UPDATE_POS_SET(SELF, I); \
        }while(0)

#define ERR_V_INT_UPDATE_POS_INSERT_MANY(SELF, I0, IN)   \
        do {                                          \
            ADD_POS_AT(SELF, I0, (IN)-(I0));          \
            ERR_V_INT_UPDATE_POS_SET_MANY(SELF, I0, IN); \
        }while(0)

#define ERR_V_INT_UPDATE_POS_ERASE(SELF, I)      \
        do {                                  \
            ERR_V_INT_UPDATE_POS_RESET(SELF, I); \
            SUB_POS_AT(SELF, I, 1);           \
        }while(0)

#define ERR_V_INT_UPDATE_POS_ERASE_MANY(SELF, I0, IN)      \
        do {                                            \
            ERR_V_INT_UPDATE_POS_RESET_MANY(SELF, I0, IN); \
            SUB_POS_AT(SELF, I0, (IN)-(I0));            \
        }while(0)

/* ******************************************************************************** */

#define ERR_V_INT_EQ_USE_ARG(X) X != ERR_ERWIN_FALSE
#define err_v_int_overwrite_copy_aux_q(A,B,C,D,E)    err_v_int_overwrite_copy_aux(A,B,C,D,E)
#define err_v_int_overwrite_no_copy_aux_q(A,B,C,D,E) err_v_int_overwrite_no_copy_aux(A,B,C,D,E)
#define err_v_int_erase_aux_q(A,B,C,D,E)             err_v_int_erase_aux(A,B,C,D,E)
#define err_v_int_erase_flags_q(A,B,C,D,E)           err_v_int_erase_flags(A,B,C,D,E)
#define err_v_int_erase_if_flags_q(A,B,C,D,E)        err_v_int_erase_if_flags(A,B,C,D,E)
#define err_v_int_swap_erase_aux_q(A,B,C,D,E)        err_v_int_swap_erase_aux(A,B,C,D,E)
#define err_v_int_swap_erase_flags_q(A,B,C,D,E)      err_v_int_swap_erase_flags(A,B,C,D,E)
#define err_v_int_swap_erase_if_flags_q(A,B,C,D,E)   err_v_int_swap_erase_if_flags(A,B,C,D,E)
#define err_v_int_chop_flags_q(A,B,C,D)              err_v_int_chop_flags(A,B,C,D)

#if ERR_V_INT_DYN_ZERO
#  define err_v_int_init_internal_q(SE,Z,SI)         err_v_int_init_internal(SE,Z,SI)
#  define err_v_int_init_from_q(SE,C,S,A,Z)          err_v_int_init_from_aux(SE,C,S,A,Z)
#else
#  define err_v_int_init_internal_q(SE,Z,SI)         err_v_int_init_internal(SE,SI)
#  define err_v_int_init_from_q(SE,C,S,A,Z)          err_v_int_init_from_aux(SE,C,S,A)
#endif

#define ERR_V_INT_DIGIT_SHR_AND(X,N,B) \
          ((int)(unsigned char)err_v_int_hex_digit[(((unsigned Tchar)(X)) >> ((B)*(N))) & ((1U<<(B))-1)])

#define ERR_V_INT_HEX_DIGIT(X,N) ERR_V_INT_DIGIT_SHR_AND(X,N,4)
#define ERR_V_INT_OCT_DIGIT(X,N) ERR_V_INT_DIGIT_SHR_AND(X,N,3)
#define ERR_V_INT_BIN_DIGIT(X,N) ERR_V_INT_DIGIT_SHR_AND(X,N,1)
...
static
int err_v_int_dummy_table;
    /* THREAD SAFE: never written, just a dummy */

#ifdef TYPE_INFO_T

static
TYPE_STRUCT_T(err_v_int_t)=
    TYPE_STRUCT_BEGIN(err_v_int_t)
#ifdef  ERR_V_INT_SUPER_TYPE
        TYPE_SUPER(err_v_int_t, ERR_V_INT_SUPER_CLASS),
#endif
        ERR_V_INT_TYPE_INFO_STD_MEMBERS
        err_v_int_record_sig       (TYPE_STRUCT_SLOT (err_v_int_t, int,              m_sig))
        err_v_int_record_table     (TYPE_STRUCT_SLOT (err_v_int_t, err_v_int_content_p, m_table))
        err_v_int_record_tablesize (TYPE_STRUCT_SLOT (err_v_int_t, err_v_int_size_t,    m_tablesize))
        err_v_int_record_nentries  (TYPE_STRUCT_SLOT (err_v_int_t, err_v_int_size_t,    m_nentries))
        err_v_int_record_pos       (TYPE_STRUCT_SLOT (err_v_int_t, err_v_int_index_t,   m_pos))
        err_v_int_record_errno     (TYPE_STRUCT_SLOT (err_v_int_t, int,              m_errno))
        err_v_int_record_zero      (TYPE_STRUCT_SLOT (err_v_int_t, int,  m_zero))
    TYPE_STRUCT_END
;


TYPE_INFO_T(err_v_int_size_t)= TYPE_INFO_UINT (err_v_int_size_t);


TYPE_INFO_T(err_v_int_cnt_t)= TYPE_INFO_SINT (err_v_int_cnt_t);


TYPE_INFO_T(err_v_int_index_t)= TYPE_INFO_SINT (err_v_int_index_t);


TYPE_INFO_T(err_v_int_t)= TYPE_INFO_STRUCT (err_v_int_t, err_v_int_t);


TYPE_INFO_T(err_v_int_t_p)= TYPE_INFO_POINTER (err_v_int_t *, err_v_int_t);


TYPE_INFO_T(err_v_int_t_const_p)= TYPE_INFO_POINTER (err_v_int_t const *, err_v_int_t);


TYPE_INFO_T(err_v_int_content)=
    TYPE_INFO_VECTOR (err_v_int_content, int, err_v_int_t, m_nentries);


TYPE_INFO_T(err_v_int_content_p)= TYPE_INFO_POINTER (err_v_int_content *, err_v_int_content);
...
E_BOOL err_v_int_init_internal_q
        (err_v_int_t *self, int  zero, err_v_int_cnt_t size)
{
#ifndef NDEBUG
    int err;
    static int tested= 0;
    ...
    ERR_V_INT_ALIGN_TABLESIZE (size);

    err_v_int_record_sig (self->m_sig= ERR_V_INT_SIG);

#if ERR_V_INT_INLINE_STORE

    if (size > ERR_V_INT_INLINE_CNT) {
        E_SET_PTR(self->m_table, ERR_V_INT_NEW_ARRAY (err_v_int_content, size));
        ...
        ERR_V_INT_SET_TABLESIZE (self, size);
    }
    else {
        ERR_V_INT_SET_TABLESIZE (self, ERR_V_INT_INLINE_CNT);
...
#if ERR_V_INT_ZERO_SIZE
    if (size == 0)
        E_SET_PTR(self->m_table, ERR_V_INT_DUMMY_TABLE);
    else
        E_SET_PTR(self->m_table, ERR_V_INT_NEW_ARRAY (err_v_int_content, size));
#else /* !ZERO_SIZE */
    if (size <= 0)
...
#if defined(OCONSTRUCTOR) && ERR_V_INT_C_MM
    /* C model for table */
    {
        err_v_int_cnt_t i;
        for (i=0; i < size; i++)
    ...
    if (ERWIN_UNLIKELY(!tested)) {
        ...
        return0_if_fail_ppii (sizeof(err_v_int_size_t) <= sizeof(err_v_int_cnt_t),
           self,
           sizeof(err_v_int_size_t), sizeof(err_v_int_cnt_t));

        return0_if_fail_ppii (sizeof(err_v_int_index_t) == sizeof(err_v_int_cnt_t),
           self,
           sizeof(err_v_int_index_t), sizeof(err_v_int_cnt_t));
...
int err_v_int_init
        (err_v_int_t *self)
{
    err_v_int_init_internal_q (self, OZERO, GET_INITIAL_SIZE);
    return e_errno;
}

CODE_SECTION(".text.init_with_initial_size")
int err_v_int_init_with_initial_size
        (err_v_int_t *self, err_v_int_cnt_t size)
{
    err_v_int_init_internal_q (self, OZERO, size);
    return e_errno;
}
...
int err_v_int_init_with_zero_and_initial_size
        (err_v_int_t *self, int  zero, err_v_int_cnt_t size)
{
    err_v_int_init_internal_q (self, zero, size);
    return e_errno;
}
#endif /* ERR_V_INT_DYN_ZERO */

CODE_SECTION(".text.delete_array")
void err_v_int_delete_array
        (err_v_int_element_ptr_t array)
{
    /* No special case for ERR_V_INT_INLINE_STORE, because those arrays
     * allowed to be deleted with this functions will be assured to
     * be allocated on the heap. */

#if ERR_V_INT_ZERO_SIZE
    if (array == ERR_V_INT_DUMMY_TABLE)
        return;
#endif
...
void err_v_int_detach
        (err_v_int_t *self)
{
    return_if_null2i_or_bad (self);

    ERR_V_INT_UPDATE_POS_ERASE_MANY(self, 0, self->m_nentries);
    ...
    ERR_V_INT_SET_TABLESIZE (self, ERR_V_INT_INLINE_CNT);
#elif ERR_V_INT_ZERO_SIZE
    E_SET_PTR (self->m_table, ERR_V_INT_DUMMY_TABLE);
    ERR_V_INT_SET_TABLESIZE (self, 0);
#else
    E_SET_PTR (self->m_table, ERR_V_INT_NEW_ARRAY (err_v_int_content, 1));
    ERR_V_INT_SET_TABLESIZE (self, 1);
...
err_v_int_t * err_v_int_new_with_zero_and_initial_size
        (int  zero, err_v_int_cnt_t size)
{
    err_v_int_t *self;
    E_SET_PTR(self, ERR_V_INT_NEW (err_v_int_t));
    if (ERWIN_UNLIKELY(self == NULL)) {
        VECTOR_NOMEM;
        return NULL;
    }

    if (ERWIN_LIKELY(err_v_int_init_internal_q (self, zero, size)))
       return self;

    ERR_V_INT_DELETE (self, err_v_int_t);
    return NULL;
}

CODE_SECTION(".text.new_with_zero")
err_v_int_t * err_v_int_new_with_zero
        (int  zero)
{
    return err_v_int_new_with_zero_and_initial_size (zero, GET_INITIAL_SIZE);
}
#endif /* ERR_V_INT_DYN_ZERO */


#if ERR_V_INT_CONSTANT_ZERO
#  define err_v_int_new_with_zero_and_initial_size(Z,S) err_v_int_new_with_initial_size(S)
#  define err_v_int_new_with_zero(Z)                    err_v_int_new()
#endif


CODE_SECTION(".text.new_with_initial_size")
err_v_int_t * err_v_int_new_with_initial_size
        (err_v_int_cnt_t size)
{
    ...
    if (ERWIN_LIKELY(err_v_int_init_internal_q (self, OZERO, size)))
        return self;
...
E_BOOL err_v_int_init_from_q
        (
    err_v_int_t *self, int *contents, err_v_int_cnt_t nentries, err_v_int_cnt_t allocsize, int  zero)
{
    if (allocsize < nentries)
        allocsize= nentries;

    /* We don't align the allocsize here since this is done in err_v_int_init_internal_q,
    ...
    if (contents == NULL || allocsize == 0)
        return err_v_int_init_internal_q (self, OZERO, allocsize);
    ...
    ERR_V_INT_SET_TABLESIZE (self, allocsize);
    ...
    ERR_V_INT_UPDATE_POS_INSERT_MANY(self, 0, nentries);
...
err_v_int_t * err_v_int_new_from_raw
        (int *contents, err_v_int_cnt_t nentries, err_v_int_cnt_t allocsize)
{
    err_v_int_t *self;
    E_SET_PTR(self, ERR_V_INT_NEW (err_v_int_t));
    if (ERWIN_UNLIKELY(self == NULL)) {
    ...
    if (ERWIN_LIKELY(err_v_int_init_from_q (self, contents, nentries, allocsize, OZERO)))
        return self;
...
err_v_int_t * err_v_int_new_from_raw_with_zero
        (int *contents, err_v_int_cnt_t nentries, err_v_int_cnt_t allocsize, int  zero)
{
    err_v_int_t *self;
    E_SET_PTR(self, ERR_V_INT_NEW (err_v_int_t));
    ...
    if (ERWIN_LIKELY(err_v_int_init_from_q (self, contents, nentries, allocsize, zero)))
        return self;
...
err_v_int_t * err_v_int_new_from_vector
        (err_v_int_t *other)
{
    err_v_int_t *self;
    err_v_int_size_t ts;
...
#endif
    if (ERWIN_LIKELY(
            err_v_int_init_from_q (self,
                TABLE(other),
                other->m_nentries,
                ts,
                other->m_zero)))
    {
        err_v_int_detach_as_is (other);
...
err_v_int_t * err_v_int_new
        (void)
{
    return err_v_int_new_with_initial_size (GET_INITIAL_SIZE);
}

CODE_SECTION(".text.zero")
o_type_result_t err_v_int_zero
        (err_v_int_t const *self)
{
    (void)self; /*unused*/
...
err_v_int_t * err_v_int_copy
        (err_v_int_t const* self)
{
    err_v_int_t *copy;
    if (self == NULL) { /* copy() *must* be NULL safe!  Regardless of ERR_V_INT_ALLOW_NULL. */
        SET_ERRNO(E_OK);
        return NULL;
    }
    ...
    E_SET_PTR(copy, err_v_int_new_with_zero_and_initial_size (ZERO(self), COPY_SIZE (self)));
    err_v_int_insert_vector (copy, 0, self);

    return copy;
}

CODE_SECTION(".text.copy_err")
err_v_int_t * err_v_int_copy_err
        (err_v_int_t const* self, int *err)
{
    err_v_int_t *copy;
    E_SET_PTR(copy, err_v_int_copy (self));
...
err_v_int_cnt_t err_v_int_table_size
        (err_v_int_t const *self)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
        return 0;
#endif
    return ERR_V_INT_GET_TABLESIZE (self);
}

CODE_SECTION(".text.nentries")
err_v_int_cnt_t err_v_int_nentries
        (err_v_int_t const *self)
{                 
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
...
void err_v_int_clear_no_resize_aux
        (err_v_int_t *self, err_v_int_cnt_t new_size)
{
#if OFREE_PROVIDED
    err_v_int_size_t i;
#endif
    ...
    ERR_V_INT_UPDATE_POS_ERASE_MANY (self, new_size, self->m_nentries);
...
void err_v_int_xchg
        (err_v_int_t *self, err_v_int_t *other)
{
    err_v_int_record_table     (ERWIN_XCHG (err_v_int_content_p, self->m_table,     other->m_table));
    err_v_int_record_tablesize (ERWIN_XCHG (err_v_int_size_t,    self->m_tablesize, other->m_tablesize));
    err_v_int_record_nentries  (ERWIN_XCHG (err_v_int_size_t,    self->m_nentries,  other->m_nentries));
    err_v_int_record_pos       (ERWIN_XCHG (err_v_int_index_t,   self->m_pos,       other->m_pos));
    err_v_int_record_errno     (ERWIN_XCHG (int,              self->m_errno,     other->m_errno));
    err_v_int_record_zero      (ERWIN_XCHG (int,         self->m_zero,      other->m_zero));
}

CODE_SECTION(".text.destroy")
void err_v_int_destroy
        (err_v_int_t *self)
{
    if (self == NULL ||
    ...
    err_v_int_clear_no_resize_aux (self, 0);

    {
        /* C model for table */
#if defined(ODESTRUCTOR) && ERR_V_INT_C_MM
        err_v_int_cnt_t i;
        err_v_int_cnt_t ts= ERR_V_INT_GET_TABLESIZE (self);
        for (i=0; i < ts; i++) {
...
#endif
        if (HAS_HEAP_TABLE(self))
            ERR_V_INT_DELETE_ARRAY (TABLE(self));
    }
}

CODE_SECTION(".text.delete")
void err_v_int_delete
        (err_v_int_t *self)
{
    if (self == NULL) /* It must handle NULL to be nestable! */
        return;
    return_if_null_or_bad (self);
    err_v_int_destroy (self);
    ERR_V_INT_DELETE (self, err_v_int_t);
}

ERWIN_STATIC_INLINE
CODE_SECTION(".text.reallocate")
E_BOOL err_v_int_reallocate
        (err_v_int_t *self, err_v_int_cnt_t newtablesize)
/* This is very ugly.  I know.
 * NOTE: newtablesize must be aligned (for LOW_MEM)
 */
{
    err_v_int_cnt_t oldtablesize=      ERR_V_INT_GET_TABLESIZE (self);
    E_BOOL       old_is_heap_table= HAS_HEAP_TABLE(self);
    int    *oldtable;
    int    *newtable;

    E_SET_PTR(newtable, ERR_V_INT_DUMMY_TABLE);
    ...
    if (newtablesize <= ERR_V_INT_INLINE_CNT) {
        /* inline,heap > inline */
        newtablesize= ERR_V_INT_INLINE_CNT; /* this is the minimum */
...
#endif
            ERR_V_INT_DELETE_ARRAY (oldtable);
...
#ifdef ERR_V_INT_RENEW_ARRAY
        E_SET_PTR(newtable,
            ERR_V_INT_RENEW_ARRAY (oldtable, err_v_int_content, newtablesize, oldtablesize));
        old_is_heap_table= 0; /* reallocated => invalidated */
#else
        E_SET_PTR(newtable,
            ERR_V_INT_NEW_ARRAY (err_v_int_content, newtablesize));
#endif

        ERR_V_INT_SET_TABLESIZE (self, newtablesize);
    ...
    if (old_is_heap_table) {
        ...
        old_is_heap_table= 0;
#else
        E_SET_PTR(newtable, ERR_V_INT_NEW_ARRAY (err_v_int_content, newtablesize));
#endif
    }
    else
        E_SET_PTR(newtable, ERR_V_INT_NEW_ARRAY (err_v_int_content, newtablesize));
...
E_BOOL err_v_int_ensure_size_sub_aux_force
        (err_v_int_t *self, err_v_int_cnt_t newsize)
  /* may leave elements uninitialized! */
  /* ensures that SIZE many elements go into THEM without problem. */
{
    err_v_int_cnt_t tablesize= ERR_V_INT_GET_TABLESIZE (self);

#if ERR_V_INT_LOW_MEM
    err_v_int_cnt_t newtablesize= ERR_V_INT_SOME_BIGGER_NUMBER (CLAMP_MINIMAL(newsize));
    if (ERWIN_UNLIKELY(tablesize != newtablesize))
        return err_v_int_reallocate (self, newtablesize);
#else
    /* Does not shrink below size 2 unless newsize == 0. */
    if (ERWIN_UNLIKELY(NEED_SHRINK(newsize, tablesize))) {
        do {
            tablesize>>=1;
#if ERR_V_INT_ZERO_SIZE || ERR_V_INT_INLINE_STORE
            if (tablesize == 0)
                break;
#endif
        } while (ERWIN_UNLIKELY(NEED_SHRINK(newsize, tablesize)));
        return0_if_fail (newsize <= tablesize);
        return err_v_int_reallocate (self, ADJUST_SHRUNK(tablesize));
    }
#endif
    return E_TRUE;
}

#if ERR_V_INT_AUTO_SHRINK
#  define err_v_int_ensure_size_sub_aux(A,B) err_v_int_ensure_size_sub_aux_force(A,B)
#else
#  define err_v_int_ensure_size_sub_aux(A,B) E_TRUE
#endif


ERWIN_STATIC_INLINE
CODE_SECTION(".text.ensure_size_add_aux")
E_BOOL err_v_int_ensure_size_add_aux
        (err_v_int_t *self, err_v_int_cnt_t newsize)
  /* May leave elements uninitialized! */
  /* Ensures that SIZE many elements go into THEM without problem. */
   ...
   */
{
    ...
    if (ERWIN_UNLIKELY(NEED_GROW(newsize, tablesize))) {
...
#if !ERR_V_INT_LOW_MEM && ERR_V_INT_SIZE_INDEX_DIFF
            /* cull at MAX(err_v_int_size_t)-1 */
            if (tn < tablesize && tn != ((err_v_int_size_t)-1))
                tn= ((err_v_int_size_t)-1);
#endif

            if (ERWIN_UNLIKELY(!VALID_SIZE_GT(tablesize, (err_v_int_cnt_t)tn))) {
                SET_ERRNO(E_TOOLARGE);
                return E_FALSE;
            }
        ...
        return err_v_int_reallocate(self, ADJUST_GROWN(tablesize));
    }

#endif /* !ERR_V_INT_LOW_MEM */
    return E_TRUE;
}

ERWIN_STATIC_INLINE
CODE_SECTION(".text.ensure_size_add_plus")
E_BOOL err_v_int_ensure_size_add_plus
        (err_v_int_t *self, err_v_int_cnt_t newsize, err_v_int_cnt_t add)
{
   if (ERWIN_UNLIKELY(!VALID_SIZE_PLUS (newsize, add))) {
       SET_ERRNO(E_TOOLARGE);
       return E_FALSE;
   }
   return err_v_int_ensure_size_add_aux (self, newsize + add);
}

ERWIN_STATIC_INLINE
CODE_SECTION(".text.init_tail")
void err_v_int_init_tail
        (err_v_int_t *self, err_v_int_cnt_t newsize)
{
    err_v_int_cnt_t ii;
...
E_BOOL err_v_int_grow_plus
        (err_v_int_t *self, err_v_int_cnt_t oldsize, err_v_int_cnt_t plus)
{
    err_v_int_cnt_t newsize= oldsize + plus;
    if (ERWIN_UNLIKELY(newsize > (err_v_int_cnt_t)self->m_nentries)) {
        if (ERWIN_UNLIKELY(!err_v_int_ensure_size_add_plus (self, oldsize, plus)))
            return E_FALSE;

        err_v_int_init_tail (self, newsize);
...
int err_v_int_ensure_size
        (err_v_int_t *self, err_v_int_cnt_t newsize)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);

    err_v_int_if_outofrange_min (newsize < 0, newsize)
        return PASS_ERRNO(E_OUTOFRANGE);

    if (ERWIN_UNLIKELY(newsize > (err_v_int_cnt_t)self->m_nentries)) {
        if (ERWIN_LIKELY(!err_v_int_ensure_size_add_aux (self, newsize)))
            return e_errno;
...
int err_v_int_ensure_size_with
        (err_v_int_t *self, err_v_int_cnt_t newsize, int  elem)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    ...
    if (ERWIN_UNLIKELY(newsize > (err_v_int_cnt_t)self->m_nentries))
        return err_v_int_make_gap_with (self, self->m_nentries, elem, newsize - self->m_nentries);
...
E_BOOL err_v_int_setsize_noinit_add
        (err_v_int_t *self, err_v_int_cnt_t newsize)
{
    if (ERWIN_LIKELY(!err_v_int_ensure_size_add_aux (self, newsize)))
        return E_FALSE;
...
E_BOOL err_v_int_setsize_noinit_sub
        (err_v_int_t *self, err_v_int_cnt_t newsize, E_BOOL resize)
{
    if (resize) {
        if (ERWIN_UNLIKELY(!err_v_int_ensure_size_sub_aux (self, newsize)))
            return E_FALSE;
    }
    return_val_if_fail_ppi (VALID_SIZE (newsize), E_FALSE, self, newsize);
    self->m_nentries= newsize;
        ...
        (err_v_int_t *self)
{
    ...
    self->m_nentries= 0;
    SET_POS (self, 0);
    err_v_int_ensure_size_sub_aux (self, 0);
}

CODE_SECTION(".text.clear_keep")
void err_v_int_clear_keep
        (err_v_int_t *self, err_v_int_cnt_t min)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
        return;
    ...
    self->m_nentries= 0;
    SET_POS (self, 0);
    err_v_int_ensure_size_sub_aux (self, min);
}

CODE_SECTION(".text.clear_flags")
void err_v_int_clear_flags
        (err_v_int_t *self, E_BOOL resize, E_BOOL del)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
        return;
    ...
    self->m_nentries= 0;
    SET_POS (self, 0);
    if (resize) err_v_int_ensure_size_sub_aux (self, 0);
}

CODE_SECTION(".text.clear_no_resize")
void err_v_int_clear_no_resize
        (err_v_int_t *self)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
...
o_type_var_t err_v_int_modify
        (err_v_int_t *self, err_v_int_index_t key, int  value)
{
    int copyerror= E_FALSE;

    return_val_if_null_or_bad (self, ZEROVAR(self));

#if ERR_V_INT_ALLOW_OUTOFRANGE

    err_v_int_if_outofrange_min (key < 0, key) {
        FATAL_ERROR_MIN (E_OUTOFRANGE, return ZEROVAR(self), "index out of range (1)", key);
    }

    if (ERWIN_UNLIKELY(!err_v_int_grow_plus (self, key, 1)))
        return ZERO(self);

#else
    err_v_int_if_outofrange_minmax (
        key < 0 || key >= (err_v_int_cnt_t)self->m_nentries, key, self->m_nentries)
    {
...
void err_v_int_swap
        (err_v_int_t *self, err_v_int_index_t key1, err_v_int_index_t key2)
/* table==NULL is ok */
{
    int help;
...
#if ERR_V_INT_ALLOW_OUTOFRANGE
    err_v_int_if_outofrange_min (key1 < 0, key1) {
        FATAL_ERROR_MIN (E_OUTOFRANGE, return, "index out of range", key1);
    }
    err_v_int_if_outofrange_min (key2 < 0, key2) {
        FATAL_ERROR_MIN (E_OUTOFRANGE, return, "index out of range", key2);
    }

    if (ERWIN_UNLIKELY(!err_v_int_grow_plus (self, key1, 1) || !err_v_int_grow_plus (self, key2, 1)))
        return;
#else
    err_v_int_if_outofrange_minmax (
        key1 < 0 || key1 >= (err_v_int_cnt_t)self->m_nentries, key1, self->m_nentries)
    {
        FATAL_ERROR (E_OUTOFRANGE, return, "index out of range",
            key1, self->m_nentries);
    }
    err_v_int_if_outofrange_minmax (
        key2 < 0 || key2 >= (err_v_int_cnt_t)self->m_nentries, key2, self->m_nentries)
    {
        FATAL_ERROR (E_OUTOFRANGE, return, "index out of range",
    ...
    ERR_V_INT_UPDATE_POS_SET2 (self, key1, key1, key2, key2);
}

CODE_SECTION(".text.reverse")
void err_v_int_reverse
        (err_v_int_t *self)
{
    int help;
    err_v_int_index_t i;
    err_v_int_index_t n1;
    err_v_int_index_t n_2;
    ...
    for (i=0; i < n_2; i++) {
        ...
        ERR_V_INT_UPDATE_POS_SET2 (self, i, i, n1-i, n1-i);
    }
}

CODE_SECTION(".text.init_iterator")
void err_v_int_init_iterator
        (err_v_int_t const *self, err_v_int_index_t *key)
{
    (void)self; /*unused*/
...
E_BOOL err_v_int_next_iteration
        (err_v_int_t const *self, err_v_int_index_t *key, int *valuep)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
        return E_FALSE;
    ...
    (*key)++;
    if (ERWIN_UNLIKELY(*key < 0 || *key >= (err_v_int_cnt_t)self->m_nentries))
        return E_FALSE;
...
E_BOOL err_v_int_next_iteration_ptr
        (err_v_int_t *self, err_v_int_index_t *key, int **valuepp)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
        return E_FALSE;
...
E_BOOL err_v_int_next_iteration_ptr_const
        (err_v_int_t const *self, err_v_int_index_t *key, int const **valuepp)
{
    return err_v_int_next_iteration_ptr((err_v_int_t *)self, key, (int **)valuepp);
}

CODE_SECTION(".text.init_iterator_reverse")
void err_v_int_init_iterator_reverse
        (err_v_int_t const *self, err_v_int_index_t *key)
{
    return_if_null_pp (key, self);
...
E_BOOL err_v_int_next_iteration_reverse
        (err_v_int_t const *self, err_v_int_index_t *key, int *valuep)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
...
E_BOOL err_v_int_next_iteration_ptr_reverse
        (
    err_v_int_t *self, err_v_int_index_t *key, int **valuepp)
{
#if ERR_V_INT_ALLOW_NULL
...
E_BOOL err_v_int_next_iteration_ptr_const_reverse
        (err_v_int_t const *self, err_v_int_index_t *key, int const **valuepp)
{
    return err_v_int_next_iteration_ptr_reverse((err_v_int_t *)self, key, (int **)valuepp);
}


CODE_SECTION(".text.set_size")
int err_v_int_set_size
        (err_v_int_t *self, err_v_int_cnt_t newsize) /* this is user-proof. */
{
    ...
    if (!err_v_int_ensure_size_add_aux (self, newsize) ||
        !err_v_int_ensure_size_sub_aux (self, newsize))
    {
        return e_errno;
    }
    ...
    err_v_int_clear_no_resize_aux (self, newsize); /* in case of shrinking */
    err_v_int_init_tail (self, newsize);           /* in case of growth */
...
int err_v_int_set_size_with
        (err_v_int_t *self, err_v_int_cnt_t newsize, int  elem)
{
    return_val_if_null2i_or_bad (self, ASSERTION_FAILED);

    if (newsize > (err_v_int_cnt_t)self->m_nentries)
        return err_v_int_ensure_size_with (self, newsize, elem);
    else
        return err_v_int_set_size (self, newsize);
}

CODE_SECTION(".text.set_size_no_resize")
int err_v_int_set_size_no_resize
        (err_v_int_t *self, err_v_int_cnt_t newsize) /* this is user-proof. */
{
    return_val_if_null2i_or_bad (self, ASSERTION_FAILED);
...
void err_v_int_shrink
        (err_v_int_t *self, E_BOOL tight)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL) {
        SET_ERRNO(E_OK);
    ...
    if (tight) {
        err_v_int_cnt_t ts= ERR_V_INT_GET_TABLESIZE (self);
        return_if_fail_ppii ((err_v_int_cnt_t)self->m_nentries <= ts,
            self, self->m_nentries, ts);

        if (ERWIN_LIKELY((err_v_int_cnt_t)self->m_nentries < ts)) {
            err_v_int_cnt_t newsize= CLAMP_MINIMAL(self->m_nentries);
            ERR_V_INT_ALIGN_TABLESIZE (newsize);
            if (!err_v_int_reallocate (self, newsize))
                return;
        }
    }
    else
    if (!err_v_int_ensure_size_sub_aux_force (self, self->m_nentries))
        return;
#endif
...
int err_v_int_ensure_table_size
        (err_v_int_t *self, err_v_int_cnt_t newsize)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
...
#if !ERR_V_INT_LOW_MEM
    if (newsize > 0 && newsize > (err_v_int_cnt_t)self->m_nentries) {
        if (!err_v_int_ensure_size_add_aux (self, newsize))
            return e_errno;
    }
#endif
...
int err_v_int_append
        (err_v_int_t *self, int  newvalue)
{
    int copyerror= E_FALSE;

    return_val_if_null2i_or_bad (self, ASSERTION_FAILED);

    if (!err_v_int_ensure_size_add_plus (self, self->m_nentries, 1))
        return e_errno;

    {
        ...
        if (ERWIN_LIKELY(copyerror == E_FALSE)) {
            ...
            ERR_V_INT_UPDATE_POS_INSERT (self, self->m_nentries-1);
            return PASS_ERRNO(E_OK);
        }
        else {
            VECTOR_NOMEM;
...
void err_v_int_make_gap_aux
        (err_v_int_t *self, err_v_int_index_t start, err_v_int_cnt_t count)
{
    err_v_int_cnt_t i;
...
#if ERR_V_INT_ALLOW_OUTOFRANGE
    err_v_int_if_outofrange_min (count < 0, count) {
        SET_ERRNO(E_OUTOFRANGE);
        return;
    }
    err_v_int_if_outofrange_min (start < 0, start) {
        SET_ERRNO(E_OUTOFRANGE);
        return;
    }

    if (count > 0 && !err_v_int_grow_plus (self, start, 1))
        return;
    ...
    err_v_int_if_outofrange_minmax (start < 0 || start > nentries, start, nentries) {
        SET_ERRNO(E_OUTOFRANGE);
        return;
    }
#endif

    if (!err_v_int_ensure_size_add_plus (self, nentries, count))
        return;

    /* Copy elements up */
    for (i=0; i < nentries - start; i++) {
        E_SET_VALUE (TABLE(self) [nentries + count - 1 - i], TABLE(self) [nentries - 1 - i]);
        ERR_V_INT_UPDATE_POS_SET (self, nentries - 1 - i);
    }
...
int err_v_int_make_gap_with
        (err_v_int_t *self, err_v_int_index_t start, int  elem, err_v_int_cnt_t count)
{
    err_v_int_cnt_t i;
    int    copyerror= E_FALSE;
    ...
    err_v_int_make_gap_aux (self, start, count); /* updates POS */
    ...
    for (i=0; i < count; i++) {
        ...
        if (ERWIN_UNLIKELY(copyerror != E_FALSE)) {  /* reset to sane state */
            VECTOR_NOMEM;
            return err_v_int_erase_flags_q (self, start, count, E_FALSE, E_FALSE);
            /* FIXME: memory leak.  The correct behaviour would be to
             *        clear the non-filled positions and then to
             *        deallocate. */
        }
        ERR_V_INT_UPDATE_POS_SET (self, start + i);
    }
        ...
        (err_v_int_t *self, err_v_int_index_t start, err_v_int_cnt_t count)
{
    ...
    err_v_int_make_gap_aux (self, start, count);
    ...
    E_BOOL docopy)
{
    ...
    if (start >= (err_v_int_cnt_t)other->m_nentries)
        count= 0;
    else
    if (start + count > (err_v_int_cnt_t)other->m_nentries)
        count= other->m_nentries - start;
    ...
    err_v_int_if_outofrange_minmax (
        (start + count) > (err_v_int_cnt_t)other->m_nentries, start + count, other->m_nentries)
    {
        return PASS_ERRNO(E_OUTOFRANGE);
    }
#endif
    ...
    if (count > 0) {
        return_val_if_null_or_bad (self, ASSERTION_FAILED);
        if (docopy)
            return err_v_int_insert_raw (self, self_start, TABLE(other) + start, count);
        else
            return err_v_int_insert_no_copy (self, self_start, TABLE(other) + start, count);
    }
                            ...
                            E_BOOL docopy)
{
    ...
    pre_alloc= (err_v_int_cnt_t)self->m_nentries - start;
    if (pre_alloc < 0)
        pre_alloc= 0;
...
#if ERR_V_INT_DYN_ZERO
    E_SET_PTR(result, err_v_int_new_with_zero_and_initial_size (ZERO(self), pre_alloc));
#else
    E_SET_PTR(result, err_v_int_new_with_initial_size (pre_alloc));
#endif
    if (ERWIN_UNLIKELY(
           err_v_int_insert_subvector (result, 0, self, start, count, docopy) != E_OK))
    {
        err_v_int_delete (result);
        return NULL;
    }
...
int err_v_int_overwrite_copy_aux_q
        (
        err_v_int_t *self,
        err_v_int_index_t       start_self,
        int const *other_table,
        err_v_int_cnt_t       count,
        E_BOOL dealloc)
{
    err_v_int_cnt_t i;
    ...
    err_v_int_if_outofrange_min (count < 0, count)
        return PASS_ERRNO(E_OUTOFRANGE);
    err_v_int_if_outofrange_min (start_self < 0, start_self)
        return PASS_ERRNO(E_OUTOFRANGE);

    if (!err_v_int_grow_plus (self, start_self, 1) ||
        !err_v_int_grow_plus (self, start_self, count))
    {
        return e_errno;
    }

#else
    err_v_int_if_outofrange_minmax (count < 0 ||
        start_self < 0 ||
        (err_v_int_cnt_t)self->m_nentries - start_self < (err_v_int_cnt_t)count ||
        start_self >= (err_v_int_cnt_t)self->m_nentries, start_self, self->m_nentries)
        return PASS_ERRNO(E_OUTOFRANGE);
#endif
    ...
    for (i=0; i < count; i++) {
        ...
        ERR_V_INT_UPDATE_POS_SET (self, start_self + i);

        if (ERR_V_INT_EQ_USE_ARG(dealloc)) {
            OFREE (old);
        }
    }
...
int err_v_int_overwrite_no_copy_aux_q
        (
    err_v_int_t       *self,
    err_v_int_index_t  start_self,
    int const *other_table,
    ...
    E_BOOL dealloc)
{
    ...
    err_v_int_if_outofrange_minmax (
        count < 0 ||
           start_self < 0 ||
           (err_v_int_cnt_t)self->m_nentries - start_self < count ||
           start_self >= (err_v_int_cnt_t)self->m_nentries,
        start_self,
        self->m_nentries)
    {
        return PASS_ERRNO(E_OUTOFRANGE);
...
int err_v_int_overwrite_raw
        (err_v_int_t *self,
                          ...
                          err_v_int_cnt_t       count)
{
    return err_v_int_overwrite_copy_aux_q (
            self,
            start_self,
            other_table,
            count,
...
int err_v_int_overwrite_string
        (
    err_v_int_t *self,
    err_v_int_index_t       start_self,
    int const *other_table)
{
    ...
    return err_v_int_overwrite_copy_aux_q (self,
                                 start_self,
                                 other_table,
                                 err_v_int_string_length (self, other_table),
                                 E_TRUE);
}
...
{
    return copy ?
         err_v_int_overwrite_copy_aux_q (self, start_self, other_table, count, del)
       : err_v_int_overwrite_no_copy_aux_q (
                 self,
                 start_self,
                 ERWIN_SUPERFLUOUS_CAST (int const*, other_table),
                 count,
                 del);
}
...
int err_v_int_overwrite
        (err_v_int_t *self,
                      err_v_int_index_t       start_self,
                      err_v_int_t const *other,
                      err_v_int_index_t       start_other,
                      err_v_int_cnt_t       maxcount)
{
    return_val_if_null2i_or_bad (other, ASSERTION_FAILED);
     ...
     */
    if (maxcount > (err_v_int_cnt_t)other->m_nentries - start_other)
        maxcount= (err_v_int_cnt_t)other->m_nentries - start_other;

    /*
     * Errors not handled by err_v_int_overwrite_raw
     */
#if ERR_V_INT_ALLOW_OUTOFRANGE
    err_v_int_if_outofrange_min (start_other < 0, start_other)
        return PASS_ERRNO(E_OUTOFRANGE);

    if (start_other >= (err_v_int_cnt_t)other->m_nentries)
        return PASS_ERRNO(E_OK);
    ...
    err_v_int_if_outofrange_minmax (
        start_other < 0 ||
            start_other >= (err_v_int_cnt_t)other->m_nentries,
        start_other,
        other->m_nentries)
        return PASS_ERRNO(E_OUTOFRANGE);
#endif
     ...
     * Do it
     */
    return err_v_int_overwrite_raw (
               self,
               start_self,
...
err_v_int_cnt_t err_v_int_string_length_aux
        (err_v_int_t const *self, int const *values)
{
    err_v_int_cnt_t count;
...
err_v_int_cnt_t err_v_int_string_length
        (err_v_int_t const *self, int const *values)
{
    return err_v_int_string_length_aux (self, values);
}
...
int err_v_int_insert_raw
        (err_v_int_t *self, err_v_int_index_t start, int const *values, err_v_int_cnt_t count)
{
    /* return_val_if_null2 (self, values); -- already done by callees */

    if (ERWIN_UNLIKELY(err_v_int_make_gap (self, start, count) != E_OK))
        return e_errno;

    err_v_int_overwrite_raw (self, start, values, count);
        ...
        (err_v_int_t *self, err_v_int_index_t start, int const *values, err_v_int_cnt_t count)
{
    ...
    err_v_int_overwrite_no_copy_aux_q (self, start, values, count, E_FALSE);
...
int err_v_int_insert_vector
        (err_v_int_t *self, err_v_int_index_t start, err_v_int_t const *other)
{
    return_val_if_null2i_or_bad (other, ASSERTION_FAILED);
    return err_v_int_insert_raw (self, start, TABLE(other), (err_v_int_cnt_t)other->m_nentries);
}
...
int err_v_int_insert
        (err_v_int_t *self, err_v_int_index_t start, int  value)
{
    return err_v_int_insert_raw (self, start, &value, 1);
}
...
int err_v_int_insert_string
        (err_v_int_t *self, err_v_int_index_t start, int const *values)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);

    return err_v_int_insert_raw (
               self,
               start,
               values,
               err_v_int_string_length_aux (self, values));
}
...
int err_v_int_append_raw
        (err_v_int_t *self, int const *values, err_v_int_cnt_t count)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_v_int_insert_raw (self, (err_v_int_cnt_t)self->m_nentries, values, count);
}
...
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_v_int_insert_no_copy (self, (err_v_int_cnt_t)self->m_nentries, values, count);
}
...
int err_v_int_append_vector
        (err_v_int_t *self, err_v_int_t const *other)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_v_int_insert_vector (self, (err_v_int_cnt_t)self->m_nentries, other);
}
...
int err_v_int_append_string
        (err_v_int_t *self, int const *values)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_v_int_insert_raw (
               self,
               (err_v_int_cnt_t)self->m_nentries,
               values,
               err_v_int_string_length_aux (self, values));
}
        ...
        E_BOOL dealloc)
{
    ...
    if (count < 0)
        count= (err_v_int_cnt_t)self->m_nentries - start;

#if ERR_V_INT_ALLOW_OUTOFRANGE
    if (start >= (err_v_int_cnt_t)self->m_nentries || count <= 0)
        return PASS_ERRNO(E_OK);
    if ((start + count) > (err_v_int_cnt_t)self->m_nentries)
        count= self->m_nentries - start;
#endif

    /* Check that both start and end are within bounds: */
    return_val_if_out_of_bounds_pp (start,         (err_v_int_cnt_t)self->m_nentries, ASSERTION_FAILED, self);
    return_val_if_out_of_bounds_pp (start+count-1, (err_v_int_cnt_t)self->m_nentries, ASSERTION_FAILED, self);

    ERR_V_INT_UPDATE_POS_ERASE_MANY (self, start, start + count);
    ...
    /* Move elements */
    for (i=0; i < (err_v_int_cnt_t)self->m_nentries - start - count; i++) {
        E_SET_VALUE (TABLE(self) [i + start], TABLE(self) [i + start + count]);
        ERR_V_INT_UPDATE_POS_SET (self, i + start);
    }

    /* Resize the table */
    if (ERWIN_UNLIKELY(
            !err_v_int_setsize_noinit_sub(self, (err_v_int_cnt_t)self->m_nentries - count, resize)))
        return e_errno;
...
int err_v_int_erase_flags_q
        ...
        E_BOOL dealloc)
{
    return err_v_int_erase_aux_q (self, start, count, resize, dealloc);
}
...
int err_v_int_erase
        (err_v_int_t *self, err_v_int_index_t start, err_v_int_cnt_t count)
{
    return err_v_int_erase_aux_q (self, start, count, E_TRUE, E_TRUE);
}

/* Cut a portion out of the vector. */
static
CODE_SECTION(".text.swap_erase_aux_q")
int err_v_int_swap_erase_aux_q
        (
        err_v_int_t *self,
        err_v_int_index_t start,
        err_v_int_cnt_t count,
        ...
        E_BOOL dealloc)
{
    ...
    /* Check that both start and end are within bounds: */
    return_val_if_out_of_bounds_pp (start,         (err_v_int_cnt_t)n, ASSERTION_FAILED, self);
    return_val_if_out_of_bounds_pp (start+count-1, (err_v_int_cnt_t)n, ASSERTION_FAILED, self);
    ...
    for (i=0; i < count; i++) {
        err_v_int_cnt_t j_from;
        err_v_int_cnt_t j_to= start + i;
        return_val_if_fail_ppi ((err_v_int_index_t)j_to >= 0, ASSERTION_FAILED, self, j_to);
        if (ERWIN_UNLIKELY(j_to >= end))
            break;

        j_from= self->m_nentries - 1 - i;
        return_val_if_fail_ppi ((err_v_int_index_t)j_from >= 0, ASSERTION_FAILED, self, j_from);
        return_val_if_fail_ppiii ((err_v_int_index_t)j_from >= start + count, ASSERTION_FAILED,
           self, j_from, start, count);

        E_SET_VALUE (TABLE(self) [j_to], TABLE(self)[j_from]);
        ERR_V_INT_UPDATE_POS_SET (self, j_to);
    }
...
int err_v_int_swap_erase_flags_q
        (
        err_v_int_t *self,
        err_v_int_index_t start,
        err_v_int_cnt_t count,
        ...
        E_BOOL dealloc)
{
    return err_v_int_swap_erase_aux_q (self, start, count, resize, dealloc);
}
...
int err_v_int_swap_erase
        (err_v_int_t *self, err_v_int_index_t start, err_v_int_cnt_t count)
{
    return err_v_int_swap_erase_aux_q (self, start, count, E_TRUE, E_TRUE);
}

CODE_SECTION(".text.erase_if_flags_q")
err_v_int_cnt_t err_v_int_erase_if_flags_q
        (
        err_v_int_t *self,
        ...
        E_BOOL dealloc)
{
    ...
    while (rp < cnt) {
        if (value == ERR_ERWIN_TO_BOOL (f(TABLE(self)[rp]))) {
            ERR_V_INT_UPDATE_POS_ERASE (self, rp);
            if (ERR_V_INT_EQ_USE_ARG (dealloc)) {
                OFREE (TABLE(self)[rp]);
            }
        }
            ...
            ERR_V_INT_UPDATE_POS_SET (self, wp);
            wp++;
        }
        rp++;
    }

    err_v_int_setsize_noinit_sub (self, wp, resize);
...
err_v_int_cnt_t err_v_int_erase_if
        (err_v_int_t *self, err_v_int_feature_t f, E_BOOL value)
{
    return err_v_int_erase_if_flags_q (self, f, value, E_TRUE, E_TRUE);
}

CODE_SECTION(".text.erase_zero")
err_v_int_cnt_t err_v_int_erase_zero
        (err_v_int_t *self)
{
    err_v_int_index_t rp, wp;
    err_v_int_cnt_t cnt;
    ...
    err_v_int_setsize_noinit_sub (self, wp, E_TRUE);
...
int err_v_int_chop
        (err_v_int_t *self, err_v_int_cnt_t count)
{
#if ERR_V_INT_ALLOW_NULL
    if (count == 0)
        return PASS_ERRNO(E_OK);
    ...
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_v_int_erase (self, (err_v_int_index_t)self->m_nentries - count, count);
}
...
#endif
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_v_int_erase_flags_q (
            self,
            (err_v_int_cnt_t)self->m_nentries - count,
            count,
            resize,
            dealloc);
}
...
o_type_var_t err_v_int_last_chop1
        (err_v_int_t *self)
{
    int result;
    ...
    err_v_int_erase_aux_q (self, (err_v_int_cnt_t)self->m_nentries - 1, 1, E_TRUE, E_FALSE);
...
o_type_var_t err_v_int_first_swap_chop1
        (err_v_int_t *self)
{
    int result;
    ...
    err_v_int_swap_erase_aux_q (self, 0, 1, E_TRUE, E_FALSE);
       /* resize, but don't delete, since we're going to return the element. */
...
o_type_result_t err_v_int_nth
        (err_v_int_t const*self, err_v_int_index_t idx)
/* table==NULL is ok */
{
    exit_if_null (self);
...
#if ERR_V_INT_ALLOW_OUTOFRANGE || ERR_V_INT_RANGE_CHECK
    if (ERWIN_UNLIKELY(idx < 0 || idx >= (err_v_int_cnt_t)self->m_nentries)) {
# if ERR_V_INT_ALLOW_OUTOFRANGE == 0
        errprint ((errstream, "in err_v_int_nth: index out of range (0 <= %d < %lu failed)\n",
                         idx,
...
o_type_result_t err_v_int_nth_char
        (err_v_int_t const*self, err_v_int_index_t idx)
/* m_table==NULL is ok */
{
    exit_if_null (self);
    ...
    if (ERWIN_UNLIKELY(idx == (err_v_int_cnt_t)self->m_nentries))
        return ZERO(self);
...
o_type_result_t err_v_int_first
        (err_v_int_t const *self)
/* m_table==NULL is ok */
{
    return err_v_int_nth (self, 0);
}
...
o_type_result_t err_v_int_last
        (err_v_int_t const *self)
/* m_table==NULL is ok */
{
    exit_if_null (self);
    ...
    return err_v_int_nth (self, (err_v_int_index_t)(self->m_nentries - 1));
}
...
err_v_int_element_ptr_t err_v_int_nth_ptr
        (err_v_int_t *self, err_v_int_index_t idx)
/* m_table==NULL is ok */
{
    return0_if_null_or_bad (self);
    ...
    if (ERWIN_UNLIKELY(idx >= (err_v_int_cnt_t)self->m_nentries)) {
# if ERR_V_INT_ALLOW_OUTOFRANGE != 0
        if (!err_v_int_grow_plus (self, idx, 1))
            return NULL;
# else
        return NULL;
# endif
    ...
    return ERWIN_SUPERFLUOUS_CAST (err_v_int_element_ptr_t, &TABLE(self)[idx]);
}
...
err_v_int_element_ptr_t err_v_int_nth_ptr_check
        (err_v_int_t *self, err_v_int_index_t idx)
/* m_table==NULL is ok */
{
    ...
    if (ERWIN_UNLIKELY(idx < 0 || idx >= (err_v_int_cnt_t)self->m_nentries)) {
# if ERR_V_INT_ALLOW_OUTOFRANGE != 0
        if (ERWIN_UNLIKELY(!err_v_int_grow_plus (self, idx, 1)))
            return NULL;
# else
        errprint ((errstream, "in err_v_int_nth: index out of range (0 <= %d < %lu failed)\n",
                         idx,
...
/* m_table==NULL is ok */
{
    ...
    if (ERWIN_UNLIKELY(!err_v_int_ensure_size_add_plus (self, self->m_nentries, 1)))
        return NULL;
#endif
...
err_v_int_element_ptr_t err_v_int_first_ptr
        (err_v_int_t *self)
/* m_table==NULL is ok */
{
    return0_if_null_or_bad (self);
...
#if ERR_V_INT_ALLOW_OUTOFRANGE || ERR_V_INT_RANGE_CHECK
    if (ERWIN_UNLIKELY(0 >= (err_v_int_cnt_t)self->m_nentries)) {
# if ERR_V_INT_ALLOW_OUTOFRANGE != 0
        if (ERWIN_UNLIKELY(!err_v_int_grow_plus (self, 0, 1)))
            return NULL;
# else
        return NULL;
# endif
    ...
    return_val_if_null_pp (TABLE(self), NULL, self);
    return ERWIN_SUPERFLUOUS_CAST (err_v_int_element_ptr_t, &TABLE(self)[0]);
}
...
err_v_int_element_ptr_t err_v_int_last_ptr
        (err_v_int_t *self)
/* m_table==NULL is ok */
{
    return0_if_null_or_bad (self);
    return err_v_int_nth_ptr (self, self->m_nentries - 1);
}

CODE_SECTION(".text.as_array")
err_v_int_element_ptr_t err_v_int_as_array
        (err_v_int_t const *self_const)
/* self==NULL    is ok: NULL is returned then.
 * m_table==NULL is ok, "" is returned then. */
{
    err_v_int_t *self;
    E_SET_PTR(self, ERWIN_CONST_CAST(err_v_int_t*, self_const));
    if (self == NULL)
        return NULL;
    return0_if_null_or_bad (self);

#if !ERR_V_INT_LOW_MEM
    /* see err_v_int_nth_ptr_char */
    if (ERWIN_UNLIKELY(!err_v_int_ensure_size_add_plus (self, self->m_nentries, 1)))
        return NULL;
#endif
    ...
    return ERWIN_SUPERFLUOUS_CAST (err_v_int_element_ptr_t, TABLE(self));
}

CODE_SECTION(".text.ensure_heap_storage")
void err_v_int_ensure_heap_storage
        (err_v_int_t *self)
{
    (void)self; /*unused*/
...
#if ERR_V_INT_INLINE_STORE
    err_v_int_ensure_table_size_force (self, ERR_V_INT_INLINE_CNT + 1);
#endif
}

CODE_SECTION(".text.has_heap_storage")
ERR_ERWIN_BOOL err_v_int_has_heap_storage
        (err_v_int_t const *self)
{
    (void)self; /*unused*/
...
err_v_int_cnt_t err_v_int_inline_store_cnt
        (void)
{
    return ERR_V_INT_INLINE_CNT;
}

CODE_SECTION(".text.as_array_detach")
err_v_int_element_ptr_t err_v_int_as_array_detach
        (err_v_int_t *self)
{
    err_v_int_element_ptr_t result;

    err_v_int_ensure_heap_storage (self);
    result= err_v_int_as_array (self);
    err_v_int_detach (self);

    return result;
}

CODE_SECTION(".text.as_open_array")
err_v_int_element_ptr_t err_v_int_as_open_array
        (err_v_int_t const *self)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
    ...
    return0_if_null_or_bad (self);
#if ERR_V_INT_ZERO_SIZE
    if (TABLE(self) == ERR_V_INT_DUMMY_TABLE)
        return NULL;
#endif
    return ERWIN_SUPERFLUOUS_CAST (err_v_int_element_ptr_t, TABLE(self));
}
...
err_v_int_element_ptr_t err_v_int_as_open_array_detach
        (err_v_int_t *self)
{
    err_v_int_element_ptr_t result;

    err_v_int_ensure_heap_storage (self);
    result= err_v_int_as_open_array(self);
    err_v_int_detach (self);
...
int err_v_int_element_priority_cmp
        (int const *a, int const *b)
{
...
err_v_int_index_t err_v_int_heap_left
        (err_v_int_t const *self, err_v_int_index_t father)
{
    err_v_int_index_t child;
    ...
    return_val_if_out_of_bounds_pp (father, (err_v_int_cnt_t)self->m_nentries, -1, self);

    child= father * 2 + 1;
    if (child < (err_v_int_cnt_t)self->m_nentries)
        return child;
    return -1;
}

CODE_SECTION(".text.heap_right")
err_v_int_index_t err_v_int_heap_right
        (err_v_int_t const *self, err_v_int_index_t father)
{
    err_v_int_index_t child;
...
err_v_int_index_t err_v_int_heap_father
        (err_v_int_t const *self, err_v_int_index_t child)
{
    (void)self; /*unused*/
    ...
    return_val_if_null_or_bad (self, -1);
    return_val_if_out_of_bounds_pp (child, (err_v_int_cnt_t)self->m_nentries, -1, self);
...
void err_v_int_check_heap_property
        (err_v_int_t const *self, err_v_int_cmp_t cmp)
{
#if 0
    err_v_int_size_t father;
#endif
    (void)self; /*unused*/
    (void)cmp;  /*unused*/
#if 0
    for (father=0; father < self->m_nentries; father++) {
        err_v_int_index_t child= err_v_int_heap_left (self, father);
        if (child >= 0) {
            return_if_fail_ppt (
                cmp (err_v_int_nth_ptr_const (self, father), err_v_int_nth_ptr_const (self, child)) >= 0,
                self,
                "The heap property is violated.");
        }
        child= err_v_int_heap_right (self, father);
        if (child >= 0) {
            return_if_fail_ppt (
                cmp (err_v_int_nth_ptr_const (self, father), err_v_int_nth_ptr_const (self, child)) >= 0,
                self,
...
void err_v_int_heap_sink
        (err_v_int_t *self, err_v_int_index_t father, err_v_int_cmp_t cmp)
{
    return_if_null_or_bad (self);
    return_if_out_of_bounds_pp (father, (err_v_int_cnt_t)self->m_nentries, self);

#ifdef OPRIORITY_CMP
    if (cmp == NULL)
        cmp= err_v_int_element_priority_cmp;
#endif
    return_if_null_pp (cmp, self);

    /* BEGIN MAIN */
    for (;;) {
        err_v_int_index_t child, largest;

        largest= father;

        child= err_v_int_heap_left (self, father);
        if (child >= 0) {
            if (cmp (err_v_int_nth_ptr (self, child), err_v_int_nth_ptr (self, largest)) > 0)
                largest= child;

            child= err_v_int_heap_right (self, father);
            if (child >= 0 &&
                cmp (err_v_int_nth_ptr (self, child), err_v_int_nth_ptr (self, largest)) > 0)
                largest= child;
        }
        ...
        err_v_int_swap (self, largest, father);
        father= largest;
    }
    /* END MAIN */
}
...
void err_v_int_make_heap
        (err_v_int_t *self, err_v_int_cmp_t cmp)
{
    err_v_int_index_t largest_father;
    err_v_int_index_t i;
    ...
    /* BEGIN MAIN */
    largest_father= err_v_int_heap_father (self, self->m_nentries - 1);
    for (i= largest_father; i >= 0; i--)
        err_v_int_heap_sink (self, i, cmp);
    /* END MAIN */

#ifndef NDEBUG
    err_v_int_check_heap_property (self, cmp);
#endif
}
...
void err_v_int_check_sort_order
        (err_v_int_t const *self, err_v_int_cmp_t cmp)
{
    err_v_int_index_t i;
    for (i=0; i < (err_v_int_index_t)self->m_nentries - 1; i++) {
        return_if_fail_ppt (
            cmp (err_v_int_nth_ptr_const (self, i), err_v_int_nth_ptr_const (self, i+1)) <= 0,
            self,
        ...
        (err_v_int_t *self, err_v_int_cmp_t cmp)
{
    ...
    /* BEGIN MAIN */
    err_v_int_make_heap (self, cmp);

    n= self->m_nentries;
    for (i=n-1; i >= 1; i--) {
         err_v_int_swap (self, 0, i);
         self->m_nentries= i;
         err_v_int_heap_sink (self, 0, cmp);
    }
...
#ifndef NDEBUG
    err_v_int_check_sort_order (self, cmp);
#endif
}

CODE_SECTION(".text.heap_extract")
o_type_var_t err_v_int_heap_extract
        (err_v_int_t *self, err_v_int_cmp_t cmp)
{
    int result;
...
#if !defined(NDEBUG) && ERR_V_INT_DEBUG_EXPENSIVE_CHECKS
    err_v_int_check_heap_property (self, cmp);
#endif
    ...
    /* BEGIN MAIN */
    result= err_v_int_first_swap_chop1 (self);
    if (self->m_nentries > 0)
        err_v_int_heap_sink (self, 0, cmp);
    /* END MAIN */
...
void err_v_int_heap_raise
        (err_v_int_t *self, err_v_int_index_t child, err_v_int_cmp_t cmp)
{
    return_if_null_or_bad (self);
    return_if_out_of_bounds_pp (child, (err_v_int_cnt_t)self->m_nentries, self);
    ...
    /* BEGIN MAIN */
    for (;;) {
        err_v_int_index_t father= err_v_int_heap_father (self, child);
        if (ERWIN_UNLIKELY(father < 0))
            break;

        if (cmp (err_v_int_nth_ptr (self, father), err_v_int_nth_ptr (self, child)) >= 0)
            break;

        err_v_int_swap (self, father, child);
        child= father;
    }
    /* END MAIN */
...
void err_v_int_heap_fix
        (err_v_int_t *self, err_v_int_index_t father, err_v_int_cmp_t cmp)
{
    err_v_int_heap_sink  (self, father, cmp);
    err_v_int_heap_raise (self, father, cmp);
}

CODE_SECTION(".text.heap_insert")
int err_v_int_heap_insert
        (err_v_int_t *self, int  x, err_v_int_cmp_t cmp)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);

#ifdef OPRIORITY_CMP
    if (cmp == NULL)
        E_SET_PTR (cmp, err_v_int_element_priority_cmp);
#endif
    ...
    /* BEGIN MAIN */
    if (ERWIN_LIKELY(err_v_int_append (self, x) == E_OK))
        err_v_int_heap_raise (self, self->m_nentries - 1, cmp);
    /* END MAIN */
...
void err_v_int_heap_erase
        (err_v_int_t *self, err_v_int_index_t i, err_v_int_cmp_t cmp)
{
    return_if_null_or_bad (self);
    return_if_out_of_bounds_pp (i, (err_v_int_cnt_t)self->m_nentries,self);
    ...
    /* BEGIN MAIN */
    err_v_int_swap_erase (self, i, 1);
    if (i < (err_v_int_cnt_t)self->m_nentries)
        err_v_int_heap_sink  (self, i, cmp);
    /* END MAIN */
...
err_v_int_cnt_t err_v_int_erase_equals
        (err_v_int_t *self, err_v_int_cmp_t cmp, err_v_int_combine_t combine)
{
    err_v_int_index_t rp, wp;
    err_v_int_cnt_t cnt;
#if ERR_V_INT_ALLOW_NULL
...
#ifdef OCMP
    if (cmp == NULL)
        E_SET_PTR (cmp, err_v_int_element_cmp);
#endif
    return_val_if_null2i_or_bad (self, ASSERTION_FAILED);
        ...
        (err_v_int_t *self, err_v_int_cmp_t cmp)
{
    ...
    ERR_V_INT_UPDATE_POS_SET_MANY (self, 0, self->m_nentries);
...
err_v_int_index_t err_v_int_bfind
        (err_v_int_t const*self, int  which, err_v_int_cmp_t cmp)
{
    int *result;
...
err_v_int_index_t err_v_int_find
        (err_v_int_t const *self, err_v_int_index_t start, int  needle)
{
    err_v_int_index_t i;
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
    ...
    for (i=start; i < (err_v_int_cnt_t)self->m_nentries; i++)
        ...
        (err_v_int_t const *self, err_v_int_index_t start, int  needle)
{
    ...
    for (i=start; i < (err_v_int_cnt_t)self->m_nentries; i++)
        if (OEQUAL ((TABLE(self)[i]), (needle)))
            return ERWIN_SUPERFLUOUS_CAST (err_v_int_element_ptr_t, &TABLE(self)[i]);
    return NULL;
}
        ...
        (err_v_int_t const *self, err_v_int_index_t start, int  needle)
{
    ...
    if (ERWIN_UNLIKELY(start >= (err_v_int_cnt_t)self->m_nentries))
        return -1;
...
err_v_int_index_t err_v_int_find_if
        (err_v_int_t const *self, err_v_int_index_t start, err_v_int_feature_t feature, E_BOOL value)
{
    err_v_int_index_t i;
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
...
E_BOOL err_v_int_is_equal_at
        (err_v_int_t const *self, err_v_int_index_t start, int const *needle, err_v_int_cnt_t len)
{
#if ERR_V_INT_ALLOW_NULL
    ...
    if (ERWIN_UNLIKELY(start < 0 || start+len > (err_v_int_cnt_t)self->m_nentries))
        return E_FALSE;
        ...
        (err_v_int_t const *self, err_v_int_index_t start, int const *needle, err_v_int_cnt_t len)
{
    ...
    while (noch > 0) {
        err_v_int_index_t i= err_v_int_find (self, start, *needle);

        if (i >= 0) {
            if (err_v_int_is_equal_at (self, i, needle, len))
                return i;
            start= i + 1;
            noch= self->m_nentries - start - len + 1;
        }
...
err_v_int_index_t err_v_int_rfind_raw
        (err_v_int_t const *self, err_v_int_index_t start, int const *needle, err_v_int_cnt_t len)
{
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
    ...
    while (start >= 0) {
        err_v_int_index_t i= err_v_int_rfind (self, start, *needle);
...
void err_v_int_ltrim_if
        (err_v_int_t *self, err_v_int_feature_t feature, E_BOOL value)
{
    err_v_int_size_t count;
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
        return;
#endif
    ...
    if (count != 0)
        err_v_int_erase (self, 0, count);
}

CODE_SECTION(".text.rtrim_if")
void err_v_int_rtrim_if
        (err_v_int_t *self, err_v_int_feature_t feature, E_BOOL value)
{
    err_v_int_size_t count;
    ...
    if (count != 0)
        err_v_int_chop (self, count);
}

CODE_SECTION(".text.trim_if")
void err_v_int_trim_if
        (err_v_int_t *self, err_v_int_feature_t feature, E_BOOL value)
{
    err_v_int_rtrim_if (self, feature, value);
    err_v_int_ltrim_if (self, feature, value);
}

CODE_SECTION(".text.map")
void err_v_int_map
        (err_v_int_t *self, err_v_int_map_t map)
{
    err_v_int_size_t count;
#if ERR_V_INT_ALLOW_NULL
    if (self == NULL)
    ...
    count= 0;
    while (count < self->m_nentries) {
        err_v_int_set (self, count, (*map) (TABLE(self)[count]));
        count++;
    }      
}
...
int err_v_int_cmp
        (err_v_int_t const *a, err_v_int_t const *b, err_v_int_cmp_t cmp)
{
    int const *alauf;
    int const *blauf;
    err_v_int_cnt_t count;
...
#if !ERR_V_INT_COMPARE_LEXICOGRAPHICALLY
...
#if !ERR_V_INT_PRIORITY_COMPARE_LEXICOGRAPHICALLY
    if (a->m_nentries > b->m_nentries)
        return 1;
...
e_hashval_t err_v_int_hash_raw
        (err_v_int_t const *self)
{
#ifdef OHASH
    int const *alauf;
    ...
    error_return0_if_reached_ppt (
        self,
        "Because ERR_INT_HASH was not #defined, err_v_int_hash_raw could not be implemented.");
#endif
}
...
E_BOOL err_v_int_locate
        (err_v_int_index_t *idx,
                    err_v_int_t const *self,
                    int  element,
                    err_v_int_cmp_t cmp,
                    int how)
/* table==NULL is ok */
{
    ...
    if (a < b) {
        ...
        do {
            ...
            if (c >= (err_v_int_cnt_t)self->m_nentries)
                b= c;  /* last iteration (in some cases), set up b appropriately */
            else {
                err_v_int_index_t result= cmp (&TABLE(self)[c], &element);
                ...
                if (result < 0) a= c+1; /* search in right half */
                    ...
                    if (how > 0) { /* return right most equal element */
                        while (c < ((err_v_int_cnt_t)self->m_nentries - 1) &&
                               cmp (&TABLE(self)[c+1], &element) == 0)
                            c++;
                    }
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (void)
{
    E_BOOL ok= err_v_int_init_internal_q (it(), OZERO, GET_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_ppt (ok, this, "err_v_int_init_internal_q failed");
}

CODE_SECTION(".text._constructor")
void err_v_int_t::_constructor
        (void)
{
    E_BOOL ok= err_v_int_init_internal_q (it(), OZERO, GET_INITIAL_SIZE);
    (void)ok; /*unused*/
...
err_v_int_t const & err_v_int_t::static_zero
        ()
{
    static err_v_int_t const *z= NULL;
    if (ERWIN_UNLIKELY(!z))
        E_SET_PTR(z, new err_v_int_t);
    return *z;
}
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (err_v_int_cnt_t initial_size)
{
    if (initial_size < 0)
        initial_size= GET_INITIAL_SIZE;
                                                        
    E_BOOL ok= err_v_int_init_internal_q (it(), OZERO, initial_size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* ERR_V_INT_HAVE_INT_CONSTRUCTOR */
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (int  z)
{
    E_BOOL ok= err_v_int_init_internal_q (it(), z, GET_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (int  z, err_v_int_cnt_t initial_size)
{
    if (initial_size < 0)
        initial_size= GET_INITIAL_SIZE;

    E_BOOL ok= err_v_int_init_internal_q (it(), z, initial_size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* ERR_V_INT_DYN_ZERO */
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (err_v_int_t const &other, bool docopy)
{
    ...
    if (TABLE(&other) == NULL && other.m_nentries == 0) {
        ...
        err_v_int_init_internal_q (it(), OZERO, GET_INITIAL_SIZE);
    }
    else
#endif
    {
        err_v_int_init_internal_q (it(), ZERO(other.it()), COPY_SIZE(other.it()));
        if (docopy)
            err_v_int_insert_vector (it(), 0, other.it());
        else
            err_v_int_insert_no_copy (it(), 0, TABLE(&other), other.m_nentries);
    }
}
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (err_v_int_t const &other, err_v_int_index_t start, err_v_int_cnt_t count, bool docopy)
{
    return_if_null_pp (&other, this);
    err_v_int_init_internal_q (it(), ZERO(other.it()), COPY_SIZE(other.it()));
    err_v_int_insert_subvector (it(), 0, other.it(), start, count, docopy);
}
...
{
    return_if_null_pp (other, this);
    err_v_int_init_internal_q (it(), ZERO(other), COPY_SIZE(other));
    err_v_int_insert_subvector (it(), 0, other, start, count, docopy);
}
        ...
        (err_v_int_t const *other, bool docopy)
{
    ...
    err_v_int_init_internal_q (it(), ZERO(other), COPY_SIZE(other));
    if (docopy)
        err_v_int_insert_vector (it(), 0, other);
    else
        err_v_int_insert_no_copy (it(), 0, TABLE(other), other->m_nentries);
}
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (int const *other, bool docopy)
{
    /* other == NULL is explicitly allowed to represent an empty string. */
    err_v_int_init_internal_q (it(), OZERO, GET_INITIAL_SIZE);
    if (docopy)
        err_v_int_insert_string (it(), 0, other);
    else
        err_v_int_insert_no_copy (it(), 0, other, err_v_int_string_length (it(), other));
}

CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (int const *other, err_v_int_cnt_t n, bool docopy)
{
    /* other == NULL is explicitly allowed if n == 0. */
    err_v_int_init_internal_q (it(), OZERO, GET_INITIAL_SIZE);
    if (docopy)
        err_v_int_insert_raw (it(), 0, other, n);
    else
        err_v_int_insert_no_copy (it(), 0, other, n);
}

CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (bool must_be_true, int *other, err_v_int_cnt_t count, err_v_int_cnt_t alloc)
{
    (void)must_be_true; /*unused*/

    err_v_int_init_from_q (it(), other, count, alloc, OZERO);
    return_if_fail_pp (must_be_true, this);
}

CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (bool must_be_true, err_v_int_t *other)
{
    (void)must_be_true; /*unused*/

    err_v_int_init_from_q (it(),
        TABLE(other),
        other->nentries(),
        other->table_size(),
        other->zero());
...
CODE_SECTION(".text.t")
 err_v_int_t::err_v_int_t
        (bool must_be_true, int *other, err_v_int_cnt_t count, err_v_int_cnt_t alloc, int  z)
{
    (void)must_be_true; /*unused*/

    err_v_int_init_from_q (it(), other, count, alloc, z);
    return_if_fail_pp (must_be_true, this);
}
#endif /* ERR_V_INT_DYN_ZERO */
...
CODE_SECTION(".text.err_v_int_t")
 err_v_int_t::~err_v_int_t
        ()
{
    err_v_int_destroy (it());
}


CODE_SECTION(".text._destructor")
void err_v_int_t::_destructor
        ()
{
    err_v_int_destroy (it());
}
...
void err_v_int_t::operator delete
        (void *p, size_t s)
{
...
err_v_int_t & err_v_int_t::operator=
        (err_v_int_t const &other)
{
    err_v_int_destroy (it());
#ifdef ERWIN_DOS
...
#endif
    {
        err_v_int_init_internal_q (it(), err_v_int_zero (other.it()), GET_INITIAL_SIZE);
        err_v_int_insert_vector (it(), 0, other.it());
    }
    return *this;
}
...
{
    err_v_int_destroy (it());
    err_v_int_init_internal_q (it(), err_v_int_zero (other), GET_INITIAL_SIZE);
    err_v_int_insert_vector (it(), 0, other);
    return *this;
}
#endif /* defined(ERR_ERWIN_COMPAT_2_0_264) */
...
#undef FATAL_ERROR
#undef COMPAT_ERROR
#undef err_v_int_global_last_format
#undef errstream
#undef errprint
#undef PASS1
#undef PASS2
