...
#ifndef ERR_SYMTAB_IMPLEMENTATION
#define ERR_SYMTAB_IMPLEMENTATION
#endif
...
#include "error/err_symtab_d.h"
#include "error/err_symtab.h"
#include "error/err_symtab_i.h"
...
#define IBEING_USED ERR_CHAR_CONST_P_BEING_USED
#endif

#ifdef ERR_CHAR_CONST_P_CMP
#define ICMP ERR_CHAR_CONST_P_CMP
#endif

#ifdef ERR_CHAR_CONST_P_ICOPY
#define ICOPY ERR_CHAR_CONST_P_ICOPY
#endif

#ifdef ERR_CHAR_CONST_P_ICOPY_PROVIDED
#define ICOPY_PROVIDED ERR_CHAR_CONST_P_ICOPY_PROVIDED
#endif

#ifdef ERR_CHAR_CONST_P_DETCMP
#define IDETCMP ERR_CHAR_CONST_P_DETCMP
#endif

#ifdef ERR_CHAR_CONST_P_DETCMP_PROVIDED
#define IDETCMP_PROVIDED ERR_CHAR_CONST_P_DETCMP_PROVIDED
#endif

#ifdef ERR_CHAR_CONST_P_EQUAL
#define IEQUAL ERR_CHAR_CONST_P_EQUAL
#endif

#ifdef ERR_CHAR_CONST_P_EQUAL_PROVIDED
#define IEQUAL_PROVIDED ERR_CHAR_CONST_P_EQUAL_PROVIDED
#endif

#ifdef ERR_CHAR_CONST_P_IFREE
#define IFREE ERR_CHAR_CONST_P_IFREE
#endif

#ifdef ERR_CHAR_CONST_P_IFREE_PROVIDED
#define IFREE_PROVIDED ERR_CHAR_CONST_P_IFREE_PROVIDED
#endif

#ifdef ERR_CHAR_CONST_P_HASH
#define IHASH ERR_CHAR_CONST_P_HASH
#endif

#ifdef ERR_CHAR_CONST_P_PRINT
#define IPRINT ERR_CHAR_CONST_P_PRINT
#endif

#ifdef ERR_CHAR_CONST_P_ZERO
#define IZERO ERR_CHAR_CONST_P_ZERO
#endif

#ifdef ERR_VOID_BEING_USED
#define OBEING_USED ERR_VOID_BEING_USED
#endif

#ifdef ERR_VOID_CMP
#define OCMP ERR_VOID_CMP
#endif

#ifdef ERR_VOID_OCOPY
#define OCOPY ERR_VOID_OCOPY
#endif

#ifdef ERR_VOID_OCOPY_PROVIDED
#define OCOPY_PROVIDED ERR_VOID_OCOPY_PROVIDED
#endif

#ifdef ERR_VOID_DETCMP
#define ODETCMP ERR_VOID_DETCMP
#endif

#ifdef ERR_VOID_DETCMP_PROVIDED
#define ODETCMP_PROVIDED ERR_VOID_DETCMP_PROVIDED
#endif

#ifdef ERR_VOID_EQUAL
#define OEQUAL ERR_VOID_EQUAL
#endif

#ifdef ERR_VOID_OFREE
#define OFREE ERR_VOID_OFREE
#endif

#ifdef ERR_VOID_OFREE_PROVIDED
#define OFREE_PROVIDED ERR_VOID_OFREE_PROVIDED
#endif

#ifdef ERR_VOID_HASH
#define OHASH ERR_VOID_HASH
#endif

#ifdef ERR_VOID_PRINT
#define OPRINT ERR_VOID_PRINT
#endif

#ifdef ERR_VOID_ZERO
#define OZERO ERR_VOID_ZERO
#endif
...
#if defined(VOID_IS_PTR) && VOID_IS_PTR
#  define E_SET_VALUE(X,Y)  E_SET_PTR(X,Y)
#else
#  define E_SET_VALUE(X,Y)  ((X)=(Y))
#endif

#if ERR_SYMTAB_INITIAL_SIZE < 0
#  error ERR_SYMTAB_INITIAL_SIZE < 0
#endif

#if ERR_SYMTAB_MINIMAL_SIZE < 0
#  error ERR_SYMTAB_MINIMAL_SIZE < 0
#endif

#if ERR_SYMTAB_MINIMAL_SIZE == 0
#  define ERR_SYMTAB_ZERO_SIZE         1
#  define ERR_SYMTAB_MINIMAL_SIZE_OR_1 1
#elif ERR_SYMTAB_INITIAL_SIZE == 0
#  define ERR_SYMTAB_ZERO_SIZE         1
#  define ERR_SYMTAB_MINIMAL_SIZE_OR_1 ERR_SYMTAB_MINIMAL_SIZE
#else
#  define ERR_SYMTAB_ZERO_SIZE         0
#  define ERR_SYMTAB_MINIMAL_SIZE_OR_1 ERR_SYMTAB_MINIMAL_SIZE
#endif

#if ERR_SYMTAB_ZERO_SIZE

#  define HAS_HEAP_TABLE_AUX(hashsize)    ((hashsize) > 1)

#  define GET_SLOT(table,hashsize,i)      (HAS_HEAP_TABLE_AUX(hashsize) ? \
                                              (table[(i)])                \
                                            : *((err_symtab_hashlist_t**)(&(table)))     \
                                          )
...
#if defined(CHAR_CONST_P_IS_PTR) && defined(VOID_IS_PTR) && !CHAR_CONST_P_IS_PTR && !VOID_IS_PTR
...
#  else
#      define e_errno (ERWIN_CONST_CAST(err_symtab_t*,self)->m_errno)
#  endif
#else
...
#  define return_if_bad(self) \
      return_if_fail_ppxxt(self->m_sig == ERR_SYMTAB_SIG, self, self->m_sig, ERR_SYMTAB_SIG, "Structure was probably overwritten");
#  define return_val_if_bad(self, val) \
      return_val_if_fail_ppxxt(self->m_sig == ERR_SYMTAB_SIG, val, self, self->m_sig, ERR_SYMTAB_SIG, "Structure was probably overwritten");
#  define return_if_bad_pC(self, prog) \
      return_if_fail_ppxxtC(self->m_sig == ERR_SYMTAB_SIG, self, self->m_sig, ERR_SYMTAB_SIG, "Structure was probably overwritten", prog);
#else
#  define return_if_bad(self)
#  define return_val_if_bad(self, val)
#  define return_if_bad_pC(self, prog)
...
#define USER_ASSERTION_FAILED_HANDLER(FI,L,FU,A)    \
            ASSERTION_FAILED;                       \
            ERR_SYMTAB_ASSERTION_FAILED_HANDLER(FI,L,FU,A)
#endif
...
#define TOOSMALL(X,Y) (((X) * ERR_SYMTAB_TRIGGER_DENOMINATOR) > ((Y) * ERR_SYMTAB_TRIGGER_NUMERATOR))

#define TOOLARGE(X,Y) (((Y) >= ERR_SYMTAB_MINIMAL_SIZE) && \
                       (((X) * ERR_SYMTAB_TRIGGER_DENOMINATOR) < (((Y) * ERR_SYMTAB_TRIGGER_NUMERATOR)/4)))
...
#define ITER               ((err_symtab_internal_Iterator*)iter)
#define SORTITER           ((err_symtab_internal_sorted_Iterator*)iter)
#define SORTITERPTR        ((err_symtab_internal_sorted_ptr_Iterator*)iter)

#ifdef E_PROFILE
typedef union {
   err_symtab_t       *mapt;
   err_symtab_t const *mapconstt;
} hack_no_const;
#endif
 ...
 *    err_symtab_index_t == i_type_t
 *    err_symtab_value_t == o_type_t
 */
typedef void       o_type_t;
typedef ERR_ERWIN_BOOL o_type_result_t;
typedef void    o_type_var_t;
typedef char const *       i_type_t;
typedef char const *  i_type_result_t;

#define SPLIT_SECTIONS ERR_SYMTAB_SPLIT
...
/* Check that ERR_SYMTAB_ALLOW_NULL and ERR_ERWIN_THREAD_SAFE are not used at
...
#if 0
/* FIXME: NYI */
#if ERR_SYMTAB_ALLOW_NULL && defined(ERR_ERWIN_THREAD_SAFE)
#  error "Cannot use ERR_SYMTAB_ALLOW_NULL and ERR_ERWIN_THREAD_SAFE at the same time."
#endif
#endif

#if ERR_SYMTAB_NOMEM_IS_FATAL
...
#if ERR_SYMTAB_DYN_ZERO
#  define err_symtab_init_internal_q(SE,Z,SI)  err_symtab_init_internal(SE,Z,SI)
#else
#  define err_symtab_init_internal_q(SE,Z,SI) err_symtab_init_internal(SE,SI)
#endif



#  define feature_q(K,V)                feature(K)
#  define err_symtab_hashlist_new_q(K,V)               err_symtab_hashlist_new(K)
#  define err_symtab_find_any_pair_q(K,V,SE)   err_symtab_find_any_pair(K,SE)
#  define err_symtab_insert_q(SE,K,V)          err_symtab_insert(SE,K)

#  define err_symtab_insert_no_copy_no_rehash_q(SE,K,V) \
                                        err_symtab_insert_no_copy_no_rehash(SE,K)

#  define err_symtab_poke_internal_q(KO,VO,SE,K,KC,DK,V,VK,DV,I,O) \
                                        err_symtab_poke_internal(KO,SE,K,KC,DK,I)

#  define err_symtab_poke_q(KO,VO,SE,K,V,I,O)  err_symtab_poke(KO,SE,K,I)

#  define err_symtab_poke_no_icopy_q(KO,VO,SE,K,V,I,O)  \
                                        err_symtab_poke_no_icopy(KO,SE,K,I)

#  define err_symtab_remove_internal_no_resize_q(SE,K,R) \
                                        err_symtab_remove_internal_no_resize(SE,K)

#  define err_symtab_remove_internal_q(SE,K,R) err_symtab_remove_internal(SE,K)
...
static
TYPE_STRUCT_T(err_symtab_hashlist_t)=
    TYPE_STRUCT_BEGIN(err_symtab_hashlist_t)
        TYPE_STRUCT_SLOT(err_symtab_hashlist_t, char_p, key)
        TYPE_STRUCT_SLOT(err_symtab_hashlist_t, err_symtab_hashlist_t_p,        next)
    TYPE_STRUCT_END
;


TYPE_INFO_T(err_symtab_hashlist_t)= TYPE_INFO_STRUCT (err_symtab_hashlist_t, err_symtab_hashlist_t);


TYPE_INFO_T(err_symtab_hashlist_t_p)= TYPE_INFO_POINTER (err_symtab_hashlist_t *, err_symtab_hashlist_t);


TYPE_INFO_T(err_symtab_hashlist_t_const_p)= TYPE_INFO_POINTER (err_symtab_hashlist_t const *, err_symtab_hashlist_t);


TYPE_INFO_T(err_symtab_content)= TYPE_INFO_VECTOR (err_symtab_content, err_symtab_hashlist_t_p, err_symtab_t, m_hashsize);


TYPE_INFO_T(err_symtab_content_p)= TYPE_INFO_POINTER (err_symtab_content *, err_symtab_content);

static
TYPE_STRUCT_T(err_symtab_internal_Iterator)=
    TYPE_STRUCT_BEGIN(err_symtab_internal_Iterator)
        TYPE_STRUCT_SLOT(err_symtab_internal_Iterator, err_symtab_hashlist_t_p, cnode)
        TYPE_STRUCT_SLOT(err_symtab_internal_Iterator, int,      cslot)
    TYPE_STRUCT_END
;


TYPE_INFO_T(err_symtab_internal_Iterator)=
    TYPE_INFO_STRUCT (err_symtab_internal_Iterator, err_symtab_internal_Iterator);

static
TYPE_STRUCT_T(err_symtab_t)=
    TYPE_STRUCT_BEGIN(err_symtab_t)
#ifdef  ERR_SYMTAB_SUPER_TYPE
        TYPE_SUPER(err_symtab_t, ERR_SYMTAB_SUPER_CLASS),
#endif
        ERR_SYMTAB_TYPE_INFO_STD_MEMBERS
        err_symtab_record_sig     (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_sig))
        err_symtab_record_table   (TYPE_STRUCT_SLOT(err_symtab_t, err_symtab_content_p,   m_table))
        err_symtab_record_sizes   (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_hashsize))
        err_symtab_record_sizes   (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_nentries))
        err_symtab_record_errno   (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_errno))
        err_symtab_record_zero    (TYPE_STRUCT_SLOT(err_symtab_t, void, m_zero))
        err_symtab_record_compat  (TYPE_STRUCT_SLOT(err_symtab_t, err_symtab_internal_Iterator, compat1_iter))
        err_symtab_record_profile (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_nrehash))
        err_symtab_record_profile (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_nrehashops))
        err_symtab_record_profile (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_nfind))
        err_symtab_record_profile (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_ninsert))
        err_symtab_record_profile (TYPE_STRUCT_SLOT(err_symtab_t, int,             m_ndelete))
    TYPE_STRUCT_END
;


TYPE_INFO_T(err_symtab_t)= TYPE_INFO_STRUCT (err_symtab_t, err_symtab_t);


TYPE_INFO_T(err_symtab_t_p)= TYPE_INFO_POINTER (err_symtab_t *, err_symtab_t);


TYPE_INFO_T(err_symtab_t_const_p)= TYPE_INFO_POINTER (err_symtab_t const *, err_symtab_t);
...
err_symtab_hashlist_t * err_symtab_hashlist_new_q
        (char *  key, void  value)
{
    err_symtab_hashlist_t *self;
    E_SET_PTR(self, ERR_ERWIN_THING_NEW1 (err_symtab_hashlist_t));
...
void err_symtab_hashlist_delete
        (err_symtab_hashlist_t *list)
{
    err_symtab_hashlist_t *next;

    while (list) {
        IFREE (list->key);
        OFREE (list->value);
        E_SET_PTR(next, list->next);
        ERR_ERWIN_THING_DELETE1 (list, err_symtab_hashlist_t);
        E_SET_PTR(list, next);
    }
}
...
void err_symtab_hashlist_delete_flags
        (err_symtab_hashlist_t *list, ERR_ERWIN_BOOL keys, ERR_ERWIN_BOOL values)
{
    err_symtab_hashlist_t *next;
...
void err_symtab_hashlist_delete_no_child_delete
        (err_symtab_hashlist_t *list)
{
    err_symtab_hashlist_t *next;
...
void err_symtab_hashlist_delete_cell
        (err_symtab_hashlist_t *list)
{
    ERR_ERWIN_THING_DELETE1 (list, err_symtab_hashlist_t);
}
...
int err_symtab_hashlist_length
        (err_symtab_hashlist_t *list)
{
    int result= 0;
    while (list) {
...
int err_symtab_errno
        (err_symtab_t const *self)
{
...
ERR_ERWIN_BOOL err_symtab_init_internal_q
        (err_symtab_t *self, void  zero, int nelem)
{
    int size;
#ifndef NDEBUG
    int err= 0;
    ...
    if (nelem >= 0) { /* Compute hash size from expected number of elements: */
        size= ((nelem * ERR_SYMTAB_TRIGGER_DENOMINATOR + ERR_SYMTAB_TRIGGER_NUMERATOR - 1) / /* round up */
                ERR_SYMTAB_TRIGGER_NUMERATOR * 2
              ) + 1;
    }

    if (size < 0)
        size= ERR_SYMTAB_INITIAL_SIZE;

#if  ERR_SYMTAB_HAVE_SIG
    self->m_sig= ERR_SYMTAB_SIG;
#endif
    ...
    if (size == 1)    /* this means that the slot table is missing and we have a single list. */
        ...
        E_SET_PTR(self->m_table, ERR_ERWIN_THING_CNEW_ARRAY (err_symtab_content, (unsigned)size));
...
int err_symtab_init
        (err_symtab_t *self)
{
    err_symtab_init_internal_q (self, OZERO, ERR_SYMTAB_INITIAL_SIZE);
    return e_errno;
}

CODE_SECTION(".text.init_with_initial_size")
int err_symtab_init_with_initial_size
        (err_symtab_t *self, int size)
{
    err_symtab_init_internal_q (self, OZERO, size);
    return e_errno;
}
...
int err_symtab_init_with_zero_and_initial_size
        (err_symtab_t *self, void  zero, int size)
{
    err_symtab_init_internal_q (self, zero, size);
    return e_errno;
}
...
err_symtab_t * err_symtab_new_with_zero_and_initial_size
        (void  zero, int size)
{
    err_symtab_t *self;

    E_SET_PTR(self, ERR_ERWIN_THING_NEW1 (err_symtab_t));
    SET_ERRNO(E_OK);
    ...
    if (ERWIN_LIKELY(err_symtab_init_internal_q (self, zero, size)))
       return self;

    ERR_ERWIN_THING_DELETE1 (self, err_symtab_t);
    return NULL;
}
...
err_symtab_t * err_symtab_new_with_zero
        (void  zero)
{
    return err_symtab_new_with_zero_and_initial_size (zero, ERR_SYMTAB_INITIAL_SIZE);
}

#endif /* ERR_SYMTAB_DYN_ZERO */

#if ERR_SYMTAB_CONSTANT_ZERO
#define err_symtab_new_with_zero_and_initial_size(Z,S) err_symtab_new_with_initial_size(S)
#define err_symtab_new_with_zero(Z)                    err_symtab_new()
#endif
...
err_symtab_t * err_symtab_new_with_initial_size
        (int size)
{
    err_symtab_t *self;
    ...
    if (ERWIN_LIKELY(err_symtab_init_internal_q (self, OZERO, size)))
       return self;
...
err_symtab_t * err_symtab_new
        (void)
{
    return err_symtab_new_with_zero_and_initial_size (OZERO, ERR_SYMTAB_INITIAL_SIZE);
}


CODE_SECTION(".text.zero")
o_type_result_t err_symtab_zero
        (err_symtab_t const *self)
{
    (void)self; /*unused*/
...
void err_symtab_struct_copy
        (err_symtab_t *copy, err_symtab_t const *self)
{
    int i;

    for (i= 0; i < self->m_hashsize; i++) {
       err_symtab_hashlist_t *lauf;
       E_SET_PTR(lauf, SLOT(self,i));
       while (lauf) {
           err_symtab_insert_q (copy, lauf->key, lauf->value);
        ...
        (err_symtab_t const* self)
{
    ...
    E_SET_PTR(copy, err_symtab_new_with_zero_and_initial_size (ZERO(self), self->m_nentries));
    err_symtab_struct_copy (copy, self);
    return copy;
}

CODE_SECTION(".text.copy_err")
err_symtab_t * err_symtab_copy_err
        (err_symtab_t const* self, int *err)
{
    err_symtab_t *result;

    E_SET_PTR(result, err_symtab_copy (self));
...
int err_symtab_hash_size
        (err_symtab_t const* self)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL)
        return 0;
#endif
...
ERR_ERWIN_BOOL err_symtab_empty
        (err_symtab_t const* self)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL)
...
void err_symtab_clear_internal
        (err_symtab_t* self)
{
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        err_symtab_hashlist_delete (SLOT(self,i));
        E_SET_PTR(SLOT(self,i), NULL);
    }
}

CODE_SECTION(".text.clear_no_resize")
void err_symtab_clear_no_resize
        (err_symtab_t* self)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    err_symtab_clear_internal (self);
...
void err_symtab_clear_internal_flags
        (err_symtab_t* self, ERR_ERWIN_BOOL keys, ERR_ERWIN_BOOL values)
{
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        err_symtab_hashlist_delete_flags (SLOT(self,i), keys, values);
        E_SET_PTR (SLOT(self,i), NULL);
    }
}
...
void err_symtab_clear_flags_no_resize
        (err_symtab_t* self, ERR_ERWIN_BOOL keys, ERR_ERWIN_BOOL values)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    err_symtab_clear_internal_flags (self, keys, values);
    self->m_nentries= 0;
    SET_ERRNO(E_OK);
}
...
void err_symtab_destroy_internal
        (err_symtab_t* self)
{
    return_if_null2i_or_bad (self);
...
void err_symtab_xchg
        (err_symtab_t *self, err_symtab_t *other)
{
    err_symtab_record_table   (ERWIN_XCHG (err_symtab_content_p,   self->m_table,      other->m_table));
    err_symtab_record_sizes   (ERWIN_XCHG (int,             self->m_hashsize,   other->m_hashsize));
    err_symtab_record_sizes   (ERWIN_XCHG (int,             self->m_nentries,   other->m_nentries));
    err_symtab_record_errno   (ERWIN_XCHG (int,             self->m_errno,      other->m_errno));
    err_symtab_record_zero    (ERWIN_XCHG (void,        self->m_zero,       other->m_zero));
    err_symtab_record_compat  (ERWIN_XCHG (map_iterator_t,  self->compat1_iter, other->compat1_iter));
    err_symtab_record_profile (ERWIN_XCHG (int,             self->m_nrehash,    other->m_nrehash));
    err_symtab_record_profile (ERWIN_XCHG (int,             self->m_nrehashops, other->m_nrehashops));
    err_symtab_record_profile (ERWIN_XCHG (int,             self->m_nfind,      other->m_nfind));
    err_symtab_record_profile (ERWIN_XCHG (int,             self->m_ninsert,    other->m_ninsert));
    err_symtab_record_profile (ERWIN_XCHG (int,             self->m_ndelete,    other->m_ndelete));
}

ERWIN_STATIC_INLINE
CODE_SECTION(".text.destroy_internal_flags")
void err_symtab_destroy_internal_flags
        (
    err_symtab_t* self,
    ...
    ERR_ERWIN_BOOL destroy_values)
{
    ...
    err_symtab_clear_internal_flags (self, destroy_keys, destroy_values);
    FREE_TABLE(self);
...
void err_symtab_destroy
        (err_symtab_t* self)
{
    err_symtab_destroy_internal (self);
}

CODE_SECTION(".text.destroy_flags")
void err_symtab_destroy_flags
        (err_symtab_t* self, ERR_ERWIN_BOOL k, ERR_ERWIN_BOOL v)
{
    err_symtab_destroy_internal_flags (self, k, v);
}
...
void err_symtab_delete_flags
        (
    err_symtab_t* self, ERR_ERWIN_BOOL destroy_keys, ERR_ERWIN_BOOL destroy_values)
{
    if (self != NULL) {
        err_symtab_destroy_internal_flags (self, destroy_keys, destroy_values);
        ERR_ERWIN_THING_DELETE1 (self, err_symtab_t);
    }
}

CODE_SECTION(".text.delete")
void err_symtab_delete
        (err_symtab_t* self)
{
    if (self != NULL) {
        err_symtab_destroy_internal (self);
...
ERR_ERWIN_BOOL err_symtab_expect_size
        (err_symtab_t *self, int newsize)
{
    int new_hash_size;
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL) {
    ...
    if (TOOSMALL(newsize, new_hash_size)) {
        ...
        if (new_hash_size <= 1) {
             ...
             * this too small. :-) */
            if (newsize <= ERR_SYMTAB_PLAIN_LIST_LENGTH)
                return ERR_ERWIN_FALSE;
        }
#endif
        ...
        err_symtab_rehash (self, new_hash_size);
    ...
    if (TOOLARGE(newsize, new_hash_size)) {
...
#endif
        {
            ...
            if (new_hash_size < ERR_SYMTAB_MINIMAL_SIZE_OR_1)
                new_hash_size= ERR_SYMTAB_MINIMAL_SIZE_OR_1;
        }
...
ERR_ERWIN_BOOL err_symtab_shrink_perhaps
        (err_symtab_t *self)
{
...
#if ERR_SYMTAB_ZERO_SIZE
        if (self->m_nentries <= ERR_SYMTAB_PLAIN_LIST_LENGTH)
            newsize= 1;
        else
#endif
        {
            ...
            if (newsize < ERR_SYMTAB_MINIMAL_SIZE_OR_1)
                newsize= ERR_SYMTAB_MINIMAL_SIZE_OR_1;
        }

        err_symtab_rehash (self, newsize);
...
void err_symtab_clear_flags
        (err_symtab_t* self, ERR_ERWIN_BOOL keys, ERR_ERWIN_BOOL values)
{
    err_symtab_clear_flags_no_resize (self, keys, values);
    err_symtab_shrink_perhaps (self);
}
...
#endif
    err_symtab_clear_no_resize(self);
    err_symtab_shrink_perhaps (self);
    SET_ERRNO(E_OK);
}
...
unsigned err_symtab_slot_no
        (err_symtab_t const *self, char const *  key)
{
    int slot;
    return0_if_null (self);
...
err_symtab_hashlist_t ** err_symtab_find_internal_ptr
        (err_symtab_t const* self, char const *  key)
{
    err_symtab_hashlist_t **lauf;
#ifdef E_PROFILE
    ...
    E_SET_PTR(lauf, &SLOT(self, err_symtab_slot_no (self, key)));
    while (*lauf) {
...
err_symtab_hashlist_t * err_symtab_find_internal
        (err_symtab_t const* self, char const *  key)
{
    err_symtab_hashlist_t **result;

    E_SET_PTR(result, err_symtab_find_internal_ptr (self, key));
#if ERR_SYMTAB_ALLOW_NULL
    if (result == NULL)
        return NULL;
#endif
 ...
 *        err_symtab_ensure which currently evaluates ENSURE_VALUE to often).
 *
...
int err_symtab_poke_internal_q
        (
        char const *                *key_out,
        void             *value_out,
        err_symtab_t                *self,
        ...
        ERR_ERWIN_BOOL     overwrite)
{
    ...
    E_SET_PTR(lauf, err_symtab_find_internal_ptr (self, key));
#if ERR_SYMTAB_ALLOW_NULL
...
#endif
    {
        ...
        if (introduce) {
            ...
            if (err_symtab_grow_perhaps (self)) {
                E_SET_PTR(lauf, err_symtab_find_internal_ptr (self, key));
            }
...

            {
                if (dokeycopy) {
                    ...
                    E_SET_PTR(*lauf, err_symtab_hashlist_new_q (keycopy, givenvaluecopy));
                    ...
                    E_SET_PTR(*lauf, err_symtab_hashlist_new_q (givenkeycopy, givenvaluecopy));
...
int err_symtab_poke_q
        (   
    char const * *ko, void *vo,
    err_symtab_t *self,
    char const *  k, void  v,
    ERR_ERWIN_BOOL i, ERR_ERWIN_BOOL o)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_symtab_poke_internal_q (ko,vo,self, k,IZEROVAR,ERR_ERWIN_TRUE, v,ZEROVAR(self),ERR_ERWIN_TRUE, i,o);
}

CODE_SECTION(".text.poke_no_icopy_q")
int err_symtab_poke_no_icopy_q
        (
    char const * *ko, void *vo, err_symtab_t *self, char *  k, void  v, ERR_ERWIN_BOOL i, ERR_ERWIN_BOOL o)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_symtab_poke_internal_q (ko,vo,self, k,k,ERR_ERWIN_FALSE, v,ZEROVAR(self),ERR_ERWIN_TRUE, i,o);
}
...
o_type_result_t err_symtab_find
        (err_symtab_t const* self, char const *  key)
{
    err_symtab_hashlist_t *lauf;

#if defined(ERR_SYMTAB_ALLOW_NULL) && ERR_SYMTAB_CONSTANT_ZERO
    if (self == NULL) {
    ...
    E_SET_PTR(lauf, err_symtab_find_internal (self, key));
    if (lauf == NULL) {
        SET_ERRNO(E_KEYNOTFOUND);
        return ZERO(self);
    }
...
i_type_result_t err_symtab_find_key
        (err_symtab_t const* self, char const *  key)
{
    err_symtab_hashlist_t *lauf;
#if defined(ERR_SYMTAB_ALLOW_NULL)
    if (self == NULL) {
        SET_ERRNO(E_KEYNOTFOUND);
        return IZERO;
    }
...
int err_symtab_insert_no_copy_no_rehash_q
        (
        err_symtab_t* self,
        char * keycopy,
        void valuecopy)
 ...
 *       delete keycopy or valuecopy! */
{
    ...
    E_SET_PTR(lauf, err_symtab_find_internal_ptr (self, keycopy));
    return_val_if_null_pp (lauf, ASSERTION_FAILED, self);

    if (*lauf != NULL)
        return PASS_ERRNO(E_EXISTINGKEY);

    E_SET_PTR(*lauf, err_symtab_hashlist_new_q (keycopy, valuecopy));
...
ATTR_FLATTEN
int err_symtab_insert_q
        (err_symtab_t* self, char const *  key, void  value)
{
    return_val_if_null_or_bad (self, ASSERTION_FAILED);
    return err_symtab_poke_internal_q (
                NULL, NULL, self,
...
int err_symtab_insert_map
        (err_symtab_t* self, err_symtab_t const *other)
{
    int i;
    int total_errno;
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       while (lauf) {
           err_symtab_insert_q (self, lauf->key, lauf->value);
           if (e_errno != E_OK)
        ...
        (err_symtab_t* self, err_symtab_t const *other)
{
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           err_symtab_erase (self, lauf->key);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
...
int err_symtab_erase_map_no_resize
        (err_symtab_t* self, err_symtab_t const *other)
{
    int i;
    int total_errno;
    ...
    for (i= 0; i < other->m_hashsize; i++) {
       ...
       E_SET_PTR (lauf, SLOT(other,i));
       while (lauf) {
           err_symtab_erase_no_resize (self, lauf->key);
           if (e_errno != E_OK)
               total_errno= e_errno; /* this does not break, it tries to do as
                                      * much as possible. */
           E_SET_PTR (lauf, lauf->next);
...
int err_symtab_intersect_no_resize
        (err_symtab_t* self, err_symtab_t const *other)
{
    int i;
    int cnt;
    ...
    for (i= 0; i < self->m_hashsize; i++) {
        ...
        E_SET_PTR (lauf, &SLOT(self,i));
        while (*lauf) {
            (void)err_symtab_find (other, (*lauf)->key);
            if (e_errno != E_OK) {
                 err_symtab_hashlist_t *cell;
                 E_SET_PTR (cell,  *lauf);
                 E_SET_PTR (*lauf, (*lauf)->next);

                 IFREE (cell->key);
                 OFREE (cell->value);
                 err_symtab_hashlist_delete_cell (cell);
                 self->m_nentries--;
                 cnt++;
            }
            else
...
int err_symtab_intersect
        (err_symtab_t* self, err_symtab_t const *other)
{
    int cnt= err_symtab_intersect_no_resize (self, other);
    err_symtab_shrink_perhaps (self);
    return cnt;
}
...
i_type_result_t err_symtab_ensure
        (err_symtab_t *self, char const *  key)
{
    err_symtab_hashlist_t **lauf;
    ...
    if (*lauf == NULL) {
        ...
        if (ERWIN_UNLIKELY(err_symtab_grow_perhaps (self))) {
            E_SET_PTR (lauf, err_symtab_find_internal_ptr (self, key));
            return_val_if_null_pp (lauf, IZERO, self);
...
i_type_result_t err_symtab_ensure_no_icopy
        (err_symtab_t *self, char *  key)
{
    err_symtab_hashlist_t **lauf;
    ...
    if (*lauf == NULL) {
...

        {
            ...
            E_SET_PTR (*lauf, err_symtab_hashlist_new_q (key, valuecopy));
            if (ERWIN_UNLIKELY(*lauf == NULL)) {
                MAP_NOMEM;
                return IZERO;
            }
...
/*
 * Remove an element and return the old one.  Avoid returning void (ZERO(self)) to
 * void here. */
ERWIN_STATIC_INLINE_SPEED
CODE_SECTION(".text.remove_internal_no_resize_q")
ERR_ERWIN_BOOL err_symtab_remove_internal_no_resize_q
        (
    err_symtab_t *self,
    char const *  key,
    void *result)
...
ERR_ERWIN_BOOL err_symtab_remove_internal_q
        (
    err_symtab_t *self,
    char const *  key,
    void *result)
{
    ...
    return err_symtab_remove_internal_no_resize_q (self, key, result);
}
        ...
        (err_symtab_t *self, char const *  key)
{
    ...
    if (self == NULL)
        return PASS_ERRNO(E_KEYNOTFOUND);
#endif
    if (err_symtab_remove_internal_q (self, key, &result)) {
        OFREE (result);
    }
    return e_errno;
}
...
int err_symtab_erase_no_resize
        (err_symtab_t *self, char const *  key)
{
#if defined(ERR_SYMTAB_ALLOW_NULL) && ERR_SYMTAB_CONSTANT_ZERO
    if (self == NULL)
        return PASS_ERRNO(E_KEYNOTFOUND);
#endif
    if (err_symtab_remove_internal_no_resize_q (self, key, &result)) {
        OFREE (result);
    }
    return e_errno;
}
...
int err_symtab_erase_if
        (err_symtab_t *self, err_symtab_feature_t feature, ERR_ERWIN_BOOL value)
{
    int slot;
    int cnt= 0;
        ...
        (err_symtab_t* self, int newsize)
{
    ...
    if (newsize != self->m_hashsize) {
        ...
        if (newsize == 1) {
            ...
            E_SET_PTR (newtable, ERR_ERWIN_THING_CNEW_ARRAY (err_symtab_content, (unsigned)newsize));
        ...
        for (i=0; i < self->m_hashsize; i++) {
            ...
            while (lauf != NULL) {
                err_symtab_hashlist_t *next;
                err_symtab_hashlist_t **newcell= &GET_SLOT(
                                      newtable,
                                      newsize,
...
err_symtab_hashlist_t * err_symtab_first_internal
        (err_symtab_t const *self, e_iterator_t *iter)
{
...
err_symtab_hashlist_t * err_symtab_next_internal
        (err_symtab_t const*self, e_iterator_t *iter)
{
    if (ERWIN_UNLIKELY(ITER->cslot == -1))
        return err_symtab_first_internal (self, iter);
...
void err_symtab_init_iterator
        (err_symtab_t const *self, e_iterator_t *iter)
{
    (void)self; /*unused*/

#ifndef ERR_SYMTAB_ALLOW_NULL
    return_if_null_or_bad (self);
#endif
        ...
        (void const *a, void const *b)
{
    ...
    (void)b; /*unused*/
#ifdef ICMP
#define err_symtab_check_cmp_keys
#define err_symtab_check0_cmp_keys
    return ICMP ((((err_symtab_pair_t*)a)->key),(((err_symtab_pair_t*)b)->key));
#else
#define err_symtab_check_cmp_keys \
...
ATTR_FUNC_UNUSED
int err_symtab_pair_detcmp_keys
        (void const *a, void const *b)
{
    int result;
    (void)a; /*unused*/
...
#define err_symtab_detcmp_pairs  err_symtab_pair_cmp_keys

#ifndef ERR_SYMTAB_DET_SORT_PROVIDED
#  define ERR_SYMTAB_DET_SORT_PROVIDED 0
#endif

#if ODETCMP_PROVIDED || ERR_SYMTAB_DET_SORT_PROVIDED
#  define ERR_SYMTAB_ODET_REQUIRE 1
#endif

#ifndef ERR_SYMTAB_ODET_REQUIRE
#  define ERR_SYMTAB_ODET_REQUIRE 0
#endif




#if ERR_SYMTAB_DET_SORT_PROVIDED

static
CODE_SECTION(".text.detcmp_pairs")
ATTR_FUNC_UNUSED
int err_symtab_detcmp_pairs
        (void const *a, void const *b)
{
    ...
    /* return0_if_null2 (a,b);  needs self via ASSERTION_FAILED when THREAD_SAFE */
    result= IDETCMP ((((err_symtab_pair_t*)a)->key),(((err_symtab_pair_t*)b)->key));


#if defined(ICMP) && IDETCMP_PROVIDED
    if (result == 0)
        result= ICMP ((((err_symtab_pair_t*)a)->key),(((err_symtab_pair_t*)b)->key));
#endif
...
#endif /* ERR_SYMTAB_DET_SORT_PROVIDED */
...
void err_symtab_det_sort
        (err_symtab_t const *self, e_iterator_sorted_t *iter)
{
    qsort ((void*)SORTITER->table,
           self->m_nentries,
           sizeof (err_symtab_pair_t),
           err_symtab_detcmp_pairs);
}
...
void err_symtab_free_sorted_iterator
        (void *iter)
{
    err_symtab_delete_entries (SORTITER->table);
}
...
void err_symtab_init_iterator_sorted_by_key
        (err_symtab_t const *self, e_iterator_sorted_t *iter)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITER->table, err_symtab_get_entries (self));
        SORTITER->count= 0;
        SORTITER->lauf=  0;
        E_SET_PTR (SORTITER->free_cb, err_symtab_free_sorted_iterator);
        return;
    }
#else
    return_if_null_or_bad (self);
    ...
    return_if_fail_pp ((void*)SORTITER_VOLATILE->free_cb == (void*)err_symtab_free_sorted_iterator, self);
    ...
    if (e_require_determinism ()) {
        err_symtab_det_sort (self, iter);
        e_merge_sort ((void*)SORTITER->table,
               self->m_nentries,
               sizeof (err_symtab_pair_t),
               err_symtab_pair_cmp_keys);
    }
    else
#endif
    {
...
void err_symtab_init_iterator_sorted_by_user
        (
    err_symtab_t const *self,
    e_iterator_sorted_t *iter,
    err_symtab_pair_cmp_t u)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (SORTITER->table, err_symtab_get_entries (self));
    ...
    return_if_null_pp2 (SORTITER_VOLATILE->p,
        SORTITER, self);
    return_if_fail_pp2 ((void*)SORTITER_VOLATILE->free_cb == (void*)err_symtab_free_sorted_iterator,
        SORTITER, self);
...
void err_symtab_init_iterator_sorted_by_key_reverse
        (err_symtab_t const *self, e_iterator_sorted_t *iter)
{
    err_symtab_init_iterator_sorted_by_key (self, iter);
...
void err_symtab_init_iterator_sorted_by_user_reverse
        (
    err_symtab_t const *self, e_iterator_sorted_t *iter, err_symtab_pair_cmp_t u)
{
    err_symtab_init_iterator_sorted_by_user (self, iter, u);
    SORTITER->lauf= SORTITER->count;
}
...
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_keys
        (err_symtab_t const *self,
                                e_iterator_sorted_t *iter,
                                char const * *keyp)
{
...
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_pairs
        (err_symtab_t const *self,
                                e_iterator_sorted_t *iter,
                                err_symtab_pair_t *p)
{
...
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_keys_reverse
        (
    err_symtab_t const *self, e_iterator_sorted_t *iter, char const * *keyp)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERR_ERWIN_FALSE;
...
ERR_ERWIN_BOOL err_symtab_next_iteration_sorted_pairs_reverse
        (
    err_symtab_t const *self, e_iterator_sorted_t *iter, err_symtab_pair_t *p)
{
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERR_ERWIN_FALSE;
...
ERR_ERWIN_BOOL err_symtab_next_iteration_keys
        (err_symtab_t const *self, e_iterator_t *iter, char const * *keyp)
{
    err_symtab_hashlist_t *it;
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL)
        return ERR_ERWIN_FALSE;
#endif
    ...
    E_SET_PTR (it, err_symtab_next_internal (self, iter));
...
ERR_ERWIN_BOOL err_symtab_next_iteration_pairs
        (err_symtab_t const *self, e_iterator_t *iter, err_symtab_pair_t *pairp)
{
    err_symtab_hashlist_t *it;
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL)
...
err_symtab_pair_t * err_symtab_get_entries
        (err_symtab_t const *self)
{
    struct _err_symtab_pair_t *result, *rlauf;
    int    i;

#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, PAIR_NEW_ARRAY (err_symtab_pair_dynarray, 1));
    ...
    E_SET_PTR (result, PAIR_NEW_ARRAY (err_symtab_pair_dynarray, self->m_nentries+1));
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (err_symtab_pair_t),
               (int(*)(void const *, void const *))err_symtab_detcmp_pairs);
#endif
        ...
        (err_symtab_t const *self)
{
...
#ifdef ERR_SYMTAB_ALLOW_NULL
    if (self == NULL) {
        E_SET_PTR (result, KEY_NEW_ARRAY (err_symtab_key_dynarray, 1));
        if (ERWIN_UNLIKELY(result == NULL)) {
            MAP_NOMEM;
            return NULL;
        }
    ...
    E_SET_PTR (result, KEY_NEW_ARRAY (err_symtab_key_dynarray, self->m_nentries+1));
    E_SET_PTR (rlauf,  result);
    if (ERWIN_UNLIKELY(result == NULL)) {
        MAP_NOMEM;
        return NULL;
    ...
    if (e_require_determinism ())
        qsort (result,
               self->m_nentries,
               sizeof (char const *),
               (int(*)(void const *, void const *))err_symtab_pair_detcmp_keys);
#endif
...
void err_symtab_delete_entries
        (err_symtab_pair_t *entries)
{
...
ERR_ERWIN_BOOL err_symtab_equal
        (err_symtab_t const *self, err_symtab_t const *other)
{
    ...
    err_symtab_forall_keys_nondet (self, iter, key)
        if (!err_symtab_find(other,key))
            return ERR_ERWIN_FALSE;
    ...
    error_return0_if_reached_ppt (
        self,
        "Because neither ERR_VOID_EQUAL nor ERR_VOID_CMP was #defined, err_symtab_equal could not be implemented.");
#endif
}


CODE_SECTION(".text.find_any_pair_q")
int err_symtab_find_any_pair_q
        (err_symtab_key_result_t *key_p, err_symtab_element_ptr_t *value_p, err_symtab_t const* self)
{
    int i;
    ...
    { /* Find smallest entry. */
        ...
        for (i= 0; i < self->m_hashsize; i++) {
           ...
           E_SET_PTR (lauf, SLOT(self,i));
           while (lauf) {
               if (first || err_symtab_pair_detcmp_keys (&lauf->key, &last) < 0) {
                   first= 0;
...
i_type_result_t err_symtab_find_any_key
        (err_symtab_t const *self)
{
    char const * result;
    int n= err_symtab_find_any_pair_q (&result, NULL, self);
...
ERR_ERWIN_BOOL err_symtab_equal_keys
        (err_symtab_t const *self, err_symtab_t const *other)
{
    e_iterator_t iter;
    char const * key;
...
/* must be NULL safe per definition! */
{
    ...
    err_symtab_pair_t *them1;
    err_symtab_pair_t *them2;
    int i;
    int result= 0;
    int cnt;
    err_symtab_check0_cmp_keys;
#endif
    ...
    E_SET_PTR (them1, err_symtab_get_entries (self));
    if (ERWIN_UNLIKELY(e_errno != E_OK))
        return 0;

    E_SET_PTR (them2, err_symtab_get_entries (other));
    if (ERWIN_UNLIKELY(e_errno != E_OK)) {
        err_symtab_delete_entries (them1);
        return 0;
    }

#if defined(E_REQUIRE_DETERMINISM) && !defined(E_WEAK_DETERMINISM)
    if (e_require_determinism()) {
        qsort ((void*)them1, cnt, sizeof (err_symtab_pair_t), err_symtab_pair_detcmp_keys);
        qsort ((void*)them2, cnt, sizeof (err_symtab_pair_t), err_symtab_pair_detcmp_keys);

        e_merge_sort ((void*)them1, cnt, sizeof (err_symtab_pair_t), err_symtab_pair_cmp_keys);
        e_merge_sort ((void*)them2, cnt, sizeof (err_symtab_pair_t), err_symtab_pair_cmp_keys);
    }
    else
#else
    {
        qsort ((void*)them1, cnt, sizeof (err_symtab_pair_t), err_symtab_pair_cmp_keys);
        qsort ((void*)them2, cnt, sizeof (err_symtab_pair_t), err_symtab_pair_cmp_keys);
    }
#endif

    for (i=0; i < cnt; i++) {
        result= err_symtab_pair_cmp_keys (them1 + i, them2 + i);
        if (result != 0)
            break;
    ...
    err_symtab_delete_entries (them1);
    err_symtab_delete_entries (them2);
    ...
    error_return0_if_reached_ppt (
        self,
        "Because either ERR_CHAR_CONST_P_CMP or ERR_VOID_CMP was not #defined, err_symtab_cmp could not be implemented.");
#endif
}
...
/* must be NULL safe per definition! */
{
    ...
    E_SET_PTR (them1, err_symtab_get_keys (self));
    if (ERWIN_UNLIKELY(e_errno != E_OK))
        return 0;

    E_SET_PTR (them2, err_symtab_get_keys (other));
    if (ERWIN_UNLIKELY(e_errno != E_OK)) {
        err_symtab_delete_keys (them1);
        return 0;
    }

    qsort ((void*)them1, cnt, sizeof (char const *), err_symtab_wrap_icmp);
    qsort ((void*)them2, cnt, sizeof (char const *), err_symtab_wrap_icmp);
    ...
    error_return0_if_reached_ppt (
        self,
        "Because either ERR_CHAR_CONST_P_CMP was not #defined, err_symtab_cmp_keys could not be implemented.");
#endif
}
...
e_hashval_t err_symtab_hash_raw
        (err_symtab_t const *self)
/* must be NULL-safe */
{
#ifdef OHASH
    ...
    err_symtab_forall_keys_nondet (self, iter, key) {
        ERR_ERWIN_STATE_MIX_UNORDERED (state, IHASH (key));
    }
    ...
    error_return0_if_reached_ppt (
        self,
        "Because ERR_VOID_HASH was not #defined, err_symtab_hash_raw could not be implemented.");
#endif
}
...
CODE_SECTION(".text.t")
 err_symtab_t::err_symtab_t
        (void)
{
    ERR_ERWIN_BOOL ok= err_symtab_init_internal_q (it(), OZERO, ERR_SYMTAB_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}

CODE_SECTION(".text._constructor")
void err_symtab_t::_constructor
        (void)
{
    ERR_ERWIN_BOOL ok= err_symtab_init_internal_q (it(), OZERO, ERR_SYMTAB_INITIAL_SIZE);
    (void)ok; /*unused*/
...
err_symtab_t const & err_symtab_t::static_zero
        ()
{
    static err_symtab_t const *z= NULL;
    if (ERWIN_UNLIKELY(!z))
        E_SET_PTR (z, new err_symtab_t);
    return *z;
}
...
#if ERR_SYMTAB_HAVE_INT_CONSTRUCTOR
CODE_SECTION(".text.t")
 err_symtab_t::err_symtab_t
        (int size)
{
    ERR_ERWIN_BOOL ok= err_symtab_init_internal_q (it(), OZERO, size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* ERR_SYMTAB_HAVE_INT_CONSTRUCTOR */
...
CODE_SECTION(".text.t")
 err_symtab_t::err_symtab_t
        (void  zeroval)
{
    ERR_ERWIN_BOOL ok= err_symtab_init_internal_q (it(), zeroval, ERR_SYMTAB_INITIAL_SIZE);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
...
CODE_SECTION(".text.t")
 err_symtab_t::err_symtab_t
        (void  zeroval, int size)
{
    ERR_ERWIN_BOOL ok= err_symtab_init_internal_q (it(), zeroval, size);
    (void)ok; /*unused*/
    return_if_fail_pp (ok, this);
}
#endif /* ERR_SYMTAB_DYN_ZERO */
...
CODE_SECTION(".text.t")
 err_symtab_t::err_symtab_t
        (err_symtab_t const &other)
{
#ifdef ERWIN_DOS
    if (other.it()->m_table == NULL && other.it()->m_nentries == 0) {
        /* See copy constructor for vectors for explanation. */
        err_symtab_init_internal_q (it(), OZERO, ERR_SYMTAB_INITIAL_SIZE);
    }
    else
#endif
    {
        err_symtab_init_internal_q (it(), ZERO(other.it()), other.it()->m_nentries);
        err_symtab_struct_copy (it(), other.it());
    }
}
...
CODE_SECTION(".text.t")
 err_symtab_t::err_symtab_t
        (err_symtab_t const *other)
{
    err_symtab_init_internal_q (it(), ZERO(other), other->m_nentries);
    err_symtab_struct_copy (it(), other);
}
...
CODE_SECTION(".text.err_symtab_t")
 err_symtab_t::~err_symtab_t
        ()
{
    err_symtab_destroy_internal (it());
}


CODE_SECTION(".text._destructor")
void err_symtab_t::_destructor
        ()
{
    err_symtab_destroy_internal (it());
}
...
void * err_symtab_t::operator new
        (size_t s)
{
...
void err_symtab_t::operator delete
        (void *p, size_t s)
{
    (void)s; /*unused*/
...
err_symtab_t & err_symtab_t::operator=
        (err_symtab_t const *other)
{
    err_symtab_clear_no_resize (it());
    err_symtab_expect_size (it(), other->it()->m_nentries); /* realloc to good size */
    err_symtab_struct_copy (it(), other->it());
    return *this;
}
...
double err_symtab_average_line_length
        (err_symtab_t const *self)
{
    double sum = 0.0;
    int i;
    for (i=0; i < self->m_hashsize; i++)
        sum+= err_symtab_hashlist_length (SLOT(self,i));
    return sum / (double)(self->m_hashsize);
}

CODE_SECTION(".text.variance_line_length")
double err_symtab_variance_line_length
        (err_symtab_t const *self)
{
    double average= err_symtab_average_line_length (self);
    double sum2= 0.0;
    int i;
    for (i=0; i < self->m_hashsize; i++) {
        double diff= err_symtab_hashlist_length (SLOT(self,i)) - average;
        sum2+= diff*diff;
    }
...
int err_symtab_max_line_length
        (err_symtab_t const *self)
{
    int i;
    int max = 0;
    for (i=0; i < self->m_hashsize; i++) {
        int len= err_symtab_hashlist_length (SLOT(self,i));
        if (len > max) max= len;
    }
    return max;
}
...
int err_symtab_min_line_length
        (err_symtab_t const *self)
{
    int i;
...
double err_symtab_deviation_line_length
        (err_symtab_t const *self)
{
    return sqrt (err_symtab_variance_line_length (self));
}
#endif /* defined(HAVE_SQRT) */


CODE_SECTION(".text.dump")
void err_symtab_dump
        (FILE *thefile, err_symtab_t const *self)
{
    int i;
    fprintf (thefile, "(err_symtab_dump):\n");
...
#undef ERR_SYMTAB_IMPLEMENTATION
